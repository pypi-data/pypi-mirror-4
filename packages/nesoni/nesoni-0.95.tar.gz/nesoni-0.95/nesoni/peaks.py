    ...
    def elaborate(self, start, end, once=False):
        while True:
            ...
            for new_start, new_end in [
                ...
                if new_start >= new_end or new_end > self.size or new_start < 0: continue
                any = True
                if (new_start,new_end) in self.step: 
                    all = False
                    continue
                self.queue.append((new_start,new_end))
                self.step[(new_start,new_end)] = step
            self.step[(start,end)] = step//2
            if once or (any and all): break
...
def find_peaks(depth, min_depth, power, min_initial_size=8):
...
@config.help(
    ...
    'Algorithm details:'
    ' Potential peaks are scored as the power-mean of the region (with exponent given by --power)'
...
@config.Float_flag(
    'power',
    '0 < power < 1\n'
    'Larger values of power will suppress calling of peaks near taller peaks.'
    )
@config.Int_flag(
    'min_depth',
...
class Peaks(config.Action_with_prefix):
    ...
    def run(self):
        ...
        for (rname, strand), span_list in spans.items():
            ...
            n = 0
            for start, end in find_peaks(depth, self.min_depth, self.power):
