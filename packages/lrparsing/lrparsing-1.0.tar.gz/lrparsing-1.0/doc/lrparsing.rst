...
.. function:: compile_grammar(grammar)
    ...
    slow.  :func:`Compile_grammar() <compile_grammar>` leaves more
    information about the grammar lying around than
    :func:`pre_compile_grammar` which can be displayed by
    :func:`repr_parse_table()`.
...
string will be cast to a literal :class:`Token`. [#symbol_example]_
...
Although the classes below are used to build :class:`Grammar` rules
...
.. class:: Ref(name)
    ...
    member ``name`` (a :func:`string <str>`).  It is be replaced by that
    member definition, which must occur later.
...
.. class:: Token(literal=None, re=None, case=True, refine=None)
    ...
    :class:`Token`.  The ``refine`` function, if passed, can translate
    a re :class:`Token` into a different :class:`TokenSymbol`.  It is
    called by the :ref:`inbuilt tokeniser <inbuilt-tokeniser>` with
    two arguments, the :class:`TokenRegistry` instance used by the
    :class:`Grammar` and the string matched by the ``re``.  Whatever
    it returns is used as the first element of the :term:`input tuple`
    given to :func:`parse`.
...
.. function:: UnrecognisedToken(refine=None)
    ...
    :exc:`TokenError` instead.  Like re :class:`Tokens <Token>`, the
    :class:`UnrecognisedToken` can be ``refine'd``.
...
``E.compile_grammar()`` this exception is raised::
    ...
    and
      accept the '/' in the hope it will match one of:
      - e = e '/' ^ e
    Please remove this ambiguity from your grammar
...
The source of |lrparsing| contains:

 * A :class:`Grammar` for all of the Data Manipulation Statements in
   Sqlite3 called *lrparsing-sqlite3.py*.

 * A :class:`Grammar` and a cross compiler to Python for Lua 5.2
   called *lua52.py*.
...
As always in the achievements of mankind, if my contribution
manages to advance the art, it is only because I managed to find
some tall shoulders to stand on.
...
.. glossary::
  ...
  Goto Table
      ...
      recognised :term:`non-terminal` onto the :term:`stack` and then
      moving to the next :term:`state` with that :term:`non-terminal`
      recognised is called a :term:`goto`.  The table representing the
      lines of the :term:`state` graph a :term:`state` must move to
      after accepting a :term:`non-terminal` is called the
      ``goto table``.
  ...
  Item Set
      ...
      :term:`non-terminal <non-terminal>`
  ...
  Left Hand Side
      ...
      Such symbols are called a :term:`non-terminal`.
  ...
  LR(1)
      ...
      automatically generated :term:`LR(1)` parsers.
  ...
  Parse Tree
      ...
      An |lrparsing| parse tree for that meaning might look like::
        ...
        (sentence
          ...
          (verb_phrase
            (verb 'like')
            (noun_phrase (determiner 'a') (noun 'arrow')))))
  ...
  Production
      The :term:`LR(1)` names the things that make up a :term:`Grammar` an
      ...
      :term:`LR(1)` productions for you.  You can display those
      productions using :func:`repr_productions`.  Productions are similar
      to :class:`rules <Rule>` in that they have a :term:`left hand side`
      which replaces the symbols on the :term:`right hand side` when they
      are recognised.  The differences between a :class:`Rule` and a
      :term:`production` is a :term:`production` can only be a
      :class:`Sequence`, and alternative derivations for the same
      :term:`non-terminal` are expressed by allowing a
      :term:`non-terminal` to have many :term:`productions <production>`.

  Non-Terminal
      A symbol that appears on the :term:`left hand side` of a
      production.  In terms of |lrparsing| all the attributes of a
      :class:`Grammar` class assigned to a :class:`Symbol` instance
      are :term:`non-terminal`\s.
  ...
  Right Hand Side
      ...
      operation called a :term:`reduce`) by the :term:`left hand side`.
  ...
  Stack
      ...
      :term:`reduce` replaces those matching symbols with the symbol
      on the :term:`left hand side` of the :term:`production`.
      That :term:`symbol` will always (unless it is the
      :term:`start symbol`) be part of another production that the
      parser was in the process of recognising, and now its (probably
      partial) :term:`right hand side` will be what is now on the top
      of the stack.  The stack continues downwards with
      ...
      The :term:`stack` is actually a partially constructed branch
      of the :term:`parse tree`, where the branch runs starts at the
      root.  :term:`Symbols <symbol>` popped from the :term:`stack`
      become nodes in the :term:`parse tree`.
  ...
  Tokeniser
      The thing that turns a text into a stream of :term:`Tokens <Token>`.
...
.. [#token_registry_corollary]
    ...
    :class:`TokenRegistry` otherwise there is no way to pass the
