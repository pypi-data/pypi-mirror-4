...
#    Example: tree.get({namespace}:name) returns value associated with android:name
...
def get_verification_request(android_properties):
    ...
    isProduction = android_properties[ANDROID_CONFIG_IS_PROD_KEY]
    if isProduction:
    ...
    return support.get_base_verification_request(appKey=appKey, appSecret=appSecret)
...
def parse_android_manifest(project_directory):
    """Parse the values of the android manifest

    :param project_directory: Path to the root directory of the Android project to test
...
def is_missing_android_uses_permissions(manifest_element_tree):
...
def is_missing_package_dependent_permissions(manifest_element_tree,
    ...
    # Looking for the <permission android:name "package.name" android:protectionLevel:"signature"
    ...
    # Looking for   <uses-permission android:name="package.foo.permission.C2D_MESSAGE" />
    package_based_permission_is_present = False
    for permission in root.iter("uses-permission"):
        name = permission.get(package_based_permission)
...
def is_missing_application_receiver_attributes(manifest_element_tree):
...
def is_missing_application_intent_filter_action_attributes(manifest_element_tree):
...
def is_airship_configured_properly(airship_api_response,
    ...
    :type key: dict
    :param airship_config_properties: Properties from the airshipconfig.properties file
    :type key: dict
    ...
    if airship_api_response[UA_API_RESPONSE_PACKAGE_KEY] != application_package_name:
        log.error("Package name in manifest {} does not match {} on UA servers".format(
            application_package_name,
            airship_api_response[UA_API_RESPONSE_PACKAGE_KEY]))
        tests.append(False)
    else:
...
def is_analytics_implemented(root_path):
    ...
    try:
        ...
        log.info(
            "analyticsEnabled key not found in properties file, defaulting to true")
    ...
    if is_analytics_enabled is not True:
        log.info(
            "Analytics has been purposefully disabled in airshipconfig.properties")
        return False
    ...
    if len(files_with_analytics) == 0 and is_analytics_enabled is True:
        log.error(
            "Analytics are enabled, but no files extend an instrumented activity")
        return False
