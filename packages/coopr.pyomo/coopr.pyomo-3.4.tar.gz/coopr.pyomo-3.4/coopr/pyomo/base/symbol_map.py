...
__all__ = [ 'SymbolMap', 'NumericLabeler', 'TextLabeler','NameLabeler', 
...
class SymbolMap(object):
    ...
    def __setstate__(self, state):
        ...
        if state['instance'] is None:
            ...
            self.instance = weakref.ref(state['instance'])
        self.byObject = dict((id(val),key) for key,val in state['bySymbol'])
        self.bySymbol = dict((key,weakref.ref(val)) for key,val in state['bySymbol'])
        self.aliases = dict((key,weakref.ref(val)) for key,val in state['aliases'])
    ...
    def updateSymbols(self, obj_symbol_tuples):
        ...
        self.bySymbol.update((symb, weakref_func(obj)) for obj,symb in obj_symbol_tuples)
    ...
    def createSymbol(self, obj, labeler, *args):
        ...
        self.bySymbol[ans] = weakref.ref(obj)
        return ans
    ...
    def alias(self, obj, name):
        ...
        if name in self.aliases:
            ...
            if old_object is None:
                self.aliases[name] = weakref.ref(obj)
                return
