...
class RowMapper(object):
    ...
    def __init__(self, header=None, loose=False, *args, **kwargs):
        self.header = header
        self.loose = loose
        warnings.warn("use 'namedtuple' instead.", DeprecationWarning)
...
class Streamer(object):
    ...
    def consume(self, stream, chunksize=1):
        ...
        rs = ifilter(lambda r: r, stream)
        if self.processes:
            pool = multiprocessing.Pool(processes=self.processes)
            for f in self.procedures:
                rs = pool.imap_unordered(f, ifilter(lambda r: r, rs),
                        chunksize=chunksize)
        else:
            for f in self.procedures:
                rs = imap(f, ifilter(lambda r: r, rs))
        start = time.time()
        i = 0
        try:
            while 1:
