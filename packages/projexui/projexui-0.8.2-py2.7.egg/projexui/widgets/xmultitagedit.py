...
class XMultiTagEdit(XListWidget):
    ...
    def addTag( self, tag ):
        ...
        self.insertItem(self.row(self.createItem()), XMultiTagItem(tag, self))
        self.createItem().setText('')
    ...
    def dropEvent( self, event ):
        ...
        if ( event.source() == self ):
            ...
            if ( curr_item == self.createItem() ):
                return
            
            targ_item = self.itemAt(event.pos())
            if ( not targ_item ):
                targ_item = self.createItem()
    ...
    def handleTagChange( self, item ):
        ...
        if ( item is self.createItem() ):
            self.addTag(self.createItem().text())
