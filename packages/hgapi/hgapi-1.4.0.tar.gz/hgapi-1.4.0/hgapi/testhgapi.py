...
class TestHgAPI(unittest.TestCase):
    ...
    def test_230_BasicDiff(self):
        diff = self.repo.hg_diff('default', 'test_branch')
        self.assertTrue('.hgtags' in diff)
        self.assertTrue('+even more stuff' in diff)

        self.assertTrue(diff.startswith('diff -r'))

    def test_240_DiffFile(self):
        diff = self.repo.hg_diff('default', 'test_branch', filenames=['file.txt'])
        self.assertFalse('.hgtags' in diff)
        self.assertTrue('file.txt' in diff)
        self.assertTrue('+even more stuff' in diff)
        self.assertTrue(diff.startswith('diff -r'))
