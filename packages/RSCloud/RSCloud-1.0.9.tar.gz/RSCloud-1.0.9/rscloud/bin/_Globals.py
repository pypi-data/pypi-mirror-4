...
def apiRequest(obj, endpoint, apiType, data=None, returnResults=False, dataType='json'):
    ...
    try:
        ...
        if re.search("GET", apiType.upper()):
            r = requests.get(endpoint, headers=obj.auth.headers, timeout=timeout)
        elif re.search("DELETE", apiType.upper()):
            r = requests.delete(endpoint, headers=obj.auth.headers, timeout=timeout)
        elif re.search("HEAD", apiType.upper()):
            r = requests.head(endpoint, headers=obj.auth.headers, timeout=timeout)
        elif re.search("POST", apiType.upper()) and data is not None:
            r = requests.post(endpoint, data=json.dumps(data), headers=obj.auth.headers, timeout=timeout)
        elif re.search("POST", apiType.upper()) and data is None:
            r = requests.post(endpoint, headers=obj.auth.headers, timeout=timeout)
        elif re.search("PUT", apiType.upper()) and data is None:
            r = requests.put(endpoint, headers=obj.auth.headers, timeout=timeout)
        elif re.search("PUT", apiType.upper()) and data is not None and dataType == "json":
            r = requests.put(endpoint, data=json.dumps(data), headers=obj.auth.headers, timeout=timeout)
        elif re.search("PUT", apiType.upper()) and data is not None and dataType == "file":
            r = requests.put(endpoint, data=data, headers = obj.auth.headers, timeout=timeout)
        else:
            log.warning('Invalid apiType(verb) or data in _Globals.apiRequest()')
        
        #log data
        obj.reqHeaders = obj.auth.headers
        obj.reqVerb = apiType
        obj.reqEndpoint = endpoint
        obj.reqDataType = dataType
        if dataType == 'json':
            obj.reqData = data
        obj.respHeaders = r.headers
