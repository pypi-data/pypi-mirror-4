...
def ConnectionDone(param=None):
    global _ConnectionDoneCallbackFunc
    if _ConnectionDoneCallbackFunc is not None:
        _ConnectionDoneCallbackFunc()
    return param

def ConnectionFailed(param=None):
    global _ConnectionFailedCallbackFunc
    if _ConnectionFailedCallbackFunc is not None:
        _ConnectionFailedCallbackFunc()
    return param
...
def getPageTwisted(url):
    ...
    if proxy_is_on():
        ...
        factory.deferred.addCallback(ConnectionDone)
        factory.deferred.addErrback(ConnectionFailed)
        return factory.deferred
    else:
        d = getPage(url, agent=_UserAgentString)
        d.addCallback(ConnectionDone)
...
def TestInternetConnection(remote_host='74.125.113.99'): # www.google.com
    try:
...
def getNetworkInterfaces():
    ...
    if plat == 'Windows':
        ...
        def convertsubnetmask(mask):
            ...
            for i in [1, 2, 3, 4]:
                if mask > 7:
                    subnetmask = subnetmask + ".255"
                elif mask == 7:
                    subnetmask = subnetmask + ".254"
                elif mask == 6:
                    subnetmask = subnetmask + ".252"
                elif mask == 5:
                    subnetmask = subnetmask + ".248"
                elif mask == 4:
                    subnetmask = subnetmask + ".240"
                elif mask == 3:
                    subnetmask = subnetmask + ".224"
                elif mask == 2:
                    subnetmask = subnetmask + ".192"
                elif mask == 1:
                    subnetmask = subnetmask + ".128"
                else:
                   subnetmask = subnetmask + ".0"
                mask = mask - 8
            return subnetmask.strip(".")            
        ips = []
        for line in os.popen("/bin/ip -f inet a"):
            check = line.strip('\n').strip().split(' ')
            if check[0] == "inet":
                if check[2] == "brd":
                    check.pop(2)
                    check.pop(2)
                if not temp.has_key(check[4]):
                    temp[check[4]] = []
                ipaddress = check[1].split("/")[0]
                subnet = convertsubnetmask(long(check[1].split("/")[1]))
                scope = check[3]
