...
def load_IRAF_multispec(fitsfilename):
    ...
    data, err, xax, header = pyspeckit.spectrum.readers.fits_reader.read_echelle(fitsfile[0])

    speclist = [pyspeckit.Spectrum(data=data[ii,:], error=err[ii,:], xarr=xax[ii,:], header=header) for ii in xrange(data.shape[0])]
