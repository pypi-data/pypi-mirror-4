...
class display_client_detail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for stat_type in models.stat_type.objects.all():
            client_cvlabels = models.cvlabel_entry.objects.filter(
                host=client,
            )
            for client_cvlabel in client_cvlabels:
                client_stat_entry = models.stat_entry.objects.filter(
                    host=client,
                    type=stat_type,
                    disk=client_cvlabel.disk,
                ).latest()
                ...
                disk_iodata[stat_type.name] = client_stat_entry.value
