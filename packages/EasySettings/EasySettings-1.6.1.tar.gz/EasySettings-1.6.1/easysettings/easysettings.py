...
'''
  ...
  ...easily saves/retrieves settings
     features:
         ...
         pickle/unpickle easysettings. (easier load)
...
if sys.version > '3':
    raise Exception("Python 3 not accepted yet, sorry.")
    exit(1)
...
class easysettings():
        ...
        try:
            ...
            with open(sfile, 'w') as fwrite:
                ...
                for skey in list(self.settings.keys()):
                    val = self.settings[skey].replace('\n', '(es_nl)')
                    fwrite.write(skey + '=' + val + '\n')
                fwrite.flush()
                # success  
                return True
            return False
    ...
    def load_pickle(self, spicklefile = None):
        ...
        try:
            ...
            with open(spicklefile) as fpickle_read:
    ...
    def save_pickle(self, spicklefile = None):
        ...
        try:
            if spicklefile is None:
                spicklefile = self.configfile.replace('.conf', '.pkl')
            with open(spicklefile, 'w') as fpickle_write:
    ...
    def set(self, soption, value = None):
        ...
        try:
            ...
            if isinstance(value, str):
                ...
                # use pickle for non-strings
                try:
                    sval = pickle.dumps(value)
                except:
                    raise esValueError("Illegel value!: " + value)
                    return False
            
            # normal string option/value
            self.settings[soption] = sval
    ...
    def set_list(self, lst_settings):
        ...
        for sset in lst_settings:
            ...
            if "=" in sset:
                ...
                sval = sset[sset.index("=") + 1:].replace('\n', '')
                self.set(sopt, sval)
        return str(lst_settings)
    ...
    def get(self, soption, default = None):
        ...
        if soption in self.settings.keys():
            sval = self.settings[soption]
            # try non-strings first
            try:
                if sval.endswith('\n.'):
                    sval = pickle.loads(sval)
                else:
                    sval = pickle.loads(sval + '\n.')
            except:
                # normal string value
    ...
    def list_settings(self, ssearch_query = None):
        ...
        for skey in (list(self.settings.keys())):
            if ssearch_query is None:
                ...
                try:
                    sval = str(pickle.loads(sval))
                except:
                    sval = self.settings[skey]
                s = skey + "=" + sval
                if str(ssearch_query) in s:
                    lst_tmp.append(s)
        return lst_tmp
    ...
    def list_values(self, ssearch_query = None):
        ...
        if ssearch_query is None:
            ...
            lst_tmp = []
            for itm in (self.settings.values()):
                # <a3
                try:
    ...
    def compare_opts(self, settings1, settings2 = None):
        ...
        for itm in set1.keys():
            if not itm in set2.keys():
                return False
        for itm2 in set2.keys():
            if not itm2 in set1.keys():
                return False
        return True   
    ...
    def compare_vals(self, settings1, settings2 = None):
        ...
        if b_esinstance:
            ...
            return False
        # do the compare        
        for itm in (set1.values()):
            if not itm in (set2.values()):
                return False
        for itm2 in (set2.values()):
            if not itm2 in (set1.values()):
                return False
        return True
...
def test_run(stestconfigfile = None):
    ...
    print ("\nReading:")
    print ("               load_file(): " + str(es.load_file()))
    print ("         read_file_noset(): " + str(es.read_file_noset()))
    print ("             reload_file(): " + str(es.reload_file()))

    print ("\nSaving:")
    print ("                    save(): " + str(es.save()))

    print ("\nSetting:")
    print ("             set('o', 'v'): " + str(es.set('o', 'v')))
    print ("        set('obool', True): " + str(es.set('obool', True)))
    print ("              set('o', 12): " + str(es.set('o', 12)))
    print ("           set('o', 12.24): " + str(es.set('o', 12.24)))
    print ("         set('o', 123456L): " + str(es.set('o', long(123456))))
    print ("   set(['o1=v1', 'o2=v2']): " + str(es.set(['o1=v1', 'o2=v2'])))
    print ("       setsave('o1', 'o2'): " + str(es.setsave('o1', 'o2')))
    print ("\nGetting:")
    print ("                 get('o1'): " + str(es.get('o1')))
    print ("                  get('o'): " + str(es.get('o')))
    print ("           if get('obool'):")
    if es.get('obool'):
        print ("                             Passed. =" + str(es.get('obool')))
    else:
        print ("                             Failed. =" + str(es.get('obool')))
    print ("           list_settings(): " + str(es.list_settings()))
    print ("            list_options(): " + str(es.list_options()))
    print ("             list_values(): " + str(es.list_values()))
    print ("\nGetting Search Query:")
    print ("        list_settings('v'): " + str(es.list_settings('v')))
    print ("         list_options('1'): " + str(es.list_options('1')))
    print ("          list_values('2'): " + str(es.list_values('2')))
    print ("\nGetting Non-String Query:")
    print ("    list_settings(123456L): " + str(es.list_settings(long(123456))))
    print ("      list_values(123456L): " + str(es.list_values(long(123456))))
    
    print ("\nSaved:")
    print ("                is_saved(): " + str(es.is_saved()))
    
    print ("\nRemoving/Clearing:")
    print ("               remove('o'): " + str(es.remove('o')))
    print ("            remove(['o2']): " + str(es.remove(['o2'])))
    print ("        clear_values('o1'): " + str(es.clear_values('o1')))
    print ("      clear_values(['o1']): " + str(es.clear_values(['o1'])))
    print ("                   clear(): " + str(es.clear()))
    
    print ("\nComparison:")
    es2 = easysettings(stestconfigfile.replace('.conf', '2.conf'))
    print ("            compare_vals(): " + str(es.compare_vals(es2)))
    print ("            compare_opts(): " + str(es.compare_opts(es2)))
    print ("                       == : " + str(es == es2))
    print ("                       != : " + str(es != es2))
    print ("                       <= : " + str(es <= es2))
    print ("                       >= : " + str(es >= es2))
    ...
    print ("             save_pickle(): " + str(es.save_pickle(spicklefile)))
    print ("             load_pickle(): ")
