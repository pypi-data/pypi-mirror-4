    ...
    def open( self ):
        ...
        if ext == '.fmf':
            ...
            try:
                ...
                if self.interactive:
                    wx.MessageBox( "I/O error opening \"%s\""%(self.fullpath), "Error", wx.ICON_ERROR )
                raise
        ...
        # unknown movie type
            ...
            try:
                ...
                if self.interactive:
                    wx.MessageBox( "Failed opening file \"%s\"."%(self.fullpath), "Error", wx.ICON_ERROR )
                raise
            else:
                if self.interactive:
                    wx.MessageBox("Ctrax is assuming your movie is in an AVI format and is most likely compressed. Out-of-order frame access (e.g. dragging the frame slider toolbars around) will be slow. At this time, the frame chosen to be displayed may be off by one or two frames, i.e. may not line up perfectly with computed trajectories.","Warning",wx.ICON_WARNING)
                else:
