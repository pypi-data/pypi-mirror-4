...
class MongoUpdateMixin(object):
    ...
    def update(self, data, safe=True, raiseErrors=False):
        ...
        try:
            if self._version == 0:
                ...
                self.__parent__.doInsert(collection, dump,
                    manipulate=False, safe=safe, check_keys=True)
            else:
...
class MongoMappingBase(MongoItemAware, zope.location.location.Location):
    ...
    _m_safe_insert = False
    _m_safe_update = False
    _m_safe_remove = False
    ...
    @property
    def cacheKey(self):
        """Provide a thread local cache key.
        
        Note: this attribute is not exposed in our interface API. Use a better
        cacheKey concept in your implementation.
        """
    ...
    def doInsert(self, collection, data, manipulate, safe, check_keys):
        collection.insert(data, manipulate, safe, check_keys)

    def doUpdate(self, collection, data, upsert, manipulate, safe, multi):
        ...
        for d in data:
            ...
            if d is not None:
                ...
                spec = {'_id': d['_id']}
                collection.update(spec, d, upsert, manipulate, safe, multi)

    def doRemove(self, collection, data, safe):
        if not isinstance(data, list):
            data = [data]
        for d in data:
            ...
            if d is not None:
                ...
                assert d['_id'] is not None
                collection.remove({'_id': d['_id']}, safe)
    ...
    def vote(self):
        ...
        if len(self._t_cache_insert):
            self.doInsert(collection, self._t_cache_insert, manipulate=False,
                safe=self._m_safe_insert, check_keys=True)
        # update existing items
        if len(self._t_cache_update):
            self.doUpdate(collection, self._t_cache_update, upsert=False,
                manipulate=False, safe=self._m_safe_update, multi=False)
