...
class JavadocHtmlParser(HTMLParser):
    def __init__(self):
        super(JavadocHtmlParser, self).__init__(self)
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'a' and 'href' in attrs:
            self.pending_template = '`%%s <%s>`_' % (attrs['href'],)
        elif tag == 'b':
            self.pending_template = '**%s**'
        elif tag == 'i':
            self.pending_template = '*%s*'
        elif tag == 'tt':
            self.pending_template = '``%s``'
    ...
    def handle_endtag(self, tag):
        if tag == self.pending_tag:
            self.output.append(self.pending_template % (''.join(self.pending_data)))
            self.pending_tag = None
