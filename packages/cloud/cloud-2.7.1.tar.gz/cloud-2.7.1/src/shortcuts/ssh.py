...
def get_ssh_info(jid, timeout=None):
    ...
    Blocks until ssh service is ready. An optional ``timeout`` in seconds may be specified
    Returned dictionary provides:
...
def get_ssh_command(jid, timeout=None):
    """Returns shell command that *nix systems can invoke to ssh into the container around job  
    
    Blocks until ssh service is ready. An optional ``timeout`` in seconds may be specified
    """
    
    from ..util.credentials import get_sshkey_path
    ssh_info = get_ssh_info(jid)
    return 'ssh -p {port} -i {identity} -o StrictHostKeyChecking=no -o ConnectTimeout=60 -o LogLevel=QUIET -o UserKnownHostsFile=/dev/null {username}@{address}'.format(**ssh_info)
