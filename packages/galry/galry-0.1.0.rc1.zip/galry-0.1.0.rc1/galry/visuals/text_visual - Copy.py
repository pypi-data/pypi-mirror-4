...
if ((x >= 0) && (x <= 1))
{
    ...
    out_color = texture2D(tex_sampler, coord);
    
    // add color for the text
    out_color = out_color * color;
    
    // if a background is specified, use the background color
    if (background.a > 0) {
        // process transparency manually
        out_color.xyz = out_color.xyz * out_color.a + background.xyz * (1 - out_color.a);
        out_color.a = 1;
    }
}
else
...
class TextVisual(Visual):
    ...
    def initialize(self, text, coordinates=(0., 0.), font='segoe', fontsize=24,
        ...
        # print background
        self.add_uniform("background", vartype="float", ndim=4, data=background)
