...
class XNodeScene( QGraphicsScene ):
    ...
    def __init__( self, view, cellWidth = 20, cellHeight = 18 ):
        ...
        self.setSceneRect( 0, 0, 1170 * cellWidth, 594 * cellHeight )
...
#        processed += nodes
    ...
    def mouseDoubleClickEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            if isinstance(item, XNode) and not self.signalsBlocked():
    ...
    def rebuild( self ):
        ...
        self._centerLines = [ QLine( cx, 0, cx, h ), QLine( 0, cy, w, cy ) ]
        ...
        while ( curr_y < h ):
            ...
            if ( count == 10 ):
                # don't draw over the center line
                if ( curr_y != cy ):
                    major_lines.append( QLine( 0, curr_y, w, curr_y ) )
                count = 1
                
            # don't draw over the center line
            elif ( curr_y != cy ):
                minor_lines.append( QLine( 0, curr_y, w, curr_y ) )
        
            # update the current y location
            curr_y += self.cellHeight()
            count += 1
        ...
        while ( curr_x < w ):
            ...
            if ( count == 10 ):
                # don't draw over the center line
                if ( curr_x != cx ):
                    major_lines.append( QLine( curr_x, 0, curr_x, h ) )
                count = 1
             
            # don't draw over the center line
            elif ( curr_x != cx ):
                minor_lines.append( QLine( curr_x, 0, curr_x, h ) )
             
            # update the current x location
            curr_x += self.cellWidth()
            count  += 1
    ...
    def setZoomAmount( self, amount ):
        ...
        self._mainView.setTransform( QTransform.fromScale( scale, scale ) )
