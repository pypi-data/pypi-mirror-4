...
class Transaction(object):
    ...
    def __init__(self, record=None):
        '''
        Args:
            record (str) -- csv record (defalt=None)
        ...
        if record is not None:
            fields = record.split(";")
            if len(fields) < 6:
                raiseCsb43Exception("""bad format, 6 fields expected, but
    ...
    def _set_date(self, value):
        ...
        if not isinstance(value, datetime.date):
            raiseCsb43Exception("datetime.date expected", strict=True)
        self.__date = value
    ...
    date = property(_get_date, _set_date, None, "date of transaction (datetime.datetime)")
    ...
    amount = property(_get_amount, _set_amount, None, "amount of the transaction")
