...
class NetworkConnector(Automat):
    ...
    def A(self, event, arg):
        ...
        #---CONNECTED---
            ...
            if event == 'timer-5sec' and self.isConnectionAlive():
                self.doDropCounters() 
            elif ( event == 'timer-5sec' and not self.isConnectionAlive() ) or ( event == 'timer-1hour' and not self.isTrafficIN() ):
                self.doDropCounters()
                self.doStun()
                self.state = 'STUN'
        #---DISCONNECTED---
    ...
    def isConnectionAlive(self):
        ...
        if _CounterSuccessConnections >= 3:
            return True
        return _CounterSuccessConnections > _CounterFailedConnections
