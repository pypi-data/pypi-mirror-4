...
#       Copyright © 2011, 2012 Brandon Invergo <brandon@brandoninvergo.com>
#       
...
def get_story_file_meta(file_buffer, truncate=False):
    ...
    if emails is not None or url is not None:
        ifiction.add_contacts(ifiction_dom, story_node, url=gameinfo.get("url"),
            email=emails)
...
def get_story_file_cover(file_buffer):
    for resc_name in ['CoverArt.jpg', 'CoverArt.png']:
        ...
        if cover_resc:
            ...
            cover = CoverImage(cover_resc, ext, width, height)
            return cover
    return None    
