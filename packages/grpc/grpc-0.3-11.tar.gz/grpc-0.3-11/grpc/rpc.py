...
from socket import error as socket_error, AF_INET
import socket as _socket
import gevent
from gevent.socket import EBADF, EWOULDBLOCK, socket, create_connection
...
##        self._send_task = spawn(self._sender)
    ...
    def stop(self):
        ...
        if not self.sock_error:
            try:
                self.call(0, 'remote_stop', tuple(), None, no_result=True)
                sleep(0.05)
            except:
                pass
...
##            pass
    ...
    def _wait_reconnect(self):
        ...
        rs = _reconnected.get(timeout=self.reconnect_timeout)
...
##        self._send_queue.put(dumps(args))
        ...
        if proxy:
            ...
            def _reg_obj(obj):
                ...
                if hasattr(obj, '_rpc_proxy_'):
                    return obj._rpc_proxy_()
                return self.svr.register(obj)
            objs = args[0]
            if isinstance(objs, (tuple, list)):
                obj_ids = []
                for o in objs:
                    obj_ids.append(_reg_obj(o))
            else:
                obj_ids = _reg_obj(objs)
            args = (obj_ids, ) + args[1:]
            dtype |= DT_PROXY
        if pickle:
            dtype |= DT_PICKLE
    ...
    def stop_console(self, shell_id):
        self.call(0, 'stop_shell', (shell_id, ), None, no_result=True, timeout=20)
    ...
    def start_console(self, pre_prompt='', shell=None):
        ...
        shell_id = self.call(0, 'start_shell', (console, pre_prompt), None, proxy=True, timeout=20)
        try:
...
class RpcProxy(object):
    ...
    def __init__(self, export_id, svc=None, addr=None):
        ...
        if addr:
            ...
            self._addr = self._svc.sock.getpeername()
