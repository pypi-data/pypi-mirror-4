...
TODOs:
 * Store dict entries as tuple keys, e.g. myblock.entries[1,2] rather than recursive dicts => v1.6.0
 * Preserve comments from read -> write (needs full-line/inline comment separation?)
...
class Block(object):
    ...
    def add_entry(self, entry):
        ...
        if type(entry) is str:
            raise Exception("Block entries must be tuples or lists")
        if len(entry) < 2:
            raise Exception("Block entry tuples must have at least two entries")
        #print "in", entry
        entry = map(_autotype, entry)
        #print "out", entry
        for e in entry[:-2]:
            if e is not entry[-1]:
                nextparent = nextparent.setdefault(e, _mkdict())
