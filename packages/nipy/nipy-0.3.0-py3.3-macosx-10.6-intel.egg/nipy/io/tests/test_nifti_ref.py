...
def test_xyz_affines():
    ...
    bad_img = fimg.renamed_reference(**dict(list(zip(out_coords, 'xyz'))))
...
def test_unknown():
    ...
    displaced_img = aimg.renamed_reference(
        **dict(list(zip(out_coords[:3], unknown_cs.coord_names))))
    assert_raises(NiftiError, nipy2nifti, displaced_img)
...
def test_dim_info():
    ...
    ncmap = cmap.renamed_domain(
        dict(list(zip(list(range(3)), ('phase', 'slice', 'freq')))))
    ni_img = nipy2nifti(Image(data, ncmap, {'header': hdr}))
...
def test_time_like_matching():
    ...
    aliases = dict(t='time',
                   ...
                   rads='radians/s')
    all_names = set(time_cans + tuple(v for v in list(aliases.values())))
