...
class MDParser(core.Parser):
    ...
    def __init__(self, engine=None):
        core.Parser.__init__(self, engine)
    ...
    def tokens(self, text):
        ...
        cmds = core.CMDRE.cre('cmd').finditer(text)
...
class HTMLSaxHandler(sax.ContentHandler):
    level = None # current level
    codelevel = None # level where code-chunk was found
    codetype = None # 'inline'|'block'
    text = None # list of text collected fragments
    ...
    def _code_element(self, name, attrs):
        ...
        >>> h = HTMLSaxHandler(styles=[], tags=['pre', 'code'])
        >>> h._code_element('pre', {})
        'block'
        ...
        'block'
        >>> h = HTMLSaxHandler(styles=['lpcode'], tags=['pre'])
        ...
        >>> h = HTMLSaxHandler(styles=['lpcode', 'other'], tags=[])
        >>> h._code_element('code', {'class': 'lpcode'})
        'block'
...
class HTMLSaxErrorHandler(sax.ErrorHandler):
...
class HTMLParser(core.Parser):
    ...
    styles = ('lpcode',)
    tags = ('pre', 'code')
    ignore = () # may be 'error', 'fatal', 'warning' (see HTMLSaxErrorHandler)
    ...
    def __init__(self, engine=None):
        ...
        if not (self.styles or self.tags):
            raise ValueError('HTMLParser needs styles or tags')
    ...
    def tokens(self, text):
        htmlsaxhandler = HTMLSaxHandler(styles=self.styles, tags=self.tags)
        htmlsaxerrhandler = HTMLSaxErrorHandler(self, self.ignore)
        # next conversion seems bug in python (why need bytes, not str?!)
        if type(text) is str:
            parsetext = core.strtobytes23(text)
        else:
            parsetext = text
        sax.parseString(parsetext, htmlsaxhandler, htmlsaxerrhandler)
        text = ''.join(htmlsaxhandler.text)
        ...
        for m in cmds:
            token = core.CmdToken(HTMLSaxHandler._decode_spec_chars(m.group(1)), m.start(0), m.end(0))
            tokens.append(token)
        #for m in inlcodes:
        #    # XXX break line in HTML is coded with paragraph styling and this linearizing
        #    # does not help
        #    tokentext = HTMLSaxHandler._decode_spec_chars(m.group(1))
        #    tokentext = core.InlCodeToken.linearize(tokentext)
        #    token = core.InlCodeToken(tokentext, m.start(0), m.end(0))
        #    tokens.append(token)
        for m in blkcodes:
            ...
            tokentext = core.deltextindent(tokentext)
            token = core.BlkCodeToken(HTMLSaxHandler._decode_spec_chars(tokentext), m.start(0), m.end(0))
            tokens.append(token)

        tokens.sort(key=lambda tok: tok.start)
        tokens.append(core.EndToken(None))
        return tokens
core.Parser.register(HTMLParser)
