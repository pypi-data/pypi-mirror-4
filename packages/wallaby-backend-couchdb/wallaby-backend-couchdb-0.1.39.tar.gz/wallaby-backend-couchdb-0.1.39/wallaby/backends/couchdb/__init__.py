...
class Database(object):
    ...
    def _save(self, doc, d, **ka):
        ...
        if '_id' in doc:
            response = yield self.request('PUT', path=urllib.quote(doc['_id'], ""), body=DataProducer(jsonString), **ka)
        elif 'docs' in doc:
    ...
    def _delete(self, doc, d):
        ...
        response = yield self.request('DELETE', path=urllib.quote(doc['_id'], ""), rev=doc['_rev'])
    ...
    def delete_attachment(self, doc, filename):
        ...
        return self.request('DELETE', path=urllib.quote(doc['_id'], "")+'/'+urllib.quote(filename), rev=doc['_rev'])

    def get_attachment(self, doc, filename):
        if not self.assertDocHasAttachment(doc, filename): return self.__error()

        return self.request('GET', path=urllib.quote(doc['_id'], "")+'/'+urllib.quote(filename), protocol=RawProtocol)
    ...
    def _put_attachment(self, doc, filename, data, contentType, d):
        response = yield self.request('PUT', path=urllib.quote(doc['_id'], "")+'/'+urllib.quote(filename), rev=doc['_rev'], body=DataProducer(data), headers={'Content-Type':[contentType]})
    ...
    def _view(self, name, d, includeCount=False, **ka):
        if 'querydoc' in ka:
            ...
            response = yield self.request('POST', path=urllib.quote(name), headers={'Content-Type': ['application/json']}, body=DataProducer(jsonString), **ka)
        else:
            response = yield self.request('GET', path=urllib.quote(name), **ka)
    ...
    def removeCallbacks(self, __id, close=True):
        ...
        for cb in self._changesCBs[__id]:
            if cb is not None and not isinstance(cb, (str, unicode)):
                # Fixme!!
                cb(None, viewID=__id)
