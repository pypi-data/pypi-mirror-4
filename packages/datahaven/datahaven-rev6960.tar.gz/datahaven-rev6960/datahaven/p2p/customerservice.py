...
# save the customer file on our local HDD or 
# save my requested data to restore the backup 
def Data(request):
    if request.OwnerID == misc.getLocalID():
        # restore and io_throttle should take care of this packet
        dhnio.Dprint(6, "customerservice.Data has data packet we restore requested " + request.PacketID)
        return
