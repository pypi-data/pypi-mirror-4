...
def bm_bn_estimate(nn):
    ...
    if np.iterable(nn): return array([bm_bn_estimate(n) for n in nn])
    est = 0.87*nn-0.15
...
def bg_lum_hi(pp, qq):
    ...
    def lum(rr):
        if np.iterable(rr): return array([lum(r) for r in rr])
        ss = rr/reff
...
def bg_lum(pp, qq, reff=1.0, lum=1.0):
    ...
    def luminosity(rr):
        if np.iterable(rr): return array([luminosity(r) for r in rr])
        ss = rr/reff
        zz = (bb/(2*pp))**(2*pp) * ss**(2*qq)
