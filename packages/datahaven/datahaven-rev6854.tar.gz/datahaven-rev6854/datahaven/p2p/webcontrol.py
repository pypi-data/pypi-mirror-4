...
#                dhnio.DprintException()
        ...
        src += '<p><a href="?action=replace">Fire %s and find another person to store My Files</a></p>\n' % self.name
        src += '<p><a href="%s/remotefiles">Show me a list of My Files stored on %s\'s machine</a></p>\n' % (request.path, self.name)
        src += '<p><a href="%s/localfiles">Print a list of My Files stored on my machine but intended for %s</a></p>\n' % (request.path, self.name)
        src += '<br><br>\n'
...
class SupplierRemoteFilesPage(Page):
    ...
    def renderPage(self, request):
        ...
        if action == 'files':
            packetID = customerservice.RequestListFiles(self.supplierNum)
        ...
        src += '<p><a href="%s?action=files">Request a list of My Files from %s</a></p>\n' % (request.path, self.name)
...
class CustomerPage(Page):
    ...
    def renderPage(self, request):
        ...
        src += '<br>\n'
        src += '<p><a href="%s" target="_blank">%s</a></p>\n' % (self.idurl, self.idurl)
        src += '<p><a href="%s?action=remove">Dismis customer %s and throw out His/Her Files from my HDD</a></p>\n' % (request.path, self.name)
        src += '<p><a href="%s/files">Show me %s\'s Files</a></p>\n' % (request.path, self.name)
