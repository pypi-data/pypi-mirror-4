...
class Connection(object):
    ...
    def init():
        ...
        backend_paths = os.path.join(os.path.dirname(__file__),'backends/*.py')
        filenames = glob.glob(backend_paths)
        for filename in filenames:
            modname = os.path.basename(filename).split('.')[0]
            try:
                __import__( 'orb.backends.%s' % modname )
            except ImportError:
                logger.exception( 'Could not import backend: %s' % modname )
