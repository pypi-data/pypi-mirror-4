...
from fbf.utils.url import geturl2, striphtml, useragent
from fbf.utils.exception import handle_exception
from fbf.utils.generic import strippedtxt, fromenc, toenc, jsonstring, getwho, getnick
from fbf.utils.rsslist import rsslist
...
from fbf.utils.id import getrssid
...
from fbf.lib.errors import NoSuchBotType, FeedAlreadyExists, NameNotSet
from fbf.lib.datadir import getdatadir
from fbf.lib.channelbase import ChannelBase
from fbf.imports import getfeedparser, getjson
from fbf.lib.cache import get, set, delete
from fbf.lib.runner import threadrunner
from fbf.contrib.natural.date import duration, delta

## tinyurl import

try: from fbf.plugs.extra.tinyurl import get_tinyurl
except ImportError:
    def get_tinyurl(url):
        return [url, ]
...
sleeptime=15*60, running=0):
        if name:
            filebase = getdatadir() + os.sep + 'plugs' + os.sep + 'fbf.plugs.extra.rss' + os.sep + name
    ...
    def setseen(self, data, itemslist=['title', 'link'], length=200):
        ...
        digest = hashlib.md5(str(d, "utf-8")).hexdigest()
    ...
    def deliver(self, datalist, save=True):
        ...
        try:
            ...
            logging.warn("DELIVERED %s %s items on: %s" % (nritems, name, str(loopover)))
            return nritems
...
class Rssdict(PlugPersist):
    ...
    def watch(self, name, sleepsec=300):
        ...
        worksleeptime.data[name] = sleepsec
        sleeptime.save()
        worksleeptime.save()
...
class Rsswatcher(Rssdict):
    ...
    def changeinterval(self, name, interval):
        ...
        sleeptime.save()
        worksleeptime.data[name] = interval
        worksleeptime.save()
...
def shouldpoll(name, curtime):
    ...
    except KeyError: st = worksleeptime.data[name] = 900 ; worksleeptime.sync()
...
def doperiodical(*args, **kwargs):
    ...
    for feed in watcher.data.names:
        ...
        got = True
        logging.info("launching %s" % feed)
        threadrunner.put(8, feed, dosync, (feed, ))
        time.sleep(1)
...
examples.add("rss-lastpoll", "shows time when a feed was last polled", "1) rss-lastpoll fbfbot 2) rss-lastpoll all")
...
examples.add("rss-nextpoll", "shows time until a feed will be polled", "1) rss-nextpoll fbfbot 2) rss-nextpoll all")
...
examples.add('rss-add', 'add a feed to the rsswatcher', 'rss-add fbfbot https://github.com/feedbackflow/fbfbot/commits/master.atom')
...
examples.add('rss-register', 'register and start a rss feed', 'rss-register fbfbot https://github.com/feedbackflow/fbfbot/commits/master.atom')
...
examples.add('rss-watch', 'start watching a feed', '1) rss-watch fbfbot 2) rss-watch fbfbot 600')
...
examples.add('rss-start', 'start a rss feed (per user/channel) ', 'rss-start fbfbot')
...
examples.add('rss-stop', 'stop a rss feed (per user/channel) ', 'rss-stop fbfbot')
...
examples.add('rss-channels', 'show channels in which a feed runs', 'rss-channels fbfbot')
...
examples.add('rss-addchannel', 'add a channel to watchchannels of a feed', '1) rss-addchannel fbfbot #dunkbots 2) rss-addchannel fbfbot main #dunkbots')
...
examples.add('rss-setitems', 'set tokens of the itemslist (per user/channel)', 'rss-setitems fbfbot author author link pubDate')
...
examples.add('rss-additem', 'add a token to the itemslist (per user/channel)', 'rss-additem fbfbot link')
...
examples.add('rss-delitem', 'remove a token from the itemslist (per user/channel)', 'rss-delitem fbfbot link')
...
examples.add('rss-markup', 'show markup list for a feed (per user/channel)', 'rss-markup fbfbot')
...
examples.add('rss-addmarkup', 'add a markup option to the markuplist (per user/channel)', 'rss-addmarkup fbfbot all-lines 1')
...
examples.add('rss-delmarkup', 'remove a markup option from the markuplist (per user/channel)', 'rss-delmarkup fbfbot all-lines')
...
examples.add('rss-delchannel', 'delete channel from feed', '1) rss-delchannel fbfbot #dunkbots 2) rss-delchannel fbfbot main #dunkbots')
...
examples.add('rss-stopwatch', 'stop polling a feed', 'rss-stopwatch fbfbot')
...
examples.add('rss-sleeptime', 'get sleeping time of a feed', 'rss-sleeptime fbfbot')
...
examples.add('rss-setsleeptime', 'set sleeping time of a feed .. min 60 sec', 'rss-setsleeptime fbfbot 600')
...
examples.add('rss-get', 'get data of a feed', 'rss-get fbfbot')
...
examples.add('rss-url', 'get url of feed', 'rss-url fbfbot')
...
examples.add('rss-seturl', 'set the url of a feed', 'rss-seturl fbfbot https://github.com/feedbackflow/fbfbot/commits/master.atom')
...
examples.add('rss-itemslist', 'get itemslist of feed', 'rss-itemslist fbfbot')
...
examples.add('rss-scan', 'get possible tokens of a feed that can be displayed ', 'rss-scan fbfbot')
...
examples.add('rss-link', 'give link of feeds which title matches search key', 'rss-link fbfbot gozer')
...
examples.add('rss-description', 'give description of item which title matches search key', 'rss-description fbfbot gozer')
...
examples.add('rss-all', "give titles of a feed", 'rss-all fbfbot')
