...
try:
    from gimpy import BBTree, etree_installed
except ImportError:
    ...
    try:
        ...
        if prob.display_mode == 'svg':
            if prob.display_interval is not None:
                prob.Tree.write_as_svg(filename = "%s_%d" % (prob.svg_prefix, prob.last_svg + 1), 
                                       prevfile = "%s_%d" % (prob.svg_prefix, prob.last_svg))
                prob.last_svg += 1
    ...
    def chooseBranchSet(self, xhat):
        ...
        try:
            ...
            if len(diff) > 0:
                ...
                dlb, dub, ulb, uub = self.branch_method(self, xhatDict)
                
                if (dlb or dub) and (ulb or uub):
                    return dlb, dub, ulb, uub
                else:
    ...
    def postProcessNode(self, node):
        ...
        try:
			...
			if gimpy_installed:
				...
				if parentInd == -1:
					if self.layout == 'bak':
						self.Tree.AddOrUpdateNode(str(nodeInd), str(parentInd), branch_direction, BAKstatus, 
												  nodeQuality, None, None)
					else:
						self.Tree.add_root(str(nodeInd), label = label, status = 'C', 
									   obj = nodeQuality, color = color, 
									   style = 'filled', fillcolor = color)
					if self.Tree.display_mode == 'svg':
					  if self.display_interval is not None:
						  if numNodes % self.display_interval in [0, 1]:
							self.Tree.write_as_svg(filename = "%s_0" % self.svg_prefix, 
												   nextfile = "%s_1" % self.svg_prefix, 
												   highlight = nodeInd)
							self.last_svg = 0
					...
					if self.layout == 'bak':
						self.Tree.AddOrUpdateNode(nodeInd, parentInd, branch_direction, 'branched', 
												  nodeQuality, None, None)
					...
					if (self.Tree.display_mode == 'svg'):
					  if self.display_interval is not None:
						  if numNodes % self.display_interval in [0, 1]:
							self.Tree.write_as_svg(filename = "%s_%d" % (self.svg_prefix, self.last_svg + 1), 
												   prevfile = "%s_%d" % (self.svg_prefix, self.last_svg), 
												   nextfile = "%s_%d" % (self.svg_prefix, self.last_svg + 2), 
												   highlight = nodeInd)
							self.last_svg += 1
        ...
        try:
			...
			if gimpy_installed:
				...
				for n in outputDict:
					if n == 'pDownUB':
						if self.layout == 'bak':
							self.Tree.AddOrUpdateNode(str(-numNodes), str(nodeInd), 'L', 
													  'candidate', nodeQuality, None, None)
						else:
							self.Tree.add_left_child(str(-numNodes), str(nodeInd), 
													 label = 'C', 
													 status = 'C', 
													 obj = nodeQuality, 
													 color = 'yellow', 
													 style = 'filled', 
													 fillcolor = 'yellow')
						if len(outputDict[n]) == 1:
							self.Tree.set_edge_attr(str(nodeInd), str(-numNodes), 
													'label', 
													''.join(x for x in 
															str(outputDict[n].keys()[0]) 
															if x not in '()_')
													+ '<=' + 
													str(int(outputDict[n].values()[0])))
						numNodes += 1

					elif n == 'pUpLB':
						if self.layout == 'bak':
							self.Tree.AddOrUpdateNode(str(-numNodes), str(nodeInd), 'R', 
													  'candidate', nodeQuality, None, None)
						else:
							self.Tree.add_right_child(str(-numNodes), str(nodeInd), 
													  label = 'C', 
													  status = 'C', 
													  obj = nodeQuality, 
													  color = 'yellow', 
    ...
    def solveRelaxed(self, key, redCostX, convexDual):
        ...
        try:
            ...
            if len(dvs) > 0:
                ...
                print "Empty variable list in solveRelaxed, changing return value to None"
                return None
  
        except:
            errorStr = "Error in solveRealxed\n" + sys.exc_info()[0]
            raise DipError(errorStr)
    ...
    def generateCuts(self, node):
        ...
        try:
            ...
            if len(cuts) > 0:
                ...
                print "Empty cut list in generateCuts, changing return value to None"
                return None
    ...
    def solveHeuristics(self, xhat, costX):
        ...
        try:
            ...
            if len(sols) > 0:
                ...
                print "Empty solution list in solveHeuristics, changing return value to None"
                return None
    ...
    def generateInitVars(self):
        ...
        try:
            ...
            if len(bvs) > 0:
                ...
                print "Empty variable list in generateInitVars, changing return value to None"
                return None
