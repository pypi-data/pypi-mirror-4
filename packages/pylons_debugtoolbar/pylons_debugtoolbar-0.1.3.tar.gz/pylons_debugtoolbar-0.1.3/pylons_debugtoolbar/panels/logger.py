...
from pylons_debugtoolbar.utils import format_fname
...
class LoggingPanel(DebugPanel):
    ...
    def __init__(self, request, config):
        if asbool(config.get('pdtb.panel_logging', True)):
            self.is_active = True
    ...
    def nav_title(self):
        records = handler.get_records()
        num = len(records)
        return literal('Logging <span class="title-grey">(%d)</span>' % num)
    ...
    def content(self):
        ...
        return self.render('/panels/templates/logger.mako', **vars)
