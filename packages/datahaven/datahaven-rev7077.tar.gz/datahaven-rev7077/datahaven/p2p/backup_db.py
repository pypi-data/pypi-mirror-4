...
class _BackupDB:
    def __init__(self):
        ...
        if os.path.exists(settings.BackupInfoFileFullPathOld()) and not os.path.exists(settings.BackupInfoFileFullPath()):
    ...
    def AddDirectory(self, dirName, recursive=True):
        self.CommandLog("AddDirectory [%s] %s" % (dirName, str(recursive)))
