...
def testGetIncoming(compressed,n,k,index,iterations=1000):
    keys = index.keys()
    with open(compressed,"rb") as fc:
        start = time()
        for i in xrange(iterations):
            offset = find_le(keys,i*n)
            getAllEdges(i,k,n,fc,offset,index[offset][0],index[offset][1],1)
        end = time()
        print "Average time for retrieving incoming edges is", (end-start)/iterations

def testGetOutgoing(compressed,n,k,index,iterations=1000):
    keys = index.keys()
    with open(compressed,"rb") as fc:
        start = time()
        for i in xrange(iterations):
            offset = find_le(keys,i*n)
            getAllEdges(i,k,n,fc,offset,index[offset][0],index[offset][1],2)
        end = time()
        print "Average time for retrieving outgoing edges is", (end-start)/iterations

def testGetAll(compressed,n,k,index,iterations=1000):
    keys = index.keys()
    with open(compressed,"rb") as fc:
        start = time()
        for i in xrange(iterations):
            offset = find_le(keys,i*n)
            getAllEdges(i,k,n,fc,offset,index[offset][0],index[offset][1],0)
        end = time()
        print "Average time for retrieving incoming and outgoing edges is", (end-start)/iterations
...
def testExists(k,n,compressed,index,iterations=1000):
    import random
    random.seed(69)
    keys = index.keys()
    with open(compressed,"rb") as fc:
         start = time()
         for i in xrange(iterations):
             a = random.choice(xrange(n))
             #b = random.choice(xrange(max(a-n/1000,0), min(n,a+n/1000)))
             b = random.choice(xrange(n))
             print a, b, existsEdge(a,b,k,n,fc,index,keys)
         end = time()
         print "Average time for checking if a random edge exists", (end-start)/iterations
...
def main(argv=None):
   ...
   parser.add_argument("-m", "--mode", type=str, dest="mode", default="checkAllEdges", help="interaction mode: checkAllEdges, existsEdge, getAllNonDiagonalEdges, or getAllEdges [default: %(default)s]")
