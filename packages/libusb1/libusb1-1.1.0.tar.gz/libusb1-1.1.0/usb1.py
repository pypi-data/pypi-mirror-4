...
#    """
#    Minmal kqueue wrapper to expose poll-ish API.
#    """
#    def __init__(self, kqueue):
#        self._kqueue = kqueue
#        self._fd_set = set()
#
#    def register(self, fd, events):
#        kevent = select.kevent(fd, flags=select.KQ_EV_ADD)
#        # It's needed to duplicate poll event values, so this works when they
#        # are not available from select module.
#        if events & :
#            kevent.filter = select.KQ_FILTER_READ
#            self._kqueue.control()
#        if events & :
#            kevent.filter = select.KQ_FILTER_WRITE
#        self._kqueue.control([select.kevent(fd, kq_events, select.KQ_EV_ADD)], 0)
#        # fd come from libusb, they are file descriptors, not python objects
#        assert isinstance(fd, int), fd
#        self._fd_set.add(fd)
#
#    def unregister(self, fd):
#        self._kqueue.control([select.kevent(fd, 0, select.KQ_EV_DELETE)], 0)
#        # fd come from libusb, they are file descriptors, not python objects
#        assert isinstance(fd, int), fd
#        self._fd_set.remove(fd)
#
#    def poll(self, timeout=None):
#        self._kqueue.control(None, len(self._fd_set), timeout * 1000.)
