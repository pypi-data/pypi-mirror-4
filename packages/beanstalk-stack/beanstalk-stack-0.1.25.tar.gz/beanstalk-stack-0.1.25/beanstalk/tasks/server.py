...
def setup(settings_path=None, **settings_patches):
    ...
    with temporary_directory(identifier=IDENTIFIER, prefix='init_remote_base') as tmp_directory:
        with lcd(tmp_directory):
            ...
            with lcd(uwsgi_src_folder):
                ...
                config.set('uwsgi', 'main_plugin', 'python')
                config.set('uwsgi', 'inherit', 'base')
                config.set('uwsgi', 'plugin_dir', '/usr/local/lib/uwsgi')
                with open(os.path.join(tmp_directory, uwsgi_src_folder, 'buildconf/beanstalk.ini'), 'w') as f:
                    config.write(f)
                # Build and install
    ...
    beanstalk_apache_conf = render_file(
        beanstalk_apache_conf_template, beanstalk_settings, BEANSTALK_APP_APACHE_CONF=apache_conf_path())
    with open('/etc/httpd/conf.d/beanstalk.conf', 'w') as f:
        f.read(beanstalk_apache_conf)
