...
class MediaShowListingView(BrowserView):
    ...
    def getJSON(self):    
        ...
        if item.portal_type == "Folder" or (item.restrictedTraverse('@@plone').isStructuralFolder()  and item.portal_type != "Topic" and item.portal_type != "Category Navigator"):
        ...
        #Python to JSON encoding
        for res in results:
            if self.getMediaURL(res.getObject()) != "" and self.canView(res.getObject()) :
