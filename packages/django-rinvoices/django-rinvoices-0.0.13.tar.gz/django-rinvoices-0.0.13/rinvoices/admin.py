...
class AdminInvoices(admin.ModelAdmin):
    ...
    inlines = [ LinesInline, ]
    list_filter = ('customer', 'date', 'registered', 'charged')
    ordering = ('-date',)
    ...
    def get_PDF(self, obj):
        return '<a href="/rinvoices/PDF/%s/es">PDFes</a>, <a href="/rinvoices/PDF/%s/en">PDFen</a>, <a href="/rinvoices/txtPDF/%s/es">TXTes</a>, <a href="/rinvoices/txtPDF/%s/en">TXTen</a>' % (str(obj.invoice_number), str(obj.invoice_number), str(obj.invoice_number), str(obj.invoice_number))
