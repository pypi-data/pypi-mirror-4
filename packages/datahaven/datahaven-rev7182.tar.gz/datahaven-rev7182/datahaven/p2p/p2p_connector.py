    ...
    def A(self, event, arg):
        ...
        #---CONNECTED---
            ...
            if ( event == 'network_connector.state' and arg is 'DISCONNECTED' ) :
                ...
                self.state = 'NETWORK?'
            elif ( event == 'network_connector.state' and arg is 'CONNECTED' ) or ( event == 'settings' and self.isIdentityChanged() ) :
                self.doUpdateIdentity(arg)
                self.doUpdateTransports(arg)
                self.state = 'TRANSPORTS'
        #---DISCONNECTED---
        elif self.state is 'DISCONNECTED':
            if event == 'inbox-packet' or ( event == 'settings' and self.isIdentityChanged(arg) ) or ( ( event == 'network_connector.state' and arg is 'CONNECTED' ) ) or ( event == 'timer-1min' and self.isNetworkConnected(arg) ) :
                self.doUpdateIdentity(arg)
                self.doUpdateTransports(arg)
                self.state = 'TRANSPORTS'
            elif ( event == 'network_connector.state' and arg not in [ 'CONNECTED, DISCONNECTED' ] ) :
