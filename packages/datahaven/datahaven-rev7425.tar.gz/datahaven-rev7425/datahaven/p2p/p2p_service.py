...
def inboxPacket(newpacket, proto, host):
    ...
    if newpacket.Command == commands.Fail():
        dhnio.Dprint(6, "p2p_service.inboxPacket got Fail command from [%s]: %s" % (newpacket.CreatorID, newpacket.Payload))
        commandhandled = True
...
def makeFilename(custid, custpacketid):
    if not contacts.IsCustomer(custid):  # SECURITY
        ...
        return ''
    if custpacketid not in [ settings.BackupInfoFileName(), settings.BackupInfoFileNameOld() ] and not packetid.Valid(custpacketid): # SECURITY
        dhnio.Dprint(1, "p2p_service.makeFilename WARNING failed packetid format: " + custpacketid )
        return ''
...
def Data(request):
    ...
    if request.OwnerID == misc.getLocalID():
        if request.PacketID in [ settings.BackupInfoFileName(), settings.BackupInfoFileNameOld() ]:
    ...
    if filename == "":
        dhnio.Dprint(1,"p2p_service.Data WARNING got empty filename, bad customer? " + request.OwnerID)
        return
    data = request.Serialize()
    if not dhnio.WriteFile(filename, data):
        dhnio.Dprint(1, "p2p_service.Data ERROR can not write to " + str(filename))
        return
...
def Retrieve(request):
    filename = makeFilename(request.OwnerID, request.PacketID)
    if filename == '':
        dhnio.Dprint(1,"p2p_service.Retrieve WARNING had bogus customer " + request.OwnerID)
        return
    if not os.path.exists(filename):
        dhnio.Dprint(1, "p2p_service.Retrieve ERROR did not find requested packet " + filename)
        transport_control.outboxNoAck(dhnpacket.dhnpacket(commands.Fail(), request.OwnerID, misc.getLocalID(), request.PacketID, '', request.CreatorID))
        return
    if not os.access(filename, os.R_OK):
        dhnio.Dprint(1, "p2p_service.Retrieve ERROR no read access to requested packet " + filename)
        transport_control.outboxNoAck(dhnpacket.dhnpacket(commands.Fail(), request.OwnerID, misc.getLocalID(), request.PacketID, '', request.CreatorID))
        return
    data = dhnio.ReadBinaryFile(filename)
    if not data:
        dhnio.Dprint(1, "p2p_service.Retrieve ERROR empty data on disk, not Valid packet " + filename)
        transport_control.outboxNoAck(dhnpacket.dhnpacket(commands.Fail(), request.OwnerID, misc.getLocalID(), request.PacketID, '', request.CreatorID))
        return
    packet = dhnpacket.Unserialize(data)
    del data 
    if packet is None:
        dhnio.Dprint(1, "p2p_service.Retrieve ERROR Unserialize data on disk, not Valid packet " + filename)
        transport_control.outboxNoAck(dhnpacket.dhnpacket(commands.Fail(), request.OwnerID, misc.getLocalID(), request.PacketID, '', request.CreatorID))
        return
    if not packet.Valid():
        dhnio.Dprint(1, "p2p_service.Retrieve ERROR unserialized packet is not Valid " + filename)
        transport_control.outboxNoAck(dhnpacket.dhnpacket(commands.Fail(), request.OwnerID, misc.getLocalID(), request.PacketID, '', request.CreatorID))
        return
