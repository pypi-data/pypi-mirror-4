...
class GPGCrypt(object):
    ...
    def import_key(self, key):
        ...
        # Ok.. this code right here supersucks.. we don't get the key
        # identifier as a returned value. Now we have to guess which key we
        # just imported :-(
        base64_data = key.replace('\r\n', '\n').split('\n\n')[1]
        base64_data = base64_data.rstrip().rsplit('\n', 1)[0]
        data = b64decode(base64_data)
        # Intentionally NOT pre-compiling this regex at the top. We expect this
        # function to run less than half the times the application is started.
        email_regex = (r"[a-z0-9!#$%&'*+/=?^_`{|}~-]+"
                       r"(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@"
                       r"(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+"
                       r"[a-z0-9](?:[a-z0-9-]*[a-z0-9])?")
        matches = re.findall(r'<(%s)>' % (email_regex), data, re.I)
        if not matches:
            raise CryptError('GPG quick hack failed; no e-mail found',
                             repr(data))

        # More than one match? Reasonably sized e-mail goes first.
        matches.sort(key=(lambda i: abs(len(i) - 20)))
        for email in matches:
            key = self.get_key(email=unicode(email))
            if key:
                break
        else:
            raise CryptError('GPG quick hack failed; no key found',
                             repr(matches))
