...
static void getSurfaceAttrsSurf(PyObject* py_surface, int *w, int *h,
...
static void getSurfaceAttrs(PyObject* py_surface, int *w, int *h, SDL_PixelFormat** format, Uint32** pixels)
{
...
/* Get surface attributes format, size and pointer to pixels array */
static void getSurfaceAttrsSurf(PyObject* py_surface, int *w, int *h, SDL_PixelFormat** format, SDL_Surface** surface)
{
    PySurfaceObject* c_surface = (PySurfaceObject*) py_surface;
...
SDL_Surface* createSurface( int width, int height ){
    ...
    if(surface == NULL) {
        ...
        fprintf(stderr, "CreateRGBASurface failed: %s\n", SDL_GetError());
    }
    return surface;
}
