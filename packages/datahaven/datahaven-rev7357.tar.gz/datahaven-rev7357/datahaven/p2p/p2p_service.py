...
def inboxPacket(newpacket, proto, host):
    ...
    if newpacket.Command == commands.Fail():
        dhnio.Dprint(6, "p2p_service.inboxPacket got Fail command")
        commandhandled = True
...
def Retrieve(request):
    ...
    if os.path.exists(filename) or not os.access(filename, os.R_OK):
        data = dhnio.ReadBinaryFile(filename)
        packet = dhnpacket.Unserialize(data)
        del data 
        if packet is not None and packet.Valid():
            dhnio.Dprint(6, "p2p_service.Retrieve sending back to " + nameurl.GetName(packet.CreatorID))
            transport_control.outboxNoAck(packet)
        else:
            dhnio.Dprint(1, "p2p_service.Retrieve ERROR found data on disk not Valid packet " + filename)
    else:
