...
class DatePickerWidget(z3c.form.browser.text.TextWidget):
    ...
    # timePostFix is only used in datetime convert for correct the appended
    ...
    def getLabel(self):
        if self.appendLabelDatePattern:
            ...
            pattern = self.translate(self.dateFormatPattern)
    ...
    @property
    def months(self):
        return [self.translate(d).encode('utf-8') for d in MONTHS]

    @property
    def monthsShort(self):
        return [self.translate(d).encode('utf-8') for d in MONTHS_SHORT]
