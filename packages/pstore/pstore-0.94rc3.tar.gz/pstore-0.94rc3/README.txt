...
1. Install the pstore client (see above).
...
3. Go to ``Pstore -> Public keys``. Add a single public key for every
   ...
   ``gpg --export --armor me@my.email``. The key ``key`` should look
...
::
    ...
    sudo pip install ./django_pstore-<version>.tar.gz
...
``django_pstore``. This won't be necessary anymore when we've uploaded
the package (info) to pypi.
...
``django_pstore``. After all, you put the app in a Django project and
...
be cached. Preferably, this is done using the external GPG
application/lib which is better equipped for that kind of task. If that
is unavailable, we cache the passwords ourselves. The notice is there to
remind you that there are more secure ways to handle your data.
