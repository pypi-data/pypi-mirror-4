    ...
    def doStartAllUDPSessions(self):
        ...
        for idurl in all:
            ...
            try:
                ...
                dhnio.DprintException()
                continue
            address = self.remapAddress(address, idurl)
    ...
    def doRemoveOldUDPSession(self, arg):
        for idurl in arg[0]:
            ...
            if idurl not in arg[1]:
                ...
                address_local = self.remapAddress(address, idurl)
...
#        self.event('datagram-received', (datagram, address))
    ...
    def remapAddress(self, address, idurl):
        if identitycache.HasLocalIP(idurl):
            address = (identitycache.GetLocalIP(idurl), address[1])
        return address
