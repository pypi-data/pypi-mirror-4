    ...
    def resolve_ambiguity(cls, item_set, token, action_0, action_1):
        ...
        def err(reason):
            ...
            def explain_action(action):
                ...
                msg = ["  accept the %s in the hope it will match one of:"
                        % token]
                msg.extend([
                        "  - " + repr(lr1_item.lr0_item)
                        for lr1_item in Lr1Item.sorted(iter(action[0]))])
                return msg
...
class Symbol(object):
    ...
    def __mul__(self, other):
        if other is Opt or other is Some or other is Many:
            return other(self)
...
class BinOp(Nonterm):
    ...
    def combine(self):
        arg1, arg2 = self.nested
        if type(self) != type(arg1):
            return self.nested
        if type(self) != type(arg2):
            return arg1.nested + [arg2]
        return arg1.nested + arg2.nested
    ...
    def compile_tokens(self, token_registry, whitespace):
        ...
        if not patterns:
            ...
            for token in sorted(patterns, key=longest):
                ...
                backref_base += len(self.CAPTURE_RE.findall(token.re))
                    ...
                    while offset < last:
                        ...
                        try:
                            ...
                            try:
                                ...
                                token = None
                            if token is None:
                                for tok in self.re_list:
                                    match = re.match(
                                            tok.re, data, self.re_flags)
                                    if match and data == match.group():
                                        token = tok
                                        break
...
class Token(TokenSymbol):
    ...
    KEYWORD = object()
    UNRECOGNISED = object()
    __slots__ = ('case', 'literal', 'pattern', 're', 'refine')
...
def repr_parse_tree(tree, indent=None):
    ...
    if indent is None:
        ...
        indent = ""
    return (' ' if indent is False else '\n').join(indent_tree(tree, ""))
