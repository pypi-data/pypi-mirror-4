...
def load_deploy_setting(project_name=None, deploy_settings_path=None, **setting_patches):
    """Get deploy_settings.
    ...
    env.deploy_settings_dict = deploy_settings_dict = \
        SettingsDict(default_deploy_settings_path, deploy_settings_path, **patches)

    # Find project name - 2
    if project_name is None:
        project_name = getattr(deploy_settings_dict, 'PROJECT_NAME', None)
    if project_name is None:
...
def load_web_servers():
    if len(env.roledefs['web_servers']) == 0:
        with hide('everything'):
            execute('server.web_server', *env.deploy_settings_dict.get('WEB_SERVERS'))
            verbose = env.deploy_settings_dict.get('VERBOSE')
            if verbose != 0:
                puts('')
    if len(env.roledefs['web_servers']) == 0:
...
def add_user(role='web_servers', project_name=None, deploy_settings_path=None, **additional_settings):
    ...
    load_deploy_setting(project_name=project_name, deploy_settings_path=deploy_settings_path, **additional_settings)
...
@task
def setup_server(project_name=None, deploy_settings_path=None, **additional_settings):
    ...
    if not has_eggplant_user:
        ...
        sudo('su -c "git config --global user.name \'eggplant\'" - eggplant')
        sudo('su -c "git config --global user.email eggplant@wantoto.com" - eggplant')

    execute(add_user)

    load_deploy_setting(project_name=project_name, deploy_settings_path=deploy_settings_path, **additional_settings)
    deploy_settings = env.deploy_settings_dict
    remote_base_path = deploy_settings.get('REMOTE_BASE')

    # Make room for deploy target
    puts('Deploy target is: %s' % remote_base_path)
    if not remote_exists(remote_base_path):
        print(yellow('Deploy target doesn\'t exist. Create one now.'))
        sudo('mkdir -p %s' % remote_base_path)
        sudo('chmod o+rx %s' % remote_base_path)
        sudo('chown -R eggplant:eggplant %s' % remote_base_path)
...
def deploy(project_name=None, deploy_settings_path=None, commit_msg='deploy', **additional_settings):
    ...
    :param deploy_settings_path:
        ...
        1. 'env' of current fabric context (env.deploy_settings_dict = <DEPLOY_SETTING_MODULE>)
...
def deploy_to_remote():
    ...
    if not target_file_existence(remote_project_path):
        ...
        target_run('mkdir -p %s' % remote_project_log_root)
