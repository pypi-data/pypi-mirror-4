...
class FileStorage:
    ...
    def __init__(self, directory_path, levels=None):
        ...
        if os.path.exists(directory_path) and os.path.isfile(directory_path):
            raise ValueError("File exists at path for Keeper store: {0}".format(directory_path))
        ...
        # TODO: We should store configuration information in a file in the root
        ...
        if not (os.path.exists(self._meta_root_path) and os.path.isdir(self._meta_root_path)):
            os.mkdir(self._meta_root_path) # TODO: LBYL race condition

        if not (os.path.exists(self._data_root_path) and os.path.isdir(self._data_root_path)):
            os.mkdir(self._data_root_path) # TODO: LBYL race condition
