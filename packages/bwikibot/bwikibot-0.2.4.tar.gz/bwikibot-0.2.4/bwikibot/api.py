...
class Wiki():
    ...
    def __init__(self, endpoint, throttle=1):
    ...
    def session_login(self, login=None, password=None):
...
class File(Page):
    ...
    def download(self, to_file, width=None):
        headers, content = http.request(self._url(width))
...
if __name__ == '__main__':
    ...
    '''
    filename = 'Betta splendens with bubble nest.jpg'
    copy_file(enwiki, cybwiki, filename)
    input('press Enter to end test')
    cybwiki.file(filename).delete('end test')
    '''
    '''
    current_text = ''
    for revision in reversed(list(page.revisions())):
        print_diff(current_text, revision['*'])
