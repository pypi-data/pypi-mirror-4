		...
		for sana in rivi.split(' '):
			...
			if skip_html:
				if not skipataan and "<" in sana:
					skipataan = True
					eka = sana.find('<')
					tavutettava_osa = sana[:eka]
					tavut = syllables(tavutettava_osa)
					tavutettu = add_hyphens(tavut, tavutettava_osa, hyphen, margin)
					sana = tavutettu + sana[eka:]
		
				if skipataan:
					if ">" in sana and sana.rfind('>') > sana.rfind('<'):
						skipataan = False
						vika = sana.rfind('>')+1
						tavutettava_osa = sana[vika:]
						tavut = syllables(tavutettava_osa)
						tavutettu = add_hyphens(tavut, tavutettava_osa, hyphen, margin)
						sana = sana[:vika] + tavutettu
