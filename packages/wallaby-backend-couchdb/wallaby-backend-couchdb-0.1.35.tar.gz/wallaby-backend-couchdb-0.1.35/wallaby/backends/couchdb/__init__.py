...
class Database(object):
    ...
    def _get(self, id, d, rev=None):
        ...
        if rev:
            response = yield self.request('GET', path=self._encodeID(id), rev=rev)
        else:
            response = yield self.request('GET', path=self._encodeID(id), conflicts=True)
    ...
    def _save(self, doc, d, **ka):
        ...
        if '_id' in doc:
            response = yield self.request('PUT', path=self._encodeID(doc['_id']), body=DataProducer(jsonString), **ka)
        elif 'docs' in doc:
    ...
    def _delete(self, doc, d):
        ...
        response = yield self.request('DELETE', path=self._encodeID(doc['_id']), rev=doc['_rev'])
    ...
    def delete_attachment(self, doc, filename):
        ...
        return self.request('DELETE', path=self._encodeID(doc['_id'])+'/'+self._encodeID(filename), rev=doc['_rev'])

    def get_attachment(self, doc, filename):
        if not self.assertDocHasAttachment(doc, filename): return self.__error()

        return self.request('GET', path=self._encodeID(doc['_id'])+'/'+self._encodeID(filename), protocol=RawProtocol)
    ...
    def _put_attachment(self, doc, filename, data, contentType, d):
        response = yield self.request('PUT', path=self._encodeID(doc['_id'])+'/'+self._encodeID(filename), rev=doc['_rev'], body=DataProducer(data), headers={'Content-Type':[contentType]})
