...
class TestMiscPiecewise(unittest.TestCase):
    ...
    def test_activate_deactivate_indexed(self):
        ...
        self.assertTrue(len(model.c[1].subcomponents(Constraint)) > 0)
        self.assertTrue(len(model.c[1].active_subcomponents(Constraint)) > 0)
        model.c[1].deactivate()
        self.assertTrue(len(model.c[1].subcomponents(Constraint)) > 0)
        self.assertEqual(len(model.c[1].active_subcomponents(Constraint)),0)
        model.c[1].activate()
        self.assertTrue(len(model.c[1].subcomponents(Constraint)) > 0)
        self.assertTrue(len(model.c[1].active_subcomponents(Constraint)) > 0)
    ...
    def test_activate_deactivate_nonindexed(self):
        ...
        model.c = Piecewise(*args,**keywords)
        self.assertTrue(len(model.c.subcomponents(Constraint)) > 0)
        self.assertTrue(len(model.c.active_subcomponents(Constraint)) > 0)
        model.c.deactivate()
        self.assertTrue(len(model.c.subcomponents(Constraint)) > 0)
        self.assertEqual(len(model.c.active_subcomponents(Constraint)),0)
        model.c.activate()
        self.assertTrue(len(model.c.subcomponents(Constraint)) > 0)
        self.assertTrue(len(model.c.active_subcomponents(Constraint)) > 0)
