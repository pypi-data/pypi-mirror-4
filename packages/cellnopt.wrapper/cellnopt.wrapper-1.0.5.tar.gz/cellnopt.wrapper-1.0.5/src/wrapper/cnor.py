...
#
##############################################################################
# $Id: cnor.py 2890 2012-12-04 15:33:36Z cokelaer $
    ...
    def plotFit(self, time='T1', plotting='pylab'):
        ...
        if plotting=='pylab':
            try:
                ...
                print("pylab not installed. skipping the plotting. you can try plotting='R' option")
                return
            pylab.figure(1)
            pylab.clf()
...
class CNORfuzzy(CNORBase):
    """
        ...
        f = CNORfuzzy(cnodata("PKN-ToyMMB.sif"), cnolist=CNOlist(cnodata("MD-ToyMMB.csv")))
        f.run(N=3)
        f.compileMultiRes()
        f.plotMeanFuzzyFit()

    """
    def __init__(self, model, data=None, cnolist=None, verbose=False, nTF=7):
        ...
        self.allRes = []
        self.params = wrapper_fuzzy.defaultParametersFuzzy(self.cnolist,
            self.model, nTF)
        if self.verbose:
            print "preprocessing performed when calling run()"
        #self.preprocessing()
    ...
    def run(self, N=1, reset=True, verbose=False):
        ...
        for i in range(0, N):
            ...
            nrows = Res.t1opt.results.nrow
            colnames = Res.t1opt.results.colnames
            for i, name in enumerate(colnames):
                try:
                    data = [float(x) for x in Res.t1opt.results[nrows*i:nrows*(i+1)]]
                except:
                    data = [x for x in Res.t1opt.results[nrows*i:nrows*(i+1)]]

                setattr(Res.t1opt.results, name, data)
    ...
    def _set_stallGenMax(self, data):
        assert data>0
        self.params.rx2['stallGenMax'] = data
    def _get_stallGenMax(self):
        return self.params.rx2('stallGenMax')
    stallGenMax = property(fget=_get_stallGenMax, fset=_set_stallGenMax)
