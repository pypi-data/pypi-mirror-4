...
#            logging.info('%6d %f %s', instance.global_pos, instance.Z, instance.rev_comp and '-' or '+')
    ...
    def __call__(self, starting_point_finder):
        ...
        if not start_results:
            raise RuntimeError('Could not find any starts in the data for these parameter settings.')
        start_results.sort()
...
#        file_handler.handleError = handleError
    ...
    def _load_sequences(self, fasta):
        ...
        if not self.options.max_num_sites:
            # allow number of sites up to 3 * # sequences / 2, with hard limit of 10000 if not user-specified
            hard_limit = 10000
            self.options.max_num_sites = min(3 * len(self.input_sequences.seqs) / 2, hard_limit)
            if hard_limit == self.options.max_num_sites:
                logger.warning('Hit cap of %d on maximum # sites, user can over-ride this with the --max-sites option. High values will slow STEME down.' % hard_limit)
            logger.info('Set maximum number of sites to %d', self.options.max_num_sites)
        if not self.options.min_num_sites:
            # always look for at least 2 sites unless max_num_sites is small
            self.options.min_num_sites = max(len(self.input_sequences.seqs) / 10, min(2, self.options.max_num_sites))
            logger.info('Set minimum number of sites to %d', self.options.min_num_sites)
