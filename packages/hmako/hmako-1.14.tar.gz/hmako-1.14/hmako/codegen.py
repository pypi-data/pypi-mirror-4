...
import time
import re
from hmako.pygen import PythonPrinter
from hmako import util, ast, parsetree, filters, exceptions
from hmako import compat
...
class _GenerateRenderMethod(object):
    ...
    def write_toplevel(self):
        ...
        self.printer.writeline('from hmako import runtime, filters, cache')
    ...
    def write_inherit(self, node):
        ...
        self.printer.writelines(
            'def _hmako_inherit(template, context):',
                '_hmako_generate_namespaces(context)',
    ...
    def write_namespaces(self, namespaces):
        ...
        self.printer.writelines(
            'def _hmako_get_namespace(context, name):',
                'try:',
        ...
        self.printer.writeline('def _hmako_generate_namespaces(context):')
    ...
    def write_variable_declares(self, identifiers, toplevel=False, limit=None):
        ...
        if toplevel and getattr(self.compiler, 'has_ns_imports', False):
            ...
            for ident, ns in self.compiler.namespaces.items():
                if 'import' in ns.attributes:
                    self.printer.writeline(
                            '_hmako_get_namespace(context, %r).'\
        ...
        for ident in to_write:
            if ident in comp_idents:
                ...
                self.printer.writeline(
                            '%s = _hmako_get_namespace(context, %r)' %
                                (ident, ident)
                            )
            else:
    ...
    def visitControlLine(self, node):
        if node.isend:
            ...
            if self.compiler.enable_loop and node.keyword == 'for':
                text = mangle_hmako_loop(node, self.printer)
            else:
                text = node.text
...
def mangle_hmako_loop(node, printer):
