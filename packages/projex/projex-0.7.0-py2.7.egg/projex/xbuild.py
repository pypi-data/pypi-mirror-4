...
APP_INSTALLER = r"""SetOutPath '$INSTDIR\${MUI_PRODUCT}\bin\'
    File /nonfatal /r /x .svn %(DEV_PATH)s\common\%(PLATFORM)s\*
    
    ; create a new shortcut
    CreateShortCut '$DESKTOP\${MUI_PRODUCT}.lnk' '"$INSTDIR\${MUI_PRODUCT}\bin\Python26\pythonw.exe"' '"$INSTDIR\${MUI_PRODUCT}\main.py"' '$INSTDIR\${MUI_PRODUCT}\resources\img\icon.ico'
    """
...
def build( module, 
    ...
    opt['MUI_VERSION']   = module.__dict__.get('__version__', '0.0.0')
    opt['PROJECT_PATH']  = project_path
    opt['PLATFORM']      = platform
    opt['LICENSE']       = license
    opt['DEV_PATH']      = os.environ.get('PROJEX_DEVPATH', '')
    opt['OUTPUT_PATH']   = r'%(PROJECT_PATH)s\bin\%(PLATFORM)s' % opt
    opt['OUTPUT_NAME']   = r'%(MUI_PRODUCT)s-%(MUI_VERSION)s-%(PLATFORM)s.exe' % opt
    ...
    if ( os.getenv('PROJEX_ZIP_NOBUILD') != '1' ):
        ...
        output_fname    = r'%(MUI_PRODUCT)s-%(MUI_VERSION)s.zip' % opt
...
def generate_setup(module, output_path, license='GPL'):
    ...
    options['version']      = module.__version__
    options['author']       = module.__maintainer__
    options['email']        = module.__email__
    options['keywords']     = ''
    ...
    deps = map(lambda x: "'%s'" % x,module.__depends__)
