...
We didn't yet find any official list of **country** INS codes, 
the data used here comes from uncertain sources.
...
ISO2INS = {
 ...
 'DEDE' : '103', # Allemagne (Rép.féd.) / Duitsland (Bondsrep.) / Deutschland (Bundesrep.)'
 'DDDE' : '104', # Allemagne (Rép. dém.) / Duitsland (Dem. rep.) / Deutschland (Dem. Rep.)'
 ...
 'LU' : '113', # Luxembourg (Grand-Duché) / Luxemburg (Groot-Hertogdom) / Luxemburg (Grossherzogtum)'
 ...
 'CS' : '132', # Serbie-et-Monténégro / Servië en Montenegro / Serbien und Montenegro'
 'VA' : '133', # Cité du Vatican (Saint-Siège) / Vatikaanstad (Heilige Stoel) / Vatikanstadt (Heiliger Stuhl)'
 ...
 'HU' : '138', # Hongrie(République) / Hongarije(Republiek) / Ungarn (Republik)'
 'PL' : '139', # Pologne(République) / Polen(Republiek) / Polen(Republik)'
 'CZ' : '140', # République Tchèque / Tsjechische Republiek / Tschechische Republik'
 ...
 'RU' : '145', # Fédération de Russie / Russische Federatie / Russische Föderation'
 ...
 'BA' : '149', # Bosnie-Herzégovine / Bosnië en Herzegovina / Bosnien und Herzegowina'
 ...
 'DDDE' : '170', # Allemagne ( Rép. dém. ) / Duitsland ( Dem. rep. ) / Deutschland ( Dem. Rep. )'
 ...
 'AE' : '260', # Emirats arabes unis / Verenigde Arabische Emiraten / Vereinigte Arabische Emirate'
 ...
 'CF' : '305', # République Centrafricaine / Centraal-Afrikaanse Republiek / Zentralafrikanische Republik'
 ...
 'DO' : '420', # République Dominicaine / Dominicaanse Republiek / Dominikanische Republik'
 'SV' : '421', # El Salvador / El Salvador / El Salvador'
 'TT' : '422', # Trinité-et-Tobago / Trinidad en Tobago / Trinidad und Tobago'
 ...
 'AN' : '482', # Antilles néerlandaises / Nederlandse Antillen / Niederländische Antillen'
 ...
 'PG' : '619', # Papouasie-Nouvelle-Guinée / Papoea-Nieuw-Guinea / Papua-Neuguinea'
        ...
        def find_country(names):
            ...
            for k,v in kw.items():
                try:
                    ...
                    pass
            #~ logger.warning("There's no country %r --> %r",names,kw)
            
        
        for ln in COUNTRIES.splitlines():
            if not ln: continue
            code,names = ln.split(None,1)
            names = names.split("',")
            names = [name.strip()[1:] for name in names]
            country = find_country(names)
            names = ' / '.join(names)
            if country is None:
