...
class DDDHandler(FileHandler):
    ...
    def upload(self):
        ...
        if check.get('name'):
            ...
            print "----- todo, remind ian ----- check for un-uploaded raw frames."
            return check
        ...
        # Create raw HDF
        raw_file = os.path.join(os.path.dirname(self.name), "raw.hdf")
        if os.path.exists(raw_file):
            self.log("Removing existing HDF file of raw frames.")
            os.unlink(raw_file)
        qs['ddd_binary_raw'] = self.create_raw_hdf(raw_file)
        found.add(raw_file)
        ...
        for f in found:
            print "Writing json data: ", f
            self.sidecar_write(f, {"name":rec.get('name')})

        # Remove temporary HDF file
        if os.path.exists(raw_file):
            os.unlink(raw_file)
    ...
    def create_raw_hdf(self, outfile):
        ...
        files = self.find('RawImage', opened=False)
        if not files:
            return []
        self.log("Creating HDF of %s raw frames: %s"%(len(files), outfile))
        outfile = str(outfile)
        # Sort the raw frames
        files = sorted(files, key=lambda x:int(x.split("_")[-1].split(".")[0]))
        for f in files:
            f = str(f)
            self.log("Adding %s to raw HDF: %s"%(f, outfile))
            e = EMAN2.EMData()
            e.read_image(f)
            e['render_min'] = -1
    ...
    # Find files!
    def find(self, ext, found=None, opened=True):
