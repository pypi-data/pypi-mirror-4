...
##############################################################################
"""
$Id: fetch.py 3558 2012-12-19 03:56:24Z roger.ineichen $
"""
...
class FetchScriptRecipe(LoggerMixin):
    ...
    def install(self):
        ...
        if options.get('md5sum'):
            ...
            md5sum =  'None'
        mode = '%s' % int(options.get('mode', '0644'), 8)
...
def process(url, md5sum, target, stripTopLevel, stripSubDirs, ignoreExisting,
    overrideExisting, downloadOnly, filename, mode):
                ...
                if os.path.exists(dest):
                    if ignoreExisting:
                        ...
                        if os.path.isfile(source):
                            ...
                            if mode:
                                os.chmod(dest, mode)
                                print "change mode %s for %s" % (mode, dest)
                    else:
