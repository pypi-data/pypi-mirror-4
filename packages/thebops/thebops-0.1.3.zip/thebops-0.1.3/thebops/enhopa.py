...
NOTE:
    ...
    the next enhopa version will consider this bazaar closed
    and apply its enhancements directly to the last optparse
    version.
"""
__version__ = (0,
               2, # version tuple; tabs; example; etc.
               6, # __all__, above NOTE, some housekeeping
               'rev-%s' % '$Rev: 684 $'[6:-2],
               )
...
try:
    ...
    STD_HELP_OPTION._short_opts.append('-?') # OK :-)
except AttributeError:
    def _add_help_option(self):
        self.add_option("-h", "--help", "-?",
                        action="help",
                        help=_("show this help message and exit"))
    optparse.OptionParser._add_help_option = _add_help_option

def aho(grp):
    optparse.OptionParser._add_help_option(grp.parser)

optparse.OptionGroup.add_help_option = aho
...
class EnhancedHelpFormatter(optparse.IndentedHelpFormatter):
    ...
    def format_option_strings(self, option):
        ...
        If a 'metavar2' attributes is present and not Null,
...
"""))
    ...
    group_ = OptionGroup(parser, 'Demo options')
