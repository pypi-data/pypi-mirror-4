...
def deploy(settings_path=None, **settings_patches):
    ...
    with temporary_directory(identifier=beanstalk_identifier, prefix='deploy~%s~' % project_name) as tmp_directory:
        ...
        with lcd(tmp_directory):
            ...
            local(
                'sed -i.bak -e \'/^\*\.py[c|o]$/d;/^\*\.py\[co\]$/d;$a\\\nlogs\/\' .gitignore && rm -rf .gitignore.bak'
            )
            ...
            run_hooked_actions('PRE_DEPLOY_ACTIONS')
            ...
            run_hooked_actions('POST_DEPLOY_ACTIONS')
...
def deploy_remote_core():
    ...
    print section_title('Create app if it\'s not existed.')
    ...
    print section_title('Send codebase to remote: %s' % white(remote_base_path))
    # Send code
    ...
    if verbose_level == 0:
        ...
        rsync_argument += ' -Pv'
    target = 'beanstalk@%s:%s' % (host_name, remote_source_path)
    local(rsync(env.temp_directory, target, rsync_argument=rsync_argument, expand_to_destination=True))

    # Call post-install script
    run_hooked_actions('POST_RSYNC_ACTIONS')

    # Update this app in server
    print section_title('Update app in server')
    run('bsjack server.update_app:%s,VERBOSE=%d --hide=status' % (project_name, beanstalk_settings['VERBOSE']))

    # Call post-install script
    run_hooked_actions('POST_INSTALL_ACTIONS')

    # Commit state to git repo
    print section_title('Commit current code to git repo')
    run('bsjack server.commit_app:%s,processes=%d,threads=%d,VERBOSE=%d --hide=status' %
        (project_name, beanstalk_settings['PROCESSES'], beanstalk_settings['THREADS'], beanstalk_settings['VERBOSE']))
