...
def mysql_server(root_password, **kwargs):
    ...
    mysql_service = service('mysql', process_name='mysqld', start=False)

    def restart():
        mysql_service.restart()
    file_('/etc/mysql/my.cnf', content=tpl('my.cnf', context=kwargs), on_update=restart, on_create=restart)
