...
class BackupMonitor(automat.Automat):
    ...
    def A(self, event, arg):
        ...
        #---RESTART---
            ...
            if event == 'timer-1sec' and backup_rebuilder.A().state in [ 'STOPPED' , 'DONE' ]:
                self.doPingAllSuppliers()
                self.state = 'PING'
        #---PING---
        elif self.state == 'PING':
            if event == 'timer-10sec' or ( event == 'timer-1sec' and self.isAllSuppliersResponded() ):
    ...
    def isAllSuppliersResponded(self):
        onlines = contact_status.countOnlineAmong(contacts.getSupplierIDs())
        dhnio.Dprint(4, 'isAllSuppliersResponded ackCounter=%d onlines=%d' % (self.ackCounter, onlines))
