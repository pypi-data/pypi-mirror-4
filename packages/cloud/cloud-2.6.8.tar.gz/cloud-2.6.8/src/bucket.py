...
def _compute_md5(f, buffersize = 8192):
    ...
    Returns tuple of hexdigest, base64_encoded md5 hash
    """
...
def _putf(f, obj_path, prefix=None, content_type=None, content_encoding=None):
    ...
    try:
        hex_md5, content_md5 = _compute_md5(f) 
    except IOError:  # not computable (file not seekable)
        hex_md5, content_md5 = None, None
...
def putf(f, obj_path, prefix=None):
    ...
    .. warning::     
        If the file-like object does not implement seek, it will be read entirely into memory 
        before being transferred to PiCloud."""
...
def remove(obj_path, prefix=None):
    """Removes object named ``effective_obj_path`` from PiCloud bucket"""
    conn = _getcloudnetconnection()

    full_obj_path = _get_effective_obj_path(obj_path, prefix)
    resp = conn.send_request(_bucket_remove_query, {'name': full_obj_path})
...
def get(obj_path, file_path=None, prefix=None, start_byte=0, end_byte=None):
    """
...
def getf(obj_path, prefix=None, start_byte=0, end_byte=None):
    ...
    cloud_file = CloudBucketObject( resp, file_size, start_byte, end_byte )
