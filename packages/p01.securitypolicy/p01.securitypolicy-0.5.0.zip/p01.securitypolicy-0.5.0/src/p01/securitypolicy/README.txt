...
This package implements the classic role-based security policy defined in 
zope.securitypolicy. This implementation offers a persistent security map
without using the annotation. The security policy is responsible for
...
grants and denials, objects must be annotatable:
  ...
  >>> class Ob(securitypolicy.SecurityAware):
  ...     pass
...
We can also use the grant info adapter and review our permission settings:

  >>> from zope.securitypolicy.interfaces import IGrantInfo
  >>> grantInfo = IGrantInfo(ob)
  >>> grantInfo.principalPermissionGrant('bob', 'P1')
  PermissionSetting: Deny

  >>> grantInfo.getRolesForPermission('P1')
  [('R1', PermissionSetting: Allow)]

  >>> grantInfo.getRolesForPrincipal('bob')
  [('R1G', PermissionSetting: Allow),
   ('gR1', PermissionSetting: Allow),
   ('R1', PermissionSetting: Deny),
   ('R2', PermissionSetting: Deny),
   ('R3', PermissionSetting: Allow)]
