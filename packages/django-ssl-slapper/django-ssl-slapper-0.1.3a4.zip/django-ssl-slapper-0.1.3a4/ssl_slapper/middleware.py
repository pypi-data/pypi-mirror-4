...
if ADMIN_PAGE is False:
    try:
        ADMIN_PAGE = "%s.*" % reverse('admin:index')
    except:
        ADMIN_PAGE = '.*/admin/.*'

SSL_ONLY_PAGES = getattr(settings, 'SSL_SLAPPER_SSL_ONLY_PAGES', (LOGIN_PAGE,ADMIN_PAGE) )
    ...
    def process_request(self, request):
        ...
        if request.is_secure():
            if SSL_REDIRECT_ANONYMOUS:
                if not request.user.is_authenticated() and not self.is_ssl_only(request.path):
