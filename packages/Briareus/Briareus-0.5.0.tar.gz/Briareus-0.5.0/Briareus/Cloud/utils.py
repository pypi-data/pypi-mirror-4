...
class Cloud(object):
    def __init__(self, f):
        self.f = f
        self.put_client = Client(config.host, "Briareus", pickler=Husky, serialize=True, interval=0.1)
...
class CachedData(object):
    ...
    def put(self, value):
        ...
        KVStore(config.host, self.name).set(self.id, value, serialize=True)

    def get(self, key):
        store = KVStore(config.host, self.name)
        res = store.get(key, serialize=False)
        while not res:
            sleep(0.01)
            res = store.get(key, serialize=False)
