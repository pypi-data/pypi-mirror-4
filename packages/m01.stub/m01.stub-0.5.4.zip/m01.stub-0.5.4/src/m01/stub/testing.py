...
def mongoShellAdminCommand(command, port):
    """
    Send a command to the admin db of the mongo shell.
    The command is basically a javascript snippet
    """
    here = os.path.dirname(__file__)
    mongobin = os.path.join(here, 'sandbox', 'bin', 'mongo')
    stdout = stderr = subprocess.PIPE
    connectionString = '127.0.0.1:%s/admin' % str(port)
    cmd = [mongobin, connectionString, '--quiet', '--eval', command]
    p = subprocess.Popen(cmd, shell=False, stdout=stdout, stderr=stderr)
    out, err = p.communicate()
    returnvalue = p.wait() # wait for the subprocess to finish:
    return (returnvalue, out, err)

def waitForMongoStartup(port, sleep):
    """
    Wait for the mongo shell to answer a status command.
    We are not currently checking for any specific value,
    just an "OK" answer.
    """
    for x in range(int(sleep)):
        returnvalue, out, err  = mongoShellAdminCommand(
                        'print(db.serverStatus()["ok"])', mongoPort)
        if returnvalue == 0 and out.strip() == '1':
            break
        time.sleep(1)
...
def startMongoDBServer(host='localhost', port=45017, options=None,
    ...
    waitForMongoStartup(port, 16)

def stopMongoDBServer(sleep=1.0):
    global mongoPort
    returnvalue, out, err = mongoShellAdminCommand('db.shutdownServer()',
        mongoPort)
