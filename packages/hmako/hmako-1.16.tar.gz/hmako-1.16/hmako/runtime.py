...
from hmako import exceptions, util, compat
from hmako.compat import compat_builtins
import inspect
import sys
import collections
...
def _inherit_from(context, uri, calling_uri):
    ...
    callable_ = getattr(template.module, '_hmako_inherit', None)
    if callable_ is not None:
        ret = callable_(template, lclcontext)
        if ret:
            return ret

    gen_ns = getattr(template.module, '_hmako_generate_namespaces', None)
...
def _populate_self_namespace(context, template, self_ns=None):
    ...
    if hasattr(template.module, '_hmako_inherit'):
        ret = template.module._hmako_inherit(template, context)
        if ret:
            return ret
...
def _render_context(tmpl, callable_, context, *args, **kwargs):
    import hmako.template as template
