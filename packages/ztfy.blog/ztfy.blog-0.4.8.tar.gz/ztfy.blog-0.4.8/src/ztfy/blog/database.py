...
from ztfy.security.interfaces import ISecurityManager, ILocalRoleIndexer
...
def updateDatabaseIfNeeded(context):
    ...
    if 'managers' not in groups:
        group = GroupInformation('Managers (group)', "Group of site services and utilities managers")
        locateAndRegister(group, groups, 'managers', intids)
        ISecurityManager(context).grantRole('zope.Manager', 'grp.managers', False)
    if 'administrators' not in groups:
        group = GroupInformation('Administrators (group)', "Group of site administrators, which handle site's structure")
