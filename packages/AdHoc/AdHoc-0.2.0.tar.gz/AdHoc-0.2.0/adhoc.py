...
#     removed during export, remaining |adhoc_remove_| are renamed to
#     |adhoc_remove| again.
#
...
class AdHoc(object):                                     # |||:cls:|||
    ...
    *Limitations*
    ...
    - All macros must match ``^# *@|:MACRO:|>\\n``
    - Macros cannot be nested

    *Attributes*

    - :attr:`macro_delimiters`

      Delimiters for macros, e.g.: ``@|:MACRO_NAME:|>``

    - :attr:`macro_expansion_delimiters`
    ...
    **User API**
    ...
    # @:adhoc_run_time_section:@ off
        ...
        '''
        # @:adhoc_run_time_section:@ on
        inc_first = lambda dlm: '' if not dlm else dlm[0] + dlm
        inc_last = lambda dlm: '' if not dlm else dlm + dlm[-1]
    ...
    @classmethod
    def expand_macros(cls, source, macro_dlm=None, macro_exp_dlm=None): # |:clm:|
        # @:adhoc_run_time_section:@ off
        """
        ...
        >>> macro_source = '# ' + AdHoc.adhoc_tag('uc_descr_end', AdHoc.macro_delimiters) + '\\n'
        ...
        """
        # @:adhoc_run_time_section:@ on
        if macro_dlm is None:
            macro_dlm = cls.macro_delimiters
        if macro_exp_dlm is None:
            macro_exp_dlm = cls.macro_expansion_delimiters
        import re
        for tag_sym, repl in cls.macros.items():
            tag = cls.adhoc_tag(tag_sym, macro_dlm, False)
            tag_rx = cls.adhoc_tag(tag_sym, macro_dlm, True)
            m_tag = ''.join(('# ', cls.adhoc_tag('adhoc_macro', macro_exp_dlm, False), "\n"))
            mr_tag = ''.join(('# ', cls.adhoc_tag('adhoc_macro_expansion', macro_exp_dlm, False), "\n"))
            repl = ''.join((
                m_tag,
                ''.join(('# ', tag, '\n')),
                m_tag,
            ...
            rx = re.compile(''.join(('^#\\s*', tag_rx, '[^\n]*\n')), re.M)
            source = rx.sub(repl, source)
        return source

    @classmethod
    def has_expanded_macros(cls, source, macro_exp_dlm=None): # |:clm:|
        # @:adhoc_run_time_section:@ off
        """
        """
        # @:adhoc_run_time_section:@ on
        if macro_exp_dlm is None:
            macro_exp_dlm = cls.macro_expansion_delimiters
        m_tag = cls.adhoc_tag('adhoc_macro_expansion', macro_exp_dlm, False)
        me_count = len(cls.tag_lines(source, m_tag))
        return me_count > 0

    @classmethod
    def activate_macros(cls, source, macro_dlm=None, macro_exp_dlm=None): # |:clm:|
        # @:adhoc_run_time_section:@ off
        """
        """
        # @:adhoc_run_time_section:@ on
        if macro_exp_dlm is None:
            macro_exp_dlm = cls.macro_expansion_delimiters
        if not cls.has_expanded_macros(source, macro_exp_dlm):
            source = cls.expand_macros(source, macro_dlm, macro_exp_dlm)
        sv = cls.set_delimiters (macro_exp_dlm)
        source = cls.remove_sections(source, 'adhoc_macro')
        source = cls.section_tag_remove(source, 'adhoc_macro_expansion')
        cls.reset_delimiters(sv)
        return source

    @classmethod
    def collapse_macros(cls, source, macro_exp_dlm=None):    # |:clm:|
        # @:adhoc_run_time_section:@ off
        """
        """
        # @:adhoc_run_time_section:@ on
        if macro_exp_dlm is None:
            macro_exp_dlm = cls.macro_expansion_delimiters
        if cls.has_expanded_macros(source, macro_exp_dlm):
            sv = cls.set_delimiters (macro_exp_dlm)
            source = cls.remove_sections(source, 'adhoc_macro_expansion')
            source = cls.section_tag_remove(source, 'adhoc_macro')
            cls.reset_delimiters(sv)
        return source
    ...
    def get_templates(cls, file_=None, source=None,          # |:clm:|
        ...
        for template_section in template_sections:
            ...
            if not tag_arg:
                tag_arg = '-'
            if tag_arg in cls.template_hooks:
                section = cls.template_hooks[tag_arg](cls, section, tag, tag_arg)
            if ignore_mark:
                tag_arg = '-'
    ...
    def encode_module_(                                      # |:mth:|
        self, module, for_=None, indent='', zipped=True, flat=None):
        m = 'gm: '
        ...
        for parent_module in parent_modules:
            result.append(self.encode_module_(
                parent_module, for_, indent, zipped, flat))
            ...
            if section[0]:
                ...
                if mo:
                    ...
                    if mo:
                        ...
                        module = mo.group(2)
                if len(module) > 0:
                    flat = True if 'flat' in import_args.lower().split() else None
                    flat = False if 'full' in import_args.lower().split() else flat
                    source = self.encode_module_(module, for_, indent, zipped, flat)
    ...
    def encode_module(                                       # |:mth:|
        ...
        if self.verbose:
            ...
            sys.stderr.write(sformat(
                '{0}--------------------------------------------------\n',
                dbg_comm))
        return self.encode_module_(module, for_, indent, zipped, flat)
...
def get_readme(file_=None, source=None, as_template=False, transform=True): # ||:fnc:||
    ...
    tpl_hooks = AdHoc.template_hooks
    AdHoc.template_hooks = {template_name: tpl_hook_readme}
    template = AdHoc.get_named_template(template_name, file_, source)
    AdHoc.template_hooks = tpl_hooks
...
def main(argv):                                            # ||:fnc:||
    ...
    if adhoc_op:
        ...
        AdHoc.template_hooks = {'doc/index.rst': tpl_hook_doc_index_rst}
