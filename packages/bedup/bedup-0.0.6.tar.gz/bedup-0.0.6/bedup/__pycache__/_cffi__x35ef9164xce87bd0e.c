...
_cffi_layout_struct_btrfs_ioctl_ino_lookup_args(PyObject *self, PyObject *noarg)
{
  ...
  static Py_ssize_t nums[] = {
    ...
    offsetof(struct btrfs_ioctl_ino_lookup_args, name),
    sizeof(((struct btrfs_ioctl_ino_lookup_args *)0)->name),
    -1
  };
  return _cffi_get_struct_layout(nums);
  /* the next line is not executed, but compiled */
...
PyMODINIT_FUNC
init_cffi__x35ef9164xce87bd0e(void)
{
  PyObject *lib;
  lib = Py_InitModule("_cffi__x35ef9164xce87bd0e", _cffi_methods);
  if (lib == NULL || 0 < 0)
    return;
  _cffi_init();
  return;
