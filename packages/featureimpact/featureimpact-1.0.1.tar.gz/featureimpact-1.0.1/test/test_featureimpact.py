...
class Test(unittest.TestCase):
    ...
    def test_get_averaged_impact(self):
        impact = []
        self.assertTrue((numpy.array([]) == get_averaged_impact(impact)).all())
        impact = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
        impave = get_averaged_impact(impact)
        self.assertTrue((numpy.array([4, 5, 6]) == impave).all())
        impavenorm = get_averaged_impact(impact, normalize=True)
