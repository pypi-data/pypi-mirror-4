...
class ResourceFactory(object):
    ...
    def __init__(self, path, checker, name, rPath=None):
    ...
    def getResourceManager(self, request):
        ...
        return interfaces.IResourceManager(request)
...
class I18nResourceFactory(object):
    ...
    def __init__(self, data, name, defaultLanguage, checker):
        self.__data = data
        self.__uris = []
        self.__name = name
    ...
    def __call__(self, request):
        manager = self.getResourceManager(request)
        resource =  I18nCDNResource(self.__manager, self.__data, request,
...
class SubDirectoryResourceFactory(object):
    ...
    def __init__(self, data, path, checker, name, rName, excludeNames):
        self.data = data
        self.__path = path
        self.__checker = checker
        self.__name = name
...
class DirectoryResourceFactory(object):
    ...
    def __init__(self, path, checker, name, excludeNames):
        self.__path = path
        self.__checker = checker
        self.__name = name
        self.__excludeNames = excludeNames
        ...
        for name in os.listdir(path):
            ...
            if os.path.isdir(objPath):
                ...
                data[name]['factory'] = SubDirectoryResourceFactory(subData,
                    path, self.__checker, name, rPath, self.__excludeNames)
            else:
                data[name]['factory'] = ResourceFactory(objPath,
                    self.__checker, name, rPath)
        return data
...
class ZRTCDNResource(CDNResource):
    ...
    def GET(self):
        ...
        zp = ZRTProcessor(data, commands={'replace': Replace})
