...
from genericSymbols import(Symbol, VerticalSymbol, Comment, EditableText,
        Cornergrabber)
...
def nextstate_merge_list(terminators):
    '''
        Helper function:
    ...
    for term in terminators:
        if term.pos_y is not None and term.kind == 'next_state':
...
class SDL_Scene(QGraphicsScene):
    ...
    def copy_selected_symbols(self):
        ...
        if ((branch_top_level == []) == (floating_items == []) or
            ...
            self.messages_window.addItem('Selection is incompatible with copy')
            return False
        ...
        for item in branch_top_level + floating_items:
            try:
                ...
                self.messages_window.addItem('ERROR: cannot copy '
    ...
    def copy_branch(self, top_level_item):
        ...
        if not isinstance(top_level_item, (Input, DecisionAnswer)):
            ...
            while next_aligned and next_aligned in self.selected_symbols():
    ...
    def paste_symbols(self):
        ...
        if len(selection) > 1:
            ...
            for item_list, terminators in COPY_PASTE:
                ...
                if self.click_coordinates:
                    ...
                    for term in terminators:
                        term.pos_x += translation.x()
                        term.pos_y += translation.y()
                next_states_coord = nextstate_merge_list(terminators)
                ...
                for i in text_areas:
                    ...
                    self.undo_stack.push(undo_cmd)
                    new_item.cam(new_item.pos(), new_item.pos())
                # Reset translations to allow for another copy
                if self.click_coordinates:
                    for term in terminators:
                        term.pos_x -= translation.x()
                        term.pos_y -= translation.y()
            ...
            LOG.debug('Pasting below item ' + repr(selection))
            self.undo_stack.beginMacro('Paste items')
            for item_list, terminators in COPY_PASTE:
                next_states_coord = nextstate_merge_list(terminators)
                states = [i for i in item_list if isinstance(i, ogAST.State)]
    ...
    def getPrData(self):
        ...
        if not start_state:
            self.messages_window.addItem('[WARNING] START Symbol missing - '
    ...
    def add_input(self):
        ...
        if not selection:
            ...
            self.place_input(parent=selection)
            self.undo_stack.endMacro()

    def place_connected_item(self, itemType, parent, pos=None):
    ...
    def insertConnectedItem(self, itemType):
        ...
        if not selection:
            ...
            self.messages_window.addItem(msg)
            ...
            selection, = selection
            return self.place_connected_item(itemType, parent=selection)
    ...
    def add_decision(self):
        ...
        if not selection:
            ...
            selection, = selection
            self.place_decision(parent=selection)

    def place_decision(self, parent):
    ...
    def add_answer(self):
        ...
        if not selection:
            ...
            selection, = selection
            return self.place_answer(parent=selection)

    def place_answer(self, parent):
        ''' Place an ANSWER symbol on the scene below a decision '''
        self.clearSelection()
        self.clearFocus()
        if isinstance(parent, DecisionAnswer):
            local_select = parent
            local_select.parentItem().select()
        else:
            parent.select()
    ...
    def place_text(self, pos):
        ''' Place the floating text symbol on the scene '''
        text = TextSymbol()
        self.addItem(text)
    ...
    def add_process_from_ast(self, process):
        ...
        next_states_coord = nextstate_merge_list(process.terminators)
    ...
    def render_transition_from_ast(
            self, parent, ast, states, next_states_coord):
        ''' Add a transition to the diagram, from the PR-parsed AST '''
        for action in ast.actions:
            parent = self.render_action_from_ast(action, parent, states,
                    next_states_coord=next_states_coord)

        if ast.terminator:
            self.render_terminator_from_ast(ast.terminator, parent, states,
                    next_states_coord)

    def render_terminator_from_ast(
            self, ast, parent, states, next_states_coord):
        '''
            Add a terminator to the scene, from the AST, and in the case of a
            NEXTSTATE, possibly merge it with a defined STATE
        '''
        LOG.debug('Render terminator' + str(ast.__dict__))
        if ast.label:
            parent = self.render_action_from_ast(
                    ast.label, parent, states,
                    next_states_coord=next_states_coord)
        if ast.kind == 'next_state':
            #tx, ty, tw, th = ast.coordinates  # or (None, None, None, None)
#            st = None
...
#                and ast.inputString == str(next_states_coord[(tx, ty)].text)):
...
#            else:
            ...
            terminator = State(parent=parent, ast=ast)
            next_states_coord[(ast.pos_x, ast.pos_y)] = terminator
            # Check if a corresponding state with inputs exists
            # If so, render the inputs too.
#            try:
                # if no states are defined ignore
            for state in states:
                if (state.inputString == ast.inputString and
                        state.pos_x == ast.pos_x and state.pos_y == ast.pos_y):
                    for i in state.inputs:
...
#                pass
            ...
            terminator = Join(parent=parent, ast=ast)
        else:
            LOG.error('Unsupported terminator: ' + repr(ast))
            return

    def render_action_from_ast(
            self, action, parent, states, next_states_coord):
        ''' Render an action symbol on the scene '''
        new_parent = None
        if isinstance(action, ogAST.Task):
            task = Task(parent=parent, ast=action)
            task.updateConnectionPoints()
            new_parent = task
        elif isinstance(action, ogAST.Output):
            if action.kind == 'output':
                out_call = Output(parent=parent, ast=action)
            elif action.kind == 'procedure_call':
                out_call = ProcedureCall(parent=parent, ast=action)
            new_parent = out_call
            out_call.updateConnectionPoints()
        elif isinstance(action, ogAST.Decision):
            dec = Decision(parent=parent, ast=action)
            ...
            label = Label(parent=parent, ast=action)
            label.updateConnectionPoints()
            new_parent = label
        elif isinstance(action, ogAST.Answer):
            ans = DecisionAnswer(parent=parent, ast=action)
            ...
            self.render_terminator_from_ast(
                    action, parent, states, next_states_coord)
        else:
            LOG.error('Unsupported symbol in branch: ' + repr(action))
        return new_parent

    def render_start_from_ast(self, ast, states, next_states_coord):
        ''' Add the start symbol from the AST '''
        start = Start(ast=ast)
        if start not in self.items():
            self.addItem(start)
        G_SYMBOLS.add(start)
...
class SDL_View(QGraphicsView):
    ...
    def load_file(self, filename):
        ...
        self.wrapping_window.setWindowTitle('process ' + PROCESS_NAME + '[*]')
        ...
        if pr_data:
            ...
            if len(errors) > 0:
                ...
                try:
                    ...
                    self.messages_window.addItem('Code generation failed')
...
class OG_MainWindow(QMainWindow):
    ...
    def start(self, file_name):
        ...
        self.scene.selectionChanged.connect(lambda:
                                    self.scene.set_selection(toolbar)) #toolbar.update_menu)
