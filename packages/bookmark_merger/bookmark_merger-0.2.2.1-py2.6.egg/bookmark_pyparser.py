...
endBM=pp.Literal("</A>") + pp.Optional(pp.LineEnd()+pp.Literal("<DD>")+pp.SkipTo(pp.LineEnd()+pp.ZeroOrMore(pp.White())+"<")) #^ (pp.Literal("</A>")+pp.Literal("<DD>")+pp.SkipTo(pp.LineEnd()))
...
foldernameend=pp.Literal("</H3>") ^ (pp.Literal("</H3>")+pp.LineEnd()+pp.Literal("<DD>")+pp.SkipTo(pp.LineEnd()))
...
#parser 
bookmarkshtml=pp.oneOf(headers)+H1+pp.Optional(startlist)+pp.ZeroOrMore(foldercontents)+endlist 
