...
from .const import Status, INDENTST, term, IDENTIFIERS
from .exc import (PregoAssertionException, log_traceback)
...
#        return FailAssertion(self)
    ...
    def remove_gen(self):
        ...
        if config.dirty:
            self.log.info('%s  $name dirty mode: generated files not removed',
                          Status.indent())
            return

        self.log.debug("%s$name   removing %s", INDENTST, self.gen)
        for f in self.gen:
            f.remove()

    # FIXME: puede no haber comando (o haber varios)
    # FIXME: log format
