...
if __name__ == '__main__':
    def get_pubkey_id_from_binary(data, bindata_class=BinaryData):
        bindata = bindata_class(data)
        for packet in bindata.packets():
            if packet.raw == 6:
                return packet.key_id

    # FIXME: proper tests..
    # FIXME: fall back to this only if pgpdump is NOT found?
    import sys
    data = b64decode(open(sys.argv[1], 'r').read().replace('\n', ''))

    print get_pubkey_id_from_binary(data)
    from pgpdump import BinaryData as OrigBinaryData
    print get_pubkey_id_from_binary(data, OrigBinaryData)
