...
logger = getLogger("EXISTDA.xslt")

try:
    from Ft.Xml.Xslt import Processor
    from Ft.Xml.InputSource import DefaultFactory
    from Ft.Xml import Domlette, cDomlette
    logger.info("4Suite found. External XSLT will be available for eXistDA.")
except:
    logger.warning(utils.EXC_NO_4SUITE)
...
def xsltproc(xsl, xml, params, uri, encoding=utils.DEFAULT_ENCODING):
    ...
    if (utils.DEBUG > 2):
        logger.info("xsltproc command line is : \n----------\n%s\n----------" % cmd)
    ...
    try:
        ...
        if (utils.DEBUG > 0):
            logger.info("xsltproc couldn't return correct encoding")
        return html
