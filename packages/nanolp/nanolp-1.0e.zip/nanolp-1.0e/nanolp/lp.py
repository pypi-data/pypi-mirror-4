...
class UseCmd(Cmd):
    ...
    def onpost(self, parser, flush):
        ...
        inclfilename = ''.join(self.body)
        mnt = self.getarg('mnt', '')
        sys.stderr.write("using '%s' mounted to '%s'...\n"%(inclfilename, mnt))
        inclparser = Parser.fileparser(inclfilename)
        inclparser.parsefile(inclfilename, flush=False)
...
class FileCmd(Cmd):
    ...
    def onpost(self, parser, flush):
        if flush:
            ...
            ofilename = ''.join(self.body)
            ofilename = os.path.join(parser.indir, ofilename)
            sys.stderr.write("writing to '%s'...\n"%ofilename)
            fwrite23(ofilename, chunk.tangle)
...
class Chunk:
    ...
    deps = None # dependencies for this chunk
    # for cycle detection:
...
# TODO all errors from parser should have line number (problem token position),
# if it's possible. So, err reporting with ParserError can show error line
class ParserError(Exception): pass
    #errloc = None
    #def __init__(self, msg, errloc=None):
        #self.errloc = errloc
...
class Parser:
    ...
    def parsefile(self, filename, flush=True):
        ...
        """
        self._reset()
        self.indir = os.path.dirname(os.path.abspath(filename))
    ...
    def parse(self, text):
        ...
        try:
            ...
            if not errlocation:
                ...
                reraise23(cls, "[%s, %d] %s"%(file, line, str(x)), tb)
...
# FIXME make one-based lines numbers
class ErrorLocator:
