...
TODOs:
   ...
   * Only allow integers to be used as block entry indices, to allow multiple
     values (request, if possible, from Anders & Are).
...
class Block(object):
    ...
    Multiple indices are possible, especially for entries in mixing matrix
    blocks. These are now implemented in the natural way, e.g. for access to the
    (1,2) element of a mxing matrix block, use bmix[1,2] = 0.123 and print
    bmix[1,2]. The value() and set_value() functions behave similarly.
    ...
    def add_entry(self, entry):
        """Add an entry to the block from an iterable (i.e. list or tuple) or
        whitespace-separated string.

        Indexing will be determined automatically such that there is always a
        single-element value: multi-value or None indices may be constructed
        implicitly.
        """
        ...
        if len(entry) == 1:
            self.entries[None] = entry[0]
        elif len(entry) == 2:
            self.entries[entry[0]] = entry[1]
        else:
            self.entries[tuple(entry[:-1])] = entry[-1]
    ...
    def set_value(self, *args):
        """Set a value in the block with supplied key, val args (or just val for
        an unindexed block).
        ...
        if len(args) == 0:
            ...
            self.entries[tuple(args[:-1])] = args[-1]
