...
logger = getLogger("eXistDASyn - query")
...
* 'kws' : other parameters
  ...
  * 'variables' : dict containing variables to pass to the xquery
  """
        ...
        if utils.DEBUG >= 3:
            logger.info("Entering query method")
        ...
        for DA in self.objectValues([utils.EXISTDA_METATYPE]):
            if utils.DEBUG >= 2:
                logger.info("Querying %s" % DA.server)
            #try:
            DA_prot = self.restrictedTraverse(DA.id)
            if utils.DEBUG >= 2:
                logger.info("DA_prot for this DA is %s" % str(DA_prot))
            if not(DA_prot.server in old_DAs) and DA_prot != None:
                ...
                if utils.DEBUG >= 1:
                    logger.info("resSet is %s" % str(resSet))
                    logger.info("thisDA_resSet is %s" % str(thisDA_resSet))
                ...
                if utils.DEBUG >= 3:
                    logger.info("Adding query result from %s to syndicator resultsSet")
                    logger.info("My new resultsSet is : %s " % resSet.getMergedXML())
