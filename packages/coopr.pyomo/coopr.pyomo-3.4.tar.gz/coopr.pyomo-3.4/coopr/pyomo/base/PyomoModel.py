        ...
        for component_name, component in iteritems(self.subcomponents()):
            ...
            if report_timing is True:
                ...
                print("    %%6.%df seconds required to construct component=%s; %d indicies total" \
                      % (total_time>=0.005 and 2 or 0, component_name, len(getattr(self, component_name))) \
                      % total_time)
                tmp_clone_counters = (
                    coopr.pyomo.base.expr.generate_expression.clone_counter,
                    coopr.pyomo.base.expr.generate_relational_expression.clone_counter,
    ...
    def _load_solution( self, soln, symbol_map,
        ...
        for _attr_key, attr_value in iteritems(soln.problem):
            ...
            if (attr_key not in ['id']):
                ...
                suff = valid_import_suffixes.get(attr_key,None)
                if suff is not None:
                    suff.setNumericValue(self,attr_value.value)
        ...
        for label,entry in iteritems(soln.objective):
            ...
            for _attr_key, attr_value in iteritems(entry):
                attr_key = _attr_key[0].lower() + _attr_key[1:]
                if (attr_key not in ['id','canonical_label','value']):
                    # GAH: Unlike the var suffix information in the solution object,
                    ...
                    suff = valid_import_suffixes.get(attr_key,None)
                    if suff is not None:
                        suff.setNumericValue(obj_value,attr_value.value)
        ...
        for label, entry in iteritems(soln.variable):
            ...
            for _attr_key, attr_value in iteritems(entry):
                ...
                if attr_key == 'value':
                    ...
                    suff = valid_import_suffixes.get(attr_key,None)
                    if suff is not None:
                        suff.setNumericValue(var_value,attr_value)

        #
        # Load constraint data
        #
        for label,entry in soln.constraint.items():
            ...
            for _attr_key, attr_value in entry.items():
                attr_key = _attr_key[0].lower() + _attr_key[1:]
                if attr_key != 'id' and attr_key != 'canonical_label':
                    # GAH: Unlike the var suffix information in the solution object,
                    ...
                    suff = valid_import_suffixes.get(attr_key,None)
                    if suff is not None:
                        suff.setNumericValue(con_value,attr_value.value)
                        
    def write(self, filename=None, format=ProblemFormat.cpxlp, solver_capability=None, symbolic_solver_labels=False):
        """
        ...
        (filename, symbol_map) = problem_writer(self, filename, solver_capability, symbolic_solver_labels)
