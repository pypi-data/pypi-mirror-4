...
class TransportUDPServer():
    ...
    def datagramReceived(self, datagram, addr):
        ...
        try:
            ...
            if addr[0] in black_IPs_dict().keys():
                dhnio.Dprint(6, 'transport_udp_server.datagramReceived %d bytes from BLACK IP: %s, %s' % (len(datagram), str(addr[0]), str(black_IPs_dict()[addr[0]])) )
                return
        except:
            pass
        if len(datagram) < 2:
