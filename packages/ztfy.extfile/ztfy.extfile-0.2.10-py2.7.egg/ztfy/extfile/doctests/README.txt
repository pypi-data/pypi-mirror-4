...
If you need to access blob data in the same transaction in which you defined it's content, you have to commit the
transaction before reading data to avoid an "Uncommitted changes" error:

    >>> # ITransactionManager(doc.data).get().commit()
    >>> #doc.data.data.startswith('====================\nZTFY.extfile package\n====================')
    #True
