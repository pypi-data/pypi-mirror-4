...
class FileToRequest:
    def __init__(self, callOnReceived, creatorID, packetID, ownerID, remoteID):
        ...
        self.requestTimeout = int(settings.PacketSizeTarget() / settings.SendingSpeedLimit())
        self.result = ''
...
class FileToSend:
    def __init__(self, fileName, packetID, remoteID, ownerID, callOnAck=None, callOnFail=None):
        ...
        self.sendTime = None
        self.ackTime = None
        self.sendTimeout = max(int(self.fileSize / settings.SendingSpeedLimit()), 20)
        self.result = ''
...
class SupplierQueue:
    def __init__(self, supplierIdentity, creatorID):
        ...
        # active files 
        self.fileSendMaxLength = 4 # 1 mean sending files one by one! 
        ...
        # active requests 
        self.fileRequestMaxLength = 4 # do requests one by one   
    ...
    def FileSendFailed(self, RemoteID, PacketID, why):
        ...
        dhnio.Dprint(8, "io_throttle.FileSendFailed %s to [%s] because %s" % (PacketID, fileToSend.remoteID, why))
        transport_control.RemoveSupplierRequestFromSendQueue(fileToSend.packetID, fileToSend.remoteID, commands.Data())
        transport_control.RemoveInterest(fileToSend.remoteID, fileToSend.packetID)
        if why == 'timeout':
...
class IOThrottle:
    ...
    def QueueRequestFile(self, callOnReceived, creatorID, packetID, ownerID, remoteID):
        ...
        if packetID not in [settings.BackupInfoFileName(), settings.BackupInfoFileNameOld()]:
