...
class _Builtin:
    ...
    def read(path):
        ...
        if ext in (".aif", ".aiff"):
            ...
            return datastr
            # return Sample(np.array(data, dtype=float), sr)
        elif ext == ".wav":
    ...
    def get_info(path):
        ...
        if ext in (".aif", ".aiff"):
            ...
            bits = f.getsampwidth()
            encoding = "pcm%d" % bits
            out = SndInfo(f.getframerate(), f.getnframes(), f.getnchannels(), encoding, "aif")
            f.close()
            return out            
        elif ext == ".wav":
...
class WavReader:
    ...
    def info(self):
        if self._info is not None:
            return self._info
        self._info = _wav_get_info(self.path)
        return self._info
    def read(self):
...
def _wav_read_fmt_chunk(f, big_endian):
    ...
    if chunksize > 16:
        f.read(chunksize - 16)
    return chunksize, format, ch, sr, brate, ba, bits
...
def _wav_read_data(fid, size, channels, encoding, big_endian):
    ...
    if bits == 8:
        ...
        if encoding in ('pcm16', 'pcm32', 'pcm64'):
            ...
            data = np.fromfile(fid, dtype=np.ubyte, count=size)
            target = np.zeros((size*4/3,), dtype=np.ubyte)
            target[3::4] = data[2::3]
            target[2::4] = data[1::3]
            target[1::4] = data[0::3]
            del data
