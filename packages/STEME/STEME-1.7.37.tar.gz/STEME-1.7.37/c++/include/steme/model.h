...
public:
	...
	/** Calculate Z for the W-mer at the given position. Can over-ride the prior with optional argument. */
	double
	calculate_Z( global_pos_t global_pos, bool rev_comp, boost::optional< double > g = boost::optional< double >() ) {
	    ...
	    const double lambda = _prior.lambda.get_lambda();
	    const double odds_ratio = std::exp( log_p_background - log_p_binding );
	    const double g_n = g ? *g : _prior._psp->get_g( seq, offset );
	    const double prior_term = ( ( 1. + g_n / ( 1. - g_n ) ) / lambda - 1. );
	    const double Z = 1. / ( 1. + prior_term * odds_ratio );
	    return Z;
	}
