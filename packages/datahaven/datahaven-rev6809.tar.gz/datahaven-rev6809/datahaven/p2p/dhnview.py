...
def wxImage(path, type=wx.BITMAP_TYPE_PNG):
    try:
        return wx.Image(path, type)
    except:
        try:
            return wx.Image(NO_IMAGE, type)
        except:
            pass
    return None
...
class MyHtmlWindow(wx.html.HtmlWindow):
    def __init__(self, parent, enable_logs=False):
...
#            self.SetFonts("Microsoft Sans Serif", "Courier New", _FONT_SIZES)
    ...
    def OpenURL(self, url_, save_scroll_pos=True):
        ...
        def errback(err):
            ...
            src += '<br><br>\n'
            exc = ''
#            if os.path.isfile(sharedPath('exception.log')):
#                f = open(sharedPath('exception.log'), 'r')
#                exc = f.read()
#                f.close()
            if exc:
                src += exc.replace('\n', '<br>\n')
...
class MyTrafficWindow(wx.Frame):
    def __init__(self, parent):
        ...
        self.lst.InsertColumn(0, 'Traffic')
        self.lst.SetColumnWidth(0, 1000)
        self.index = 0

        icon = wx.Icon('icons/dhnappicon.ico', wx.BITMAP_TYPE_ICO)
        self.SetIcon(icon)
...
class MyStatesWindow(wx.Frame):
    def __init__(self, parent):
        ...
        self.lst.InsertColumn(1, 'Name', width=200)
...
#        toolbar.AddSimpleTool(1, wxImage('stock_new.png', wx.BITMAP_TYPE_PNG).ConvertToBitmap(), 'New', '')
#        toolbar.AddSimpleTool(2, wxImage('stock_open.png', wx.BITMAP_TYPE_PNG).ConvertToBitmap(), 'Open', '')
#        toolbar.AddSimpleTool(3, wxImage('stock_save.png', wx.BITMAP_TYPE_PNG).ConvertToBitmap(), 'Save', '')
#        toolbar.AddSeparator()
#        toolbar.AddSimpleTool(4, wxImage('stock_exit.png', wx.BITMAP_TYPE_PNG).ConvertToBitmap(), 'Exit', '')
        toolbar.Realize()
...
class MyFrame(wx.Frame):
    def __init__(self, parent, ID, url):
        ...
        #---tool bar
        #self.toolbar_vbox = wx.BoxSizer(wx.VERTICAL)
        self.toolbar = self.CreateToolBar(wx.TB_RIGHT) # wx.TB_HORIZONTAL
        self.toolbar.SetToolBitmapSize((31,31))
        #self.toolbar = wx.ToolBar(self, -1, style=wx.TB_HORIZONTAL | wx.NO_BORDER)
        self.Bind(wx.EVT_MENU, self.OnLogs, self.toolbar.AddSimpleTool(1, wxImage('icons/log32.png', wx.BITMAP_TYPE_PNG).ConvertToBitmap(), 'Logs', ''))
        self.Bind(wx.EVT_MENU, self.OnTraffic, self.toolbar.AddSimpleTool(2, wxImage('icons/traffic32.png', wx.BITMAP_TYPE_PNG).ConvertToBitmap(), 'Traffic', ''))
        self.Bind(wx.EVT_MENU, self.OnEvents, self.toolbar.AddSimpleTool(3, wxImage('icons/event32.png', wx.BITMAP_TYPE_PNG).ConvertToBitmap(), 'Events', ''))
        self.Bind(wx.EVT_MENU, self.OnStates, self.toolbar.AddSimpleTool(4, wxImage('icons/states32-1.png', wx.BITMAP_TYPE_PNG).ConvertToBitmap(), 'Automats', ''))
        self.toolbar.AddSeparator()
#        self.toolbar.AddSeparator()
#        self.toolbar.AddSeparator()
#        self.toolbar.AddSeparator()
#        self.toolbar.AddSeparator()
        self.Bind(wx.EVT_MENU, self.OnClose, self.toolbar.AddSimpleTool(5, wxImage('icons/exit32-3.png', wx.BITMAP_TYPE_PNG).ConvertToBitmap(), 'Close', ''))
#        toolbar.AddSimpleTool(2, wxImage('stock_open.png', wx.BITMAP_TYPE_PNG).ConvertToBitmap(), 'Open', '')
#        toolbar.AddSimpleTool(3, wxImage('stock_save.png', wx.BITMAP_TYPE_PNG).ConvertToBitmap(), 'Save', '')
#        toolbar.AddSeparator()
#        toolbar.AddSimpleTool(4, wxImage('stock_exit.png', wx.BITMAP_TYPE_PNG).ConvertToBitmap(), 'Exit', '')
        self.toolbar.Realize()
        #wx.Frame.Add(self.toolbar, 0, border=5)
        #self.toolbar_vbox.Add(self.toolbar, 0, border=5)
        #self.SetSizer(self.toolbar_vbox)
    ...
    def OnLogs(self, evt):
        ...
        def eb(x):
            d = wx.MessageDialog(
                self,
                'To enable logs go to [menu]->settings->debug options->enable logs',
                'Logs disabled',
    ...
    def SetStatusBarText(self, text):
        if self.statusTextTimer is not None:
            if not self.statusTextTimer.called:
