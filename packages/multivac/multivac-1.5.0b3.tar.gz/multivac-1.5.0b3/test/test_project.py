...
class TestQuery(TestProject):
    ...
    def test_tag_true(self):
        r = self.session.query(Project).with_transformation(Project.filter_tags(tags={'t2': True})).all()
        assert len(r) == 2
        assert self.project['p1'] in r
        assert self.project['p2'] in r
    
    def test_tag_none(self):
        r = self.session.query(Project).with_transformation(Project.filter_tags(tags={'t4': None})).all()
        assert len(r) == 2
        assert self.project['p1'] in r
        assert self.project['p3'] in r
    
    def test_tag_value(self):
        r = self.session.query(Project).with_transformation(Project.filter_tags(tags={'t5': 'p3t5'})).all()
        assert len(r) == 1
        assert self.project['p3'] in r
    
    def test_tag_filter(self):
        r = self.session.query(Project).filter_by(name='p1').with_transformation(Project.filter_tags(tags={'t4': None})).all()
        assert len(r) == 1
        assert self.project['p1'] in r
