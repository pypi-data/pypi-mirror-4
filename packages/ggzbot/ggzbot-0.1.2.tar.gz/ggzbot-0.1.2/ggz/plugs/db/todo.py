...
from ggz.utils.timeutils import strtotime, striptime, today 
from ggz.utils.locking import lockdec
from ggz.utils.generic import getwho
from ggz.lib.commands import cmnds
from ggz.lib.examples import examples
from ggz.lib.users import getusers
from ggz.lib.datadir import datadir
from ggz.lib.persist import Persist
from ggz.lib.aliases import setalias
    ...
    def delete(self, name, indexnr):
        ...
        try:
            ...
            if warnsec:
                alarmnr = 0 - warnsec
                if alarmnr > 0:
                    alarms = plugs.get("ggz.plugs.common.alarm")
...
examples.add('todo-chan', 't-chan [<item>] .. add channel todo', 'todo-chan fix bla')
...
def handle_settodo(bot, ievent):
    ...
    if not ttime  == None:
        ievent.reply('time detected ' + time.ctime(ttime))
        what = striptime(what)
        karma = plugs.get("ggz.plugs.db.karma2")
...
examples.add('todo-week', 't-week .. todo items for this week', 'todo-week')
...
examples.add('todo-tomorrow', 't-tomorrow .. todo items for tomorrow', 'todo-tomorrow')
...
examples.add('todo-settime', 't-settime [<channel|name>] <itemnr> <timestring> .. set todo time', '1) todo-settime #dunkbots 2 13:00 2) todo-settime owner 3 2-2-2010 3) todo-settime 2 22:00')
