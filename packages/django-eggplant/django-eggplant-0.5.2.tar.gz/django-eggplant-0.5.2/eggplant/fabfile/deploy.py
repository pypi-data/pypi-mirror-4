    ...
    with temporary_directory(prefix='deploy~%s~' % project_name) as tmp_directory:
        ...
        with lcd(tmp_directory):
            ...
            if use_symbolic_link_settings:
                ...
                production_settings_module = deploy_settings.get(
                    'PRODUCTION_SETTINGS', '%s.production_settings' % project_name
                )
...
def deploy_to_remote():
    ...
    local(rsync(
        ...
        (host_string + ':' if is_remote else '') + remote_project_source_root, rsync_schema=rsync_argument))
