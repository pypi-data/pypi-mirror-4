from hooover.input import LogglyInput
from hooover.exceptions import NotFound, AuthFail
from hooover.utils import time_translate
import logging
try:
...
_methods = ("HEAD", "GET", "POST", "PUT", "OPTIONS", "DELETE")
...
class LogglySession(object):
    ...
    def _api_help(self, endpoint, params=None, method='GET'):
        ...
        url = '%s://%s.%s/%s' % (self.protocol, self.subdomain, self.domain, endpoint)
        if method == 'GET':
            body = ''
            if params:
        ...
        auth_header = HTTPBasicAuth(self.username, self.password)
        req = requests.request(method, url, auth=auth_header, data=body)
