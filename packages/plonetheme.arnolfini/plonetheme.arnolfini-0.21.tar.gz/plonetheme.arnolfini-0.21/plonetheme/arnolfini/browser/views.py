...
class CommonBrowserView(BrowserView):
    ...
    def getFormattedEventDate(self, event):
        ...
        dates = "%s"%(samedate and event.start().strftime('%A, %d %B %Y') or "%s to %s"%(event.start().strftime('%A, %d %B %Y'), event.end().strftime('%A, %d %B %Y')))
        
        openingHour = event.start().strftime('%H:%M')
        closingHour = event.end().strftime('%H:%M')
