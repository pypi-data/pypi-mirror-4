...
def _isNonDecreasing(vals):
    ...
    advance_iterator(it)
    op = operator.ge
    return all(itertools.starmap(op, six.moves.zip(it,vals)))
...
def _characterize_function(name,tol,f_rule,model,points,*index):
    ...
    if f_rule.__class__ is dict:
        if len(index) == 1:
            values = f_rule[index[0]]
        else:
            values = f_rule[index]
    elif f_rule.__class__ in [list,tuple]:
        values = f_rule
    else:
    ...
    if not all(itertools.starmap(lambda x1,x2: (True) if ((x1 is None) or (x2 is None)) else (abs(x1-x2) > tol), six.moves.zip(slopes, itertools.islice(slopes, 1, None)))):
...
class _PiecewiseData(_BlockData):
    ...
    def activate(self):
        self.active=True
        [con.activate() for con in itervalues(self.subcomponents(Constraint))]
    
    def deactivate(self):
        [con.deactivate() for con in itervalues(self.active_subcomponents(Constraint))]
        self.active=False
...
class _SimpleSinglePiecewise(object):
    ...
    def construct(self,pblock,x_var,y_var):
        ...
        LHS = y_var
        F_AT_XO = y_pts[0]
        # call float() to protect against integer division
        dF_AT_XO = (y_pts[1]-y_pts[0])/float(x_pts[1]-x_pts[0])
...
class _SimplifiedPiecewise(object):
    ...
    def construct(self,pblock,x_var,y_var):
        ...
        def con_rule(model,i):
            ...
            F_AT_XO = y_pts[i]
            # call float() to protect against integer division
            dF_AT_XO = (y_pts[i+1]-y_pts[i])/float(x_pts[i+1]-x_pts[i])
        ...
        pblock.simplified_piecewise_constraint = Constraint(bounds_constraint_index,rule=con_rule)
...
class _SOS2Piecewise(object):
    ...
    def construct(self,pblock,x_var,y_var):
        ...
        # create vars
        pblock.SOS2_y = Var(sos2_index,within=NonNegativeReals)
        sos2_y = pblock.SOS2_y
        
        # create piecewise constraints
        pblock.SOS2_constraint1 = Constraint( expr=x_var == 
                                              sum(sos2_y[i]*x_pts[i] for i in sos2_index) )
        ...
        pblock.SOS2_constraint2 = Constraint(expr=expr)
        pblock.SOS2_constraint3 = Constraint(expr=sum(sos2_y[j] for j in sos2_index) == 1)
        pblock.SOS2_constraint4 = SOSConstraint(var=sos2_y, sos=2)
...
class _MCPiecewise(object):
    ...
    def construct(self,pblock,x_var,y_var):
        ...
        # create constants
        SLOPE = dict((p,(y_pts[p]-y_pts[p-1])/float(x_pts[p]-x_pts[p-1])) \
                         for p in polytopes)
    ...
    def construct(self,pblock,x_var,y_var):
        ...
        def con1_rule(model,i):
            if bound_type in [Bound.Upper,Bound.Equal]:
                ...
                ((y_pts[i]-y_pts[i-1])/float(x_pts[i]-x_pts[i-1]))*(x_var-x_pts[i-1])\
                <= rhs
            elif bound_type == Bound.Lower:
                rhs = 1.0
...
-unbounded_domain_var=True/False        Default=False
    ...
    def deactivate(self):
        """
        Deactivates all _PiecewiseData components of this class.
        """
        [comp.deactivate() for comp in itervalues(self._data)]
        self.active = False
    
    # GAH: Perhaps we should add 'activate' functionality to Blocks
    def activate(self):
        """
        Activates all _PiecewiseData components of this class.
        """
        [comp.activate() for comp in itervalues(self._data)]
        self.active = True
