...
class Queue(object):
    ...
    def push(self, messages, delay=0):
        ...
        Each object in *messages* must be pickle-able (by cPickle)
        ...
        # FIXME: We should probably upload in 1MB batches.
    
        encoded_messages = [self._encode_message(message) for message in messages]
        return self._low_level_push(encoded_messages, delay)
    ...
    def _low_level_pop(self, max_count, timeout):
        ...
        # split results by boundary and filter out empty strings
        results_data = [result_data for result_data in rest.split(boundary) if result_data]
...
def _low_level_attach(queue, message_handler, expand_iterable_output, output_queues, 
    ...
    # Determine appropriate batch_size
    # Use max batch_size if no parallel jobs
    #  minimize as more parallelism
    if not batch_size:
        batch_size = min(10, max(1, 10-3*max_parallel_jobs+readers_per_job))
    ...
    def attachment_runner(message_queue, result_queue):
        ...
        while True:
            ...
            if not encoded_message: # sentinel
                ...
                result_queue.put(None) # send result queue sentinel
                break
    ...
    def attachment():
        ...
        def push_to_queue():
            ...
            while sentinel_signals < readers_per_job:
                ...
                # extract sentinels
                sentinels = [encoded_message for encoded_message in encoded_message_list if not encoded_message]
                sentinel_signals += len(sentinels)
                
                encoded_message_list = [encoded_message for encoded_message in encoded_message_list if encoded_message]
