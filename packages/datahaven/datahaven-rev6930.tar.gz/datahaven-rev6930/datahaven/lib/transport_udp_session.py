...
def init(parent):
    ...
    # client.datagram_received_callback = data_received
...
def open_session(address, arg=None):
    global _SessionsDict
    if not _SessionsDict.has_key(address):
        idurl = None
        if isinstance(arg, str):
            idurl = arg
        name = 'udp_peer_%s:%s' % (address[0], address[1])
        idurls = identitycache.GetIDURLsByContact('udp://%s:%d' % (address[0], address[1]))
        if len(idurls) > 0:
            if idurl is not None and idurl != idurls[0]:
                dhnio.Dprint(4, 'transport_udp_session.open_session WARNING unknown idurl? %s and %s' % (idurl, str(idurls)) )
            idurl = idurls[0]
            name = 'udp_peer_' + nameurl.GetName(idurl)
...
def outbox_file(address, filename, fast=False):
    global _SessionsDict
...
#    if sess is None:
#        dhnio.Dprint(4, 'transport_udp_session.data_received from unknown address: %s, making a new session' % str(address))
#        A('init', (None, None))
    # dhnio.Dprint(6, '                  [UDP] %d bytes from %s' % (len(datagram), address))
    A(address, 'datagram-received', (datagram, address))
    # if sess is not None:
    #     sess.automat('datagram-received', (datagram, address))
    # else:
        # TODO 
        # check remote IP - if it is the same with our IP we stay in the same sub network
        # need to get local IP and use it  
        # if address[0] == misc.getLocalIdentity().getIP():
        #     pass  
