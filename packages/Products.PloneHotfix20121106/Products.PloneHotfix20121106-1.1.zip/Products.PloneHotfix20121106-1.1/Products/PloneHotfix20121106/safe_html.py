...
safe_html.decode_htmlentities = decode_htmlentities
...
if getattr(safe_html, 'safeToInt', None) is None:
    def safeToInt(value):
        ...
        try:
            ...
            return 0
    safe_html.safeToInt = safeToInt

if getattr(safe_html, 'hasScript', None) is None:
    def unknown_starttag(self, tag, attrs):
        ...
        if tag in self.valid:
            ...
            #UNUSED endTag = '</%s>' % tag
            if safe_html.safeToInt(self.valid.get(tag)):
                self.result.append('>')
            else:
                self.result.append(' />')
    ...
    safe_html.StrippingParser.unknown_starttag = unknown_starttag


if getattr(safe_html, 'CSS_COMMENT', None) is None:
    ...
    safe_html.CSS_COMMENT = CSS_COMMENT
...
safe_html.StrippingParser.entitydefs = html5entities
...
safe_html.StrippingParser.handle_entityref = handle_entityref
...
def convert_entityref(self, name):
    if name + ';' in self.entitydefs:
        ...
        x = ';'
    return '&%s%s' % (name, x)
safe_html.StrippingParser.convert_entityref = convert_entityref
