...
class KeyReuseError(Exception):
    """Notify abouat an existing key"""


class UnknownKeyError(Exception):
    """Notify about an unknown key"""


class LeakedCallbackError(Exception):
    """Notify about a leaked callback"""


class BadYieldError(Exception):
    """Notify about a malformed yield statement"""
...
class WaitAll(YieldPoint):
    ...
    """
    # pylint: disable-msg=W0231,W0201
            ...
            while True:
                if self.exc_info is None:
                    try:
                        if not self.yield_point.is_ready():
                            return
                        next_run = self.yield_point.get_result()
                    except Exception:
                ...
                try:
                    if self.exc_info is not None:
                        ...
                        yielded = self.gen.send(next_run)
                ...
                if isinstance(yielded, YieldPoint):
                    ...
                    msg = "yielded unknown object %r" % yielded
                    self.exc_info = (BadYieldError(msg),)
        finally:
            self.running = False

    def result_callback(self, key):
        """A wrapper around result"""
        # pylint: disable-msg=C0111
