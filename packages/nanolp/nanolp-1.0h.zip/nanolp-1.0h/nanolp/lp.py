...
class UseCmd(Cmd):
    ...
    def onpost(self, parser, flush):
        ...
        inclparser = Parser.fileparser(self.inclfilename, engine=parser.engine)
...
'''
    ...
    def _footer(self):
        yield '<div id="about">'
        yield '<a href="http://code.google.com/p/nano-lp">%s</a> - ' % __ABOUT__
        yield '<a href="http://balkansoft.blogspot.com">BalkanSoft@BlogSpot</a>'
        yield '</div></body></html>'
    ...
    def _output_info(self):
        ...
        if self.parser.engine:
            ...
            fn = os.path.split(cfgfile)[1]
            yield '<a href="file://%s" title="%s">%s</a>'%(cfgfile, cfgfile, fn)
            yield '</td>'
            yield '</tr>'
        # Index of commands
    ...
    def _body(self):
        ...
        yield '<h1><span class="dlarrow"><a href="file://%s">&dArr;</a></span>%s</h1>' % \
                (self.parser.infilename, self.title)
...
class Parser:
    ...
    @staticmethod
    def fileparser(filename, engine):
...
class Txt2TagsParser(Parser):
    ...
    def tokens(self, text):
        ...
        tokens.append(EndToken(None))
        return tokens
Parser.parsers.append(Txt2TagsParser)
...
class AsciiDocParser(Parser):
    ...
    def tokens(self, text):
        ...
        tokens.append(EndToken(None))
        return tokens
Parser.parsers.append(AsciiDocParser)
