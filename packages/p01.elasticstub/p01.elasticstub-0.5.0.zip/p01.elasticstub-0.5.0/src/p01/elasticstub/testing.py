...
def startElasticSearchServer(sandBoxDir=None, dataDir=None, logDir=None,
    ...
    mappingSourceCopyTreeIgnorePatterns='*.svn', downloadURL=None,
    sleepCopy=3):
    ...
    if not 'lib' in names and not 'bin' in names:
        if downloadURL is None:
            # windows
            git = 'https://github.com/downloads/elasticsearch/elasticsearch'
            if os.name == 'nt':
                url = '%s/elasticsearch-0.19.9.zip' % git
            # non windows
            else:
                url = '%s/elasticsearch-0.19.9.tar.gz' % git
        else:
            url = downloadURL
    ...
    ES_CLASSPATH = "%s%s*%s%s%s*%s%s" % (libPath, os.sep, sep, sigarPath,
        os.sep, sep, CLASSPATH)
