...
def pyzo_dirs(newdir=None, makelast=False):
    ...
    Get the locations of installed Pyzo directories. In most cases the
    returned list will consists of zero or one directory names. If
    newdir is given, ensure that it is in the list. if makelast is True,
    ensure that newdir is the *last* item in the list, i.e. the active
    one.
    """
    # Get application dir
    userDir = os.path.expanduser('~')
    ...
    if not os.path.isdir(path):
        os.mkdir(path)
    # Open file and parse
    fname = os.path.join(path, 'pyzodir')
    dirs, ndirs = [], 0
    if os.path.isfile(fname):
        dirs = open(fname, 'rb').read().decode('utf-8').split('\n')
    ...
    if newdir and os.path.isdir(newdir):
        ...
        if newdir not in dirs:
            dirs.append(newdir)
    # Check validity of all dirs, write back if necessary, and return
    pythonname = 'python' + '.exe' * sys.platform.startswith('win')
    dirs = [d for d in dirs if os.path.isfile(os.path.join(d, pythonname))]
    if len(dirs) != ndirs:
