...
(function ($) {
    ...
    function drawSeriesErrors(plot, ctx, multibarIdx, numBars, s){
        ...
        for (var i = 0; i < s.datapoints.points.length; i += ps) {
		    ...
		    var upper = [x, y][e] + errRanges[e * err.length + 1], 
                    ...
                    // multibar calculations
                    mid = ax[0].p2c(1);
                    barWidth = mid * 2;
                    multiBarMidWidth = barWidth / numBars / 2;
                    multiBarOffset = (multibarIdx/numBars * barWidth) - mid + multiBarMidWidth;

                    // convert to pixels
                    x = ax[0].p2c(x) + multiBarOffset;
                    y = ax[1].p2c(y);
    ...
    function draw(plot, ctx){
        ...
        numBars = plot.getData().length;
        $.each(plot.getData(), function (i, s) {
            if (s.points.errorbars && (s.points.xerr.show || s.points.yerr.show))
                drawSeriesErrors(plot, ctx, i, numBars, s);
        });
        ctx.restore();
    }
