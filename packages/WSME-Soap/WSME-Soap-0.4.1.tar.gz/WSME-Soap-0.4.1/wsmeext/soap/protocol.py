...
def date_fromsoap(datatype, el, ns):
    if el.get(nil_qn) == 'true':
        return None
    if el.get(type_qn) not in (None, 'xsd:date'):
        raise exc.InvalidInput(el.tag, et.tostring(el))
...
def time_fromsoap(datatype, el, ns):
    if el.get(nil_qn) == 'true':
        return None
    if el.get(type_qn) not in (None, 'xsd:time'):
        raise exc.InvalidInput(el.tag, et.tostring(el))
...
def datetime_fromsoap(datatype, el, ns):
    if el.get(nil_qn) == 'true':
        return None
    if el.get(type_qn) not in (None, 'xsd:dateTime'):
        raise exc.InvalidInput(el.tag, et.tostring(el))
...
def binary_fromsoap(datatype, el, ns):
    if el.get(nil_qn) == 'true':
        return None
    if el.get(type_qn) not in (None, 'xsd:base64Binary'):
        raise exc.InvalidInput(el.tag, et.tostring(el))
