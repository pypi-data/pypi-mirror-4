...
class restore(Automat):
    ...
    def __init__(self, BackupID, OutputFile): # OutputFileName 
        ...
        Automat.__init__(self, 'restore', 'AT_STARTUP', 4)
        self.automat('init')
    ...
    def doRequestPackets(self, arg):
        ...
        for SupplierNumber in range(self.EccMap.paritysegments):
            SupplierID = contacts.getSupplierID(SupplierNumber) 
            if not self.OnHandParity[SupplierNumber] and contact_status.isOnline(contacts.getSupplierID(SupplierNumber)):
