...
def _generate_ampl_repn(exp):
    ...
    if exp.is_expression():
        ...
        #
            ...
            ###### GAH: Deactivating this preprocessing step, which 
            ######      also eliminates the need for expression cloning
            # preprocess out the denominator
            #exp._denominator = []
            #exp._coef *= 1.0/denom
            # Make sure to reset the denom to 1.0, this is important
            # when the outer expression is linear so that
            # we don't propigate this effect into the ampl
            # repn more than once.
            #denom = 1.0
            ######
...
def generate_ampl_repn(exp):
    ...
    if (degree is None) or (degree > 1):
        ...
        if using_py3 is False:
            ...
            repn._linear_terms_coef = tuple(val for val in itervalues(coef) if val)
            repn._linear_terms_coef = tuple((varmap[var_hash] for var_hash,val in iteritems(coef) if val))
    return repn
