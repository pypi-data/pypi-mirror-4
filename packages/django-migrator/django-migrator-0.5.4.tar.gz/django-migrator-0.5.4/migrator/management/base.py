...
class MigrationCommand(BaseCommand):
    ...
    def _check_previous_migrations_question(self):
        return self.ask_yesno_question(
            'There is half migrated objects in this database, do you want to continue?', 'yes')
    ...
    def _check_previous_migrations(self):
        ...
        if not_finished_migrations:
            ...
            self.before_delete_not_finished_migrations(not_finished_migrations)
    ...
    def migrate_model(self, source_name, model_name, limit, source_fields, migration_fields, migration_step):
        ...
        source_fields += [d['source'] for d in migration_fields if d['source'] not in source_fields and not d.get('fake_field', False)]
        ...
        for i, source_data in enumerate(source_results):
            model = self.get_model(model_name, source_data)
    ...
    def get_model(self, model_name, source_data):
        """
        Return Django model from a string like "app_name.model".
        It can use source_data to have different model based on source data
        """
        ...
        for field_mapping in migration_fields:
            try:
                ...
                if setter_name:
                    ...
                    setter_func(obj, source_field, value)
                elif field_name:
                    self.setter(obj, source_field, field_name, value)
                else:
    ...
    def getter(self, source_field, source_data):
        """
        Get an attribute from source data
        """
        return source_data[source_field]

    def setter(self, obj, source_field, field_name, value):
        """
    ...
    def before_delete_not_finished_migrations(self, not_finished_migrations):
        """
