...
def event_design(event_spec, t, order=2, hrfs=[glover]):
    ...
    if len(e_factors) > 1:
        for i in range(1, order+1):
            for comb in combinations(list(zip(fields, e_factors)), i):
    ...
    for l, h in enumerate(hrfs):
        ...
        for n, c in list(e_contrasts.items()):
...
def stack2designs(old_X, new_X, old_contrasts={}, new_contrasts={}):
    ...
    if set(old_contrasts.keys()).intersection(list(new_contrasts.keys())) != set([]):
        raise ValueError('old and new contrasts must have different names')

    for n, c in list(old_contrasts.items()):
        if c.ndim > 1:
    ...
    for n, c in list(new_contrasts.items()):
        if c.ndim > 1:
            cm = np.zeros((c.shape[0], X.shape[1]))
            cm[:,old_X.shape[1]:] = c
        else:
...
def stack_contrasts(contrasts, name, keys):
    ...
    None
    """
    if name in list(contrasts.keys()):
