...
logquery = getLogger('Exist Database Adaptor - query')
logdocs  = getLogger('Exist Database Adaptor - docs')
logserver = getLogger('Exist Database Adaptor - server')
logparams = getLogger('Exist Database Adaptor - parameters')

## Zope method for management
manage_addeXistDAForm = HTMLFile('dtml/eXistDAAdd', globals())
def manage_addeXistDA(self, id, title='', server_version='', server='', port='', username='', password='', encoding=utils.DEFAULT_ENCODING, xmlrpc_path=utils.DEFAULT_XMLRPC_PATH, REQUEST=None):
    """Add an eXist connector object."""
    # Avoid empty encoding
    if encoding.strip() == '':
        encoding = utils.DEFAULT_ENCODING

    msg = utils.getInvalidMsg(server_version, server, port, username, password, encoding)
    if msg is None :
        self._setObject(id.strip(), eXistDA(id, title, server_version, server, port, username, password, encoding, xmlrpc_path))
...
class eXistDA (OFS.SimpleItem.Item, Persistent, Acquisition.Implicit, AccessControl.Role.RoleManager):
    ...
    def __init__(self, id, title, server_version='', server='', port=utils.DEFAULT_PORT, username='', password='', encoding=utils.DEFAULT_ENCODING, xmlrpc_path=utils.DEFAULT_XMLRPC_PATH):
        """Class constructor.
...
* 'server_version' : string containing the server version. Also used to determinate the XQuery version.
...
"""
        ...
        self.server_version = str(server_version).strip()
...
"""
    ...
    security.declarePublic('getExistVersion')
    def getExistVersion(self):
        """ Return the eXist version

        Returns '1.4' if the server version is unset (Backwaed compatibility)"""
        if hasattr(self, 'server_version'):
            return self.server_version
        else:
            return '1.4'

    security.declarePublic('getXQueryVersion')
    def getXQueryVersion(self):
        """ Return the XQuery version.

        Return '3.0' for all 2.x eXist versions
        Return '1.0' for all 1.x versions
        """
        return self.xquery_version
...
* Returns the first address if found, or an empty string if nothing is found."""
        ...
        xmlrpc_paths = ['/exist/xmlrpc', '/xmlrpc']
        if self.xmlrpc_path:
            xmlrpc_paths.insert(0, self.xmlrpc_path)

        results = []
        for path in xmlrpc_paths:
            try:
                self._getConnection(path).sync()
                results.append(path)
            except:
                pass
...
* 'kws' : dict for any other parameters. See 'README.txt' for recognized parameters
  ...
  * 'variables' : key to a dict containing variables to pass to the xquery"""
        ...
        if utils.DEBUG >= 1:
            logquery.info("Got a resultsSet for %s" % self.server)
            if utils.DEBUG >= 3:
                logquery.info("XML result is :\n%s" % str(resSet.getMergedXML()))

        xmlrpc_server.releaseQueryResult(result)
        if utils.DEBUG >= 3:
                logquery.info("Returning resultsSet to caller")
        return resSet
...
- 'rsc_name' : string containing the name of a document or collection that must be deleted, with the **FULL** path"""
    ...
    def manage_edit(self, title="", server_version='', server="", port=utils.DEFAULT_PORT, username="", password="", encoding=utils.DEFAULT_ENCODING, xmlrpc_path=utils.DEFAULT_XMLRPC_PATH, REQUEST=None):
        ...
        if msg is None:
            ...
            major = self.server_version.split('.')[0]
            if int(major) > 1:
                setattr(self, 'xquery_version', '3.0')
            else:
                setattr(self, 'xquery_version', '1.0')
    ...
    def fastquery(self, query, start, nbmax, **kws):
        ...
        * 'variables' : key to a dict containing variables to pass to the xquery"""
        ...
        if utils.DEBUG >= 1:
            if self.username.strip() != '' and self.password.strip() != '':
                logserver.info("http://" + self.username + ':' + self.password +'@' + self.server + ":" + self.port + self.xmlrpc_path)
            else:
                logserver.info("http://" + self.server + ":" + self.port + self.xmlrpc_path)
            logquery.info("Query is :\n%s" % query)
        ...
        if kws.has_key(utils.KEY_VARIABLES):
            if utils.DEBUG >=3:
                logparams.info(repr({utils.KEY_VARIABLES:kws[utils.KEY_VARIABLES], utils.OK_NAMESPACES:ns}))
            result = xmlrpc_server.query(Binary(query), nbmax, start, {utils.KEY_VARIABLES:kws[utils.KEY_VARIABLES], utils.OK_NAMESPACES:ns})
        else:
            if utils.DEBUG >=3:
                logparams.info(repr({utils.OK_NAMESPACES:ns}))
