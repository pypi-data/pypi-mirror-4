...
def main():
    root = 'yyy'
    splitoutput.log_stdout(
        root + '-chained' if CHAINED else root
        )
    ...
    for nterm in ntermlist:
        ...
        if CHAINED:
            ...
            if False:
                for tag in fitter.chained_fits:
                    print("------------ " + tag)
                    print(fitter.chained_fits[tag])
        if not CHAINED or CHAINED == 'both':
            if CHAINED == 'both':
                p0 = fit.pmean
            fit = fitter.lsqfit(prior=prior, data=data, p0=p0, maxit=10000)
        p0 = fit.pmean
...
def make_models():
    ...
    # return models[:] 
    return [models[5], models[2], models[0]]
    # return [models[0], models[2], models[5]]
##
