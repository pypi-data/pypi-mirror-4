...
def totalQueryset(queryset):
	...
	for i in queryset_total:
		for k,v in i.items():
			...
			if not isinstance(v,NoneType) and not isinstance(v,datetime) and not isinstance(v,str) and not isinstance(v,bool) and not isinstance(v,unicode):
	...
	if request_data['val1'] !='NULL' or request_data['val2'] !='NULL':
		...
		if len(detail_filters)==2: # two fields
			...
			if more_date == 'Month Field':
				new_from_date = datetime(int(year),int(month)-1,int(day),int(hour),int(minute),00)
			elif more_date == 'Week Field':
				new_from_date = datetime(int(year),int(month),int(day)-int(v)*7,int(hour),int(minute),00)
			else:
				new_from_date = datetime(int(year),int(month),int(day)-1,int(hour),int(minute),00)	
