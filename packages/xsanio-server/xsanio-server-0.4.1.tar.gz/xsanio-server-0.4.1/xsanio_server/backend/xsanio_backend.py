...
def client_ping_response(client):
    ...
    """
    ping_response = pyping.ping(client.ip_address, count=1)
    return ping_response.ret_code
...
def get_stats_from_client(client):
    ...
    if ping_resp == 0:
        ...
        LOGGER.error('Client %s responded to ping with %s', client.host_name, ping_resp)
        client.save()
