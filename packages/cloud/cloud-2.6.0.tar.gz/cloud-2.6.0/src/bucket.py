"""
For managing bucket objects on PiCloud's S3 store.
...
class CloudFile(object):
    """A CloudFile represents an object stored on PiCloud as a readonly file-like stream.
...
def put(source, key_name=None):
    """Transfers the bucket object specified by ``file_path`` to PiCloud. The bucket object
    can be retrieved later using the get function.    
    
    If ``key_name`` is specified, the key (name) of the bucket will be set to **key_name** 
    ...
    This will transfer the bucket object from the local path 'data/names.txt'
    to PiCloud and store it as 'names.txt'.
    It can be later retrieved via cloud.bucket.get('names.txt')"""
    
    if key_name is None:
        key_name = os.path.basename(source)
    elif not key_name:
        raise ValueError('Cannot upload bucket object with key_name "%s"' % key_name)
...
def putf(f, key_name):
    ...
    if '../..' in key_name:
        raise ValueError('"../.." cannot be in key_name')
    
    if key_name[-1] == '/':
        raise ValueError('Cannot end a key_name with a forward slash')
    
    if key_name[0] == '/':
        raise ValueError('Cannot begin a key_name with a forward slash')
    
    if '//' in key_name:
        raise ValueError('Cannot have consecutive forward slashes in key_name')
    ...
    try:
        cloudLog.debug('bucket object key_name in client: %s' % key_name)
        # get a file ticket
        resp = conn.send_request(_bucket_new_query, {'name': key_name})
        ticket = resp['ticket']
        params = resp['params']
...
def sync_to_cloud(source, key_name=None):
    """Upload bucket object if it has changed.
    
    cloud.bucket.put(source,key_name) 
    only if contents of local file (specified by *source*)
    differ from those on PiCloud (specified by *key_name* or basename(*source*))
    (or if bucket object does not exist on PiCloud)"""
    if not key_name:
        key_name = os.path.basename(source)
    ...
    try:
        remote_md5 = _get_md5(key_name, log_missing_file_error = False)
...
def list(folderize = False, prefix=None, marker=None, max_keys=1000):
    """List all bucket objects stored on PiCloud. Returns keys in lexicographic order
    
    folderize: Treat listing as directory based; compact keys containing "/" into a single folder
    prefix: Return only keys beginning with prefix.  If folderize=True, list 'folders' by setting prefix to foldername/      
    marker: Return only keys with where key > marker
    max_keys: Maximimum number of keys that can be returned
    """
...
def _file_info(key_name):
    """
    get information about key_name
    """
    conn = _getcloudnetconnection()
    
    resp = conn.send_request(_bucket_exists_query, {'name':key_name})
    return resp

def exists(key_name):
    """Check if key ``key_name`` is stored on PiCloud."""
    conn = _getcloudnetconnection()
...
def _get_md5(key_name, log_missing_file_error = True):
    conn = _getcloudnetconnection()
    resp = conn.send_request(_bucket_md5_query, {'name': key_name},
                             log_cloud_excp = log_missing_file_error)
    md5sum = resp['md5sum']
    return md5sum

def get_md5(key_name):
    """Return the md5 checksum of the object referenced by *key_name*"""
    return _get_md5(key_name)
    
def remove(key_name):
    """Removes the bucket object referenced by ``key_name`` from PiCloud."""
    conn = _getcloudnetconnection()

    resp = conn.send_request(_bucket_remove_query, {'name': key_name})
    removed = resp['removed']
    return removed

def make_public(key_name):
    """Makes the bucket object referenced by ``key_name`` publicly accessible by a URL"""
    conn = _getcloudnetconnection()
    
    resp = conn.send_request(_bucket_make_public_query, {'name': key_name})
    public_url = S3_URL+resp['url']
    return public_url

def is_public(key_name):
    """Checks if the bucket object referenced by ``key_name`` is publicly accessible by a URL"""
    conn = _getcloudnetconnection()
    
    resp = conn.send_request(_bucket_is_public_query, {'name': key_name})
    if resp['status']:
        public_url = S3_URL+resp['url']
        return public_url
    else:
...
def make_private(key_name):
    """Removes the publically accessible URL (if any) associated with the bucket object referenced by ``key_name``"""
    conn = _getcloudnetconnection()
    
    resp = conn.send_request(_bucket_make_private_query, {'name': key_name})
    
def get(key_name, destination=None, start_byte=0, end_byte=None):
    """
    Retrieves the bucket object referenced by ``key_name`` from PiCloud and stores it to ``destination``.
        
    Example::    
    
        cloud.bucket.get('names.txt','data/names.txt') 
    
    This will retrieve the 'names.txt' bucket object on PiCloud and save it locally to 
    'data/names.txt'. 
    ...
    cloud_file = getf(key_name, start_byte, end_byte)
...
def sync_from_cloud(key_name, destination=None):
    """Download bucket object if it has changed.
    
    cloud.bucket.get(key_name,destination) 
    only if contents of local file (specified by *destination* or basename(key_name))
    differ from the object on PiCloud referenced by *key_name*
    ...
    if not os.path.exists(destination):
        ...
        cloudLog.debug('remote_md5=%s. local_md5=%s. downloading? %s',
                       remote_md5, local_md5, do_update)
    if do_update:
        get(key_name, destination)

def getf(key_name, start_byte=0, end_byte=None):
    """
    Retrieves the object referenced by ``key_name`` from PiCloud.
    ...
    resp = conn.send_request(_bucket_get_query, {'name': key_name})
...
#    
#    resp = conn.send_request(_bucketmap_job_query, params)
#    
#    return resp['jids']
