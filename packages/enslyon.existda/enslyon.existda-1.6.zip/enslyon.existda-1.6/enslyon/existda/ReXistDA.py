...
def manage_addReXistDA(self, id, title='', server='', port='', username='', password='', encoding=utils.DEFAULT_ENCODING, xmlrpc_path=utils.DEFAULT_ZOPE_XMLRPC_PATH, REQUEST=None):
    ...
    if msg is None :
        self._setObject(id.strip(), ReXistDA(id, title, server_version, server, port, username, password, encoding, xmlrpc_path))
...
class ReXistDA (eXistDA.eXistDA):
    ...
    def __init__(self, id, title, server_version='', server='', port=utils.DEFAULT_PORT, username='', password='', encoding=utils.DEFAULT_ENCODING, xmlrpc_path=utils.DEFAULT_ZOPE_XMLRPC_PATH):
        """Class constructor.
