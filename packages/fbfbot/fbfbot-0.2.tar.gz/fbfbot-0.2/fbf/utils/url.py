...
def useragent():
    ...
    return 'Mozilla/5.0 (X11; Linux x86_64); %s %s; http:///docs/fbfbot)' % (name, version)
...
class Url(LazyDict):
    ...
    def fetch(self, *args, **kwargs):
        self.html = geturl2(self.url, html=True)
...
def geturl2(url, decode=False, timeout=5, bytes=False, html=False):
    ...
    res = result.read()
    if not bytes: res = fromenc(res, "utf-8")
    if not bytes and html: res = res.replace("//", "/")
    if not bytes: res = istr(res)
    else: res = ibytes(res)
    info = result.info()
    result.close()
