...
import itertools
import collections
from _chunk import Chunk, StreamItem, StreamItem_v0_1_0, serialize
        ...
        for si in Chunk(path=fpath, mode='rb', message=message_class):
            if si.stream_id == stream_id:
                if si.body and si.body.raw:
                    print 'StreamItem.source:'
                    print si.source
                    print 'StreamItem.body.raw:'
                    print si.body.raw
                    print 'StreamItem.body.clean_html:'
                    print si.body.clean_html
                    print 'StreamItem.body.clean_visible:'
                    print si.body.clean_visible
                    for mdata_name, mdata in si.source_metadata.items():
                        print 'StreamItem.source_metadata[%r]:' % mdata_name
                        print mdata
                    _path = '/tmp/%s.sc' % si.stream_id
                    open(_path, 'wb').write(serialize(si))
                    print 'wrote single-item chunk to: %s' % _path
                    sys.exit()
                elif si.body:
