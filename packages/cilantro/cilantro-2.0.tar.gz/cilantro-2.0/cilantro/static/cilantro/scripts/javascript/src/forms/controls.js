...
define(['environ', 'mediator', 'jquery', 'underscore', 'backbone', 'serrano/channels'], function(environ, mediator, $, _, Backbone, channels) {
  var BooleanControl, Control, DEFAULT_EVENTS, EnumerableControl, NEGATION_OPERATORS, NumberControl, SearchableControl, formActionsTemplate, placeholderText;
    ...
    Control.prototype.submit = function(data) {
      ...
      if (data.value === void 0) {
        if (this.node) {
          this.setOperator();
          this.setValue();
          mediator.publish(channels.DATACONTEXT_REMOVE, this.node);
          delete this.node;
          this.$remove.hide();
        }
        return;
      ...
      if (this.node) {
        ...
        this.node = new App.DataContextNode(data);
      }
      mediator.publish(channels.DATACONTEXT_ADD, this.node);
      return this.$remove.show();
    };
  ...
  placeholderText = function(values, parse) {
    var examples, i, idx, indexes, len, value, _i, _ref;
    ...
    examples = [];
    indexes = [];
    for (i = _i = 0, _ref = Math.min(3, len); 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
  ...
  BooleanControl = (function(_super) {
    ...
    function BooleanControl() {
      return BooleanControl.__super__.constructor.apply(this, arguments);
    }
