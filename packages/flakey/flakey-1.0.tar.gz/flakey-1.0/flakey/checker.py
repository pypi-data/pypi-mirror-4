...
try:
    ...
    # python 2 has no such module
    builtins = __builtins__

import os.path
import ast

from flakey import messages
...
def getNodeName(node):
    ...
    if hasattr(node, 'id'): # One of the many nodes with an id
        return node.id
    elif hasattr(node, 'name'): # a ExceptHandler node
        return node.name
    else:
        return None
...
class Checker:
    ...
    def handleNodeLoad(self, node):
        ...
        # In python2, builtins (__builtins__) is a dictionary, this wraps it
        # so if the obj is a dict, this will return the correct result
        ...
        try:
            ...
            if ((not _hasattr(builtins, name)) and (name not in _MAGIC_GLOBALS) and not importStarred):
            ...
            def checkUnusedAssignments():
                ...
                for name, binding in self.scope.items():
                    if (not binding.used and not name in self.scope.globals
                        ...
                        n = name.id if hasattr(name, 'id') else name
                        self.report(messages.UnusedVariable,
                                    binding.source.lineno, n)
