...
##                Dprint(2, 'dhnio.InstallLocale encoding is: ' + encoding)
...
def Dprint(level, s, new_line='\n'):
    ...
    if WebStreamFunc is not None:
        ...
        WebStreamFunc(level, s_ + new_line)
    LogLinesCounter += 1
    if LogLinesCounter % 1000 == 0:
...
def find_process(applist):
    ...
    for appname in applist:
        pidsL.extend(find_process_linux(appname))
    return pidsL
...
def find_process_linux(appname):
    pidsL = []
    for pid, cmdline in list_processes_linux():
        if cmdline.count(appname):
            try:
                pid = int(pid)
            except:
                continue
