...
#        file_handler.handleError = handleError
    ...
    def _set_min_and_max_number_sites(self):
        ...
        if not self.options.max_num_sites:            
            # allow number of sites up to 3 * # sequences / 2, with a hard limit if not user-specified
            hard_limit = 10000
            ...
            # however if user has set minimum number of sites always have at least 50% more
            if self.options.min_num_sites:
                ...
                if self.options.max_num_sites < twice:
                    ...
                    logger.info('Maximum number of sites not explicitly set and is less than twice minimum. Increasing to %d', self.options.max_num_sites)
        ...
        if not self.options.min_num_sites:
            # Look for at least one site in every 10th sequence
            # but always look for at least 2 sites unless max_num_sites is small
