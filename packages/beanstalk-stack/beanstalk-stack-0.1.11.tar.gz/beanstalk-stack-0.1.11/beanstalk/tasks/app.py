...
def deploy_remote_core():
    ...
    run('bsjack server.create_app:%s,VERBOSE=%d' % (project_name, beanstalk_settings['VERBOSE']))

    # Call pre-install script
    run_hooked_actions('PRE_INSTALL_ACTIONS')

    # Get app info first
    app_info = json.loads(run('bsjack server.app_info:%s,VERBOSE=%d,USE_JSON=True' %
    ...
    run('bsjack server.update_app:%s,VERBOSE=%d' % (project_name, beanstalk_settings['VERBOSE']))
