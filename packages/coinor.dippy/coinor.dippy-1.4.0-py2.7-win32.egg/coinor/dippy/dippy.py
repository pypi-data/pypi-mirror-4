...
def Solve(prob, params=None):
    ...
    if prob.post_process_node == None:
        params['pyPostProcessNode'] = '0'
    if prob.post_process_branch == None:
...
class DipProblem(pulp.LpProblem, DipAPI):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.display_mode != 'off':
            if gimpy_installed == False:
                ...
                self.Tree = BBTree()
                self.Tree.set_display_mode('pygame')
    ...
    def postProcessNode(self, node):
        ...
        if parentInd == -1:
            ...
            if etree_installed and self.Tree.display_mode == 'svg':
                self.Tree.write_as_svg(filename = "node%d" % numNodes, 
                                       nextfile = "node%d" % (numNodes + 1), 
                                       highlight = nodeInd)
        else:
            if branch_direction == 'R':
                ...
                self.Tree.set_node_attr(n, 'label', label)
                self.Tree.set_node_attr(n, 'obj', nodeQuality)
                self.Tree.set_node_attr(n, 'color', color)
                self.Tree.set_node_attr(n, 'fillcolor', color)
                self.Tree.set_node_attr(n, 'status', nodeStatus)
                if etree_installed and self.Tree.display_mode == 'svg':
                    self.Tree.write_as_svg(filename = "node%d" % numNodes, 
                                           prevfile = "node%d" % (numNodes-1), 
                                           nextfile = "node%d" % (numNodes+1), 
                                           highlight = nodeInd)
 
        self.Tree.display()

        if self.post_process_node:
        ...
        for n in outputDict:
            if n == 'pDownUB':
                self.Tree.add_left_child(str(numNodes), str(nodeInd), 
                                         label = 'C', 
                                         branch_var = str(outputDict[n][0]), 
                                         status = 'C', 
                                         obj = nodeQuality, color = 'yellow', 
                                         style = 'filled', 
                                         fillcolor = 'yellow')
                self.Tree.set_edge_attr(str(nodeInd), str(numNodes), 'label', 
                                str(outputDict[n][0]) + '<=' + 
                                str(outputDict[n][1]))
                numNodes += 1
