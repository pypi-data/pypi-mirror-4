...
from storm.expr import And, Column, Select, Table
...
from .configuration import (
    ...
    load_configuration,
    load_configuration_with_command_line,
    )
...
def iter_published_binaries(lp, since=None, name=None, exact_match=True):
    architectures = load_configuration().architectures_supported
...
class PackageDatabase(object):
    ...
    def _get_query(self, library_names, arch):
        return Select(
            [Column('library'), Column('dependency')],
            And(Column('architecture') == arch,
                Column('library').is_in(map(unicode, library_names))),
            Table('libdep'))

    def get_multiple_dependencies(self, library_names, arch):
        """Get the binary packages that provide libraries.

        :return: (deps, missing), where ``deps`` is a dict mapping library
            names to sets of packages that provide them, and ``missing`` is a
            set of library names for which no dependencies could be found.
        """
        arch = unicode(arch)
        result = self._store.execute(self._get_query(library_names, arch))
        found = {}
        for row in result:
            [lib, dependency] = row
            if lib in found:
                found[lib].add(dependency)
            else:
                found[lib] = set([dependency])
        return found
...
class LibdepServiceClient(object):
    """Implements the read part of PackageDatabase's interface."""

    DB_TYPE = 'libdep-service'
    ...
    def get_multiple_dependencies(self, library_names, arch):
        ...
        :return: (deps, missing), where ``deps`` is a dict mapping library
            ...
            set of library names for which no dependencies could be found.
        """
        deps = self._client.get_binaries_for_libraries(library_names, [arch])
        return dict((lib, set(dep)) for (lib, dep) in deps[arch].items())
...
def get_dependency_database():
    ...
    # TODO: Change this to return LibdepServiceClient sometimes
    databases = {
        AptFilePackageDatabase.DB_TYPE: AptFilePackageDatabase.from_options,
        PackageDatabase.POSTGRES: PackageDatabase.from_options,
        PackageDatabase.SQLITE: PackageDatabase.from_options,
        LibdepServiceClient.DB_TYPE: LibdepServiceClient.from_options,
        }
    options = load_configuration()
    return databases[options.database_db_type](options)
...
def main():
    ...
    glue = load_configuration_with_command_line()
