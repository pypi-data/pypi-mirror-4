...
from projexui.qt.QtCore import Qt, QMimeData, QByteArray
...
from projexui.xthread                               import XThread
...
class XOrbTreeWorkerThread(XThread):
    def __init__( self, parent ):
        super(XOrbTreeWorkerThread, self).__init__(parent)
        
        self._recordSet = None
    
    def recordSet( self ):
        """
        Returns the record set linked with this thread.
    ...
    def run( self ):
        ...
        """
        if ( self._recordSet.groupBy() ):
            self._recordSet.grouped()
        else:
            self._recordSet.all()
    
    def setRecordSet( self, recordSet ):
        """
        Sets the record set instance for this thread.
...
class XOrbTreeWidget(XTreeWidget):
    ...
    def __init__( self, parent = None ):
        ...
        self._workerThread      = XOrbTreeWorkerThread(self)
        ...
        self._workerThread.finished.connect( self.__loadRecords )
    
    def __loadRecords( self ):
        """
        Loads the records from the thread.
        """
        ...
        # load groups
        if ( rset.groupBy() ):
            grouping = rset.grouped()
            index = 0
            for key, records in grouping.items():
                self.createGroupItem(self, key, records)
                
                index += 1
                if ( not index % 10 and self._loaderVisible ):
                    QApplication.processEvents()
        
        # load all records
        else:
            index = 0
            for record in rset.all():
                self.createRecordItem(self, record)
    ...
    def createRecordItem( self, parent, record ):
        """
    ...
    def createGroupItem( self, parent, groupName, records ):
        """
    ...
    def refresh( self, reloadData = False ):
        ...
        if ( currset and len(currset) > 100 ):
            self._loaderVisible = True
            XLoaderWidget.start(self)
        else:
            self._loaderVisible = False
        
        self._workerThread.setRecordSet(currset)
        self._workerThread.setLocalThreadingEnabled(currset.isThreadEnabled())
    ...
    def searchRecords( self, search ):
        ...
        self._pageCount = None
        self._currentRecordSet = self.recordSet().search(search)
    ...
    def workerThread( self ):
        """
        Returns the worker thread for this widget.
        
        :return     <XOrbTreeWorkerThread>
        """
