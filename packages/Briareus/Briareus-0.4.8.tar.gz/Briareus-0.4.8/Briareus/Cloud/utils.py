...
class Cloud(object):
    ...
    def __call__(self, *args):
        return Client(config.host, "Briareus", pickler=Husky, serialize=True, interval=0.1).eval(self.f, args)
...
class CachedData(object):
    ...
    def put(self, value):
        ...
        KVStore(config.host, self.name).set(self.id, value, serialize=False)

    def get(self, key):
        res = KVStore(config.host, self.name).get(key)
