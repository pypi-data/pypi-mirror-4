...
This implementation is compatible with Python 2.5+ and Python 3.
...
<method-wrapper '__setattr__' of dict object ...>
...
try:
    ...
    try:
        ...
        printf('error: stringformat missing. Try `easy_install stringformat`.',
               file=sys.stderr)
...
dbg_fwid = globals()['dbg_fwid'] if 'dbg_fwid' in globals() else 20
...
def main(argv):                                            # ||:fnc:||
    ...
    # |:opt:| add options
    parser.add_argument(
        '-q', '--quiet', action='store_const', const=-2, dest='debug',
        default=0, help='suppress warnings')
    parser.add_argument(
        '-v', '--verbose', action='store_const', const=-1, dest='debug',
        default=0, help='verbose test output')
    parser.add_argument(
    ...
    if adhoc_op:
        adhoc_have_rt = 'RtAdHoc' in globals()
        adhoc_compile = (
            _parameters.implode
            or (adhoc_export and not adhoc_have_rt))

        source = None
        if adhoc_compile or not adhoc_have_rt:
            os_path = os.defpath
            for pv in ('PATH', 'path'):
                try:
        ...
        AdHoc.debug = _debug
        AdHoc.include_path.append(os.path.dirname(__file__))
        ah = AdHoc()
        if adhoc_compile:
            source = ah.compileFile(__file__)
        else:
        ...
        if _parameters.implode:
            ...
            #         ["warning: ", os.path.basename(__file__),
            #          " already imploded!\n"])
            # @:adhoc_enable:@
        ...
        # explode
        if _parameters.explode:
            AdHoc.export_dir = _parameters.adhoc_arg
            AdHoc.export(__file__, source)
            exit(0)

        # extract
        if _parameters.extract:
            AdHoc.extract_dir = _parameters.adhoc_arg
            AdHoc.extract(__file__, source)
            exit(0)

        # template
        if _parameters.template:
            template_name = _parameters.adhoc_arg
            if not template_name:
                template_name = '-'
            if template_name == 'list':
                sys.stdout.write('\n'.join(AdHoc.template_table(__file__, source)))
            else:
                template = AdHoc.get_named_template(template_name, __file__, source)
