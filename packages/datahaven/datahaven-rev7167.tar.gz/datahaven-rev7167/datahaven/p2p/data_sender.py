...
class DataSender(automat.Automat):
    ...
    def A(self, event, arg):
        ...
        #---SCAN_BLOCKS---
            ...
            if event == 'scan-done' and self.isQueueEmpty():
                self.state = 'READY'
            elif event == 'scan-done' and not self.isQueueEmpty():
                self.state = 'SENDING'
        #---SENDING---
        elif self.state == 'SENDING':
            if event == 'restart' or ( ( event == 'block-acked' or event == 'block-failed' ) and self.isQueueEmpty() ):
                self.doScanAndQueue()
                self.state = 'SCAN_BLOCKS'
