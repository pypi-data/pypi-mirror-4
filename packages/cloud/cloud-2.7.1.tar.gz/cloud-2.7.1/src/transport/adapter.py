...
class SerializingAdapter(Adapter):
    ...
    def check_size(self, serializerInst, logfilename, is_result=False, size_limit = None):
        ...
        if totalsize > self.max_transmit_data:
            ...
            if hasattr(serializerInst,'str_debug_report'):
                ...
                exmessage += '\n'
            exmessage += 'Cloud has produced this error as you are %s too large of an object.\n' %('returning' if is_result else 'sending')
    ...
    def _cloud_serialize_helper(self, func, arg_serialization_level, args, argnames=[], logprefix="", coerce_cnt = None):
    ...
    def cloud_serialize(self, func, arg_serialization_level, args, argnames=[], logprefix="", coerce_cnt = None):
        """Return serialized_func, list of serialized_args
        Will save func and args to files.
        """
        serialize_stream, logname, cnt = self._cloud_serialize_helper(func, arg_serialization_level, args, argnames, logprefix, coerce_cnt)
        if func:
