...
def _http_call(the_url, method_type, authorization, **kw):
    '''
    send an http request and expect to return a json object if no error.
    '''
    params = None
    boundary = None
    if method_type==_HTTP_UPLOAD:
...
#            the_url = the_url.replace('https://api.', 'https://rm.api.')
    http_url = '%s?%s' % (the_url, params) if method_type==_HTTP_GET else the_url
    http_body = None if method_type==_HTTP_GET else params
...
class APIClient(object):
    ...
    '''
    def __init__(self, app_key, app_secret, redirect_uri=None, response_type='code',authdomain='graph.renren.com' ,domain='api.renren.com/restserver.do', version='2'):
        self.client_id = str(app_key)
        self.client_secret = str(app_secret)
        self.redirect_uri = redirect_uri
        self.response_type = response_type
        self.auth_url = 'https://%s/oauth/' % authdomain
    ...
    def request_access_token(self, code, redirect_uri=None):
        ...
        if remind_in:
            ...
            if rtime < expires:
                expires = rtime
        r.expires_in = expires
        r.expires = expires
        return r#JsonDict(access_token=r.access_token, expires=expires, expires_in=expires, uid=r.get('uid', None))
...
class _Executable(object):
    ...
    def __call__(self, **kw):
        func_Type = _METHOD_MAP[self._method]
        if func_Type==_HTTP_POST and 'pic' in kw:
            func_Type = _HTTP_UPLOAD
        kw['format']="JSON"
        kw['method']=str(self._path)
        kw['v']='1.0'
        kw['access_token']= self._client.access_token
        return _http_call('%s' % (self._client.api_url), func_Type, self._client.access_token, **kw)
