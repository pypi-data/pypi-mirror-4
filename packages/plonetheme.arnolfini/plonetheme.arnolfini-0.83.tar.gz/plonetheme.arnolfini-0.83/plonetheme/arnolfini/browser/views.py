...
class CommonBrowserView(BrowserView):
    ...
    def addPaypalButton(self, label, name, price):
        return """
            <form action="https://www.paypal.com/cgi-bin/webscr" method="post" onSubmit="return Arnolfini.trackEcommerce('%(name)s', '%(price).2f')">
...
class EventListingView(FolderListing):
    ...
    def resultsToday(self, only_documented=False):
        ...
        for res in self.cachedResults:
            if res.start.strftime("%d/%m/%Y") == today.strftime("%d/%m/%Y") and res.end <= today:
                res_list.append(res)
