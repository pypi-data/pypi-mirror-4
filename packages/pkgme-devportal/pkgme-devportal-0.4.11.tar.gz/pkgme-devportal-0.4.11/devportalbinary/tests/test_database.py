...
class TestDatabase(TestCase, ResourcedTestCase):
    ...
    def test_differing_dependencies(self):
        ...
        self.assertEqual(deps, {'libfoo.so.0': set(['foo', 'bar'])})

    def test_get_dependencies(self):
        db = self.get_package_db()
        db.insert_new_library('foo-src', 'libfoo.so.0', 'foo', 'i386')
        deps = db.get_multiple_dependencies(['libfoo.so.0'], 'i386')
        self.assertEqual(deps, {'libfoo.so.0': set(['foo'])})

    def test_respects_architecture(self):
        db = self.get_package_db()
        db.insert_new_library('foo-src', 'libfoo.so.0', 'foo', 'i386')
        db.insert_new_library('foo-src', 'libfoo.so.0', 'foo-amd64', 'amd64')
        deps = db.get_multiple_dependencies(['libfoo.so.0'], arch='amd64')
        self.assertEqual(deps, {'libfoo.so.0': set(['foo-amd64'])})
    ...
    def test_update_package(self):
        ...
        deps = db.get_multiple_dependencies(['libfoo.so.1'], 'i386')
        self.assertEqual(deps, {'libfoo.so.1': set(['foo-bin'])})
    ...
    def test_update_package_two_architectures(self):
        ...
        deps = db.get_multiple_dependencies(['libfoo.so.1'], arch='i386')
        self.assertEqual(deps, {'libfoo.so.1': set(['foo-bin'])})
...
class TestLibdepServiceClient(TestCase):
    ...
    def test_wraps_libdep_service(self):
        ...
        self.assertEqual(
            {'libfoo': set(['libfoo-bin'])},
            wrapper.get_multiple_dependencies(['libfoo'], 'i386'))
        self.assertEqual(
            {}, wrapper.get_multiple_dependencies(['libbar'], 'i386'))
