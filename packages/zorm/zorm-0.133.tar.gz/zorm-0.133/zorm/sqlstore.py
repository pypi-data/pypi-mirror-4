...
class SqlFarm:
    ...
    def __init__(self, conf=None, **kwargs):
        dbcnf = self.parse_config_string(conf)
        dbcnf.update(kwargs)
        conf = "%s:%s:%s:%s:%s"%(dbcnf["host"], dbcnf["port"], dbcnf["db"], dbcnf["user"], dbcnf["passwd"])
        self._config = conf
        self._cursor = None
        self.b_commit = True

    def connect(self):
        cursor = self.connection_pool.get(self._config,None)
        if cursor:
            return cursor

        dbcnf = self.parse_config_string(self._config)
        conn_params = dict(init_command='set names utf8', **dbcnf)
        ...
        self.connection_pool[self._config] = cursor
        return cursor
    ...
    def cursor(self, ro=False):
        if self._cursor is None:
            self.connection_pool[self._config] = None
...
class SqlStore:
    ...
    def get_db_by_table(self, table):
        tables = self.tables
        farm = tables.get(table)
        if farm is None:
            print "%s TABLE NOT CONFIGURE" % table
            return tables['*']
        else:
            return farm
