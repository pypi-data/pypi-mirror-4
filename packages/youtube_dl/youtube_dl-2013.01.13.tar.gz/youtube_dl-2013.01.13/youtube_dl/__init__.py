...
def parseOpts():
    ...
    postproc.add_option('-P', '--play', action='store_true', dest='play', default=False,
            help='Play video')
    postproc.add_option('--player', metavar='PROGRAM', dest='player', default=None,
            help='Video player executable to use')
    general.add_option('--play-buffer', dest='playbuffer', metavar='BYTES', default='2M',
            help='Start player when downloaded pogress reaches specified (default: %default).'
                ' "finished" to start only when playing has finished.')
...
def _real_main():
    ...
    # Playing the video
    pb = FileDownloader.parse_bytes(opts.playbuffer)
    player = PlayerManager(fd, opts.player, pb)
    if opts.play:
        player.start()
