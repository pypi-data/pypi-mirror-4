...
sleeptime=15*60, running=0):
        if name:
            ...
            self.data['url'] = self.data.url or url
            self.data['owner'] = self.data.owner or owner
            self.data['result'] = []
    ...
    def checkseen(self, data, itemslist=["title", "link"]):
        ...
        for item in itemslist:
            i = item # str(item, "utf-8")
            try: d[i] = data[i]
            except (KeyError, TypeError): continue
        digest = hashlib.md5(bytes(str(d), "utf-8")).hexdigest()
    ...
    def fetchdata(self, data=None):
        ...
        if data:
            result = feedparser.parse(str(data.content, "utf-8"), etag=etag)
                    ...
                    if item.startswith('http://'):
                        if self.markup.get(jsonstring([name, type, channel]), 'tinyurl'):
                            try:
                                ...
                                logging.warn('tinyurl is: %s' % str(tinyurl))
...
class Rsswatcher(Rssdict):
    ...
    def searchall(self, item, search):
        ...
        for name in self.data['names']:
            feed = self.byname(name)
            if feed: res.append(str(feed.search(item, search), "utf-8"))
        return res
...
examples.add('rss-add', 'add a feed to the rsswatcher', 'rss-add fbfbot http://code.google.com/feeds/p/fbfbot/hgchanges/basic')
...
examples.add('rss-register', 'register and start a rss feed', 'rss-register fbfbot-hg http://code.google.com/feeds/p/fbfbot/hgchanges/basic')
...
def handle_rssget(bot, ievent):
    ...
    except Exception as ex: handle_exception() ; ievent.reply('%s error: %s' % (name, str(ex))) ; return
...
examples.add('rss-seturl', 'set the url of a feed', 'rss-seturl fbfbot-hg http://code.google.com/feeds/p/fbfbot/hgchanges/basic')
...
examples.add('rss-import', 'import rss feeds from a remote OPML file.', 'rss-import http:///feeds.opml')
