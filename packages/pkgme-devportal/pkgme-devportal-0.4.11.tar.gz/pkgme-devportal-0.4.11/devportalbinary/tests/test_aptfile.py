...
class AptFilePackageDatabaseTestCase(TestCase):
    ...
    def test_get_dependencies(self):
        ...
        self.assertEqual(
            self.db.get_multiple_dependencies(["libz.so.1"], 'i386'),
            ({'libz.so.1': 'zlib1g'}, set()))
...
""")
        ...
        self.assertEqual(
            db.get_multiple_dependencies(["libfoo.so.2"], arch="i386"),
            ({'libfoo.so.2': set(["pkgfoo", "pkgbar"])}, set()))
