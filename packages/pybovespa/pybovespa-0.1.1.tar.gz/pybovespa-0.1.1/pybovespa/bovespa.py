    ...
    def load_from_file(self, file):
        ...
        for name in zfobj.namelist():
            for line in zfobj.open(name):
                tipreg = decode(line[0:2])
                if tipreg == '01':
                    tpmerc = decode(line[24:27])
                    if tpmerc in ('010', '020'):
                        data = datetime.strptime(decode(line[2:10]), '%Y%m%d')
                        codneg = decode(line[12:24])
                        nomres = decode(line[27:39])
                        #modref = decode(line[52:56])
                        preabe = decode(line[56:59])
                        premax = decode(line[69:82])
                        premin = decode(line[82:95])
                        premed = decode(line[95:108])
                        preult = decode(line[108:121])
