...
#================================================================
  ...
  def close(self, *args):
  #----------------------
    self._req_close = True
    super(StreamClient, self).close(*args)

  def closed(self, code, reason=None):
  #-----------------------------------
    if not self._req_close:
      pass   ### We need to cleanly raise an exception and stop thread,,,
      ## logging.debug('UNREQUESTED CLOSE...')
    self._receiver(None)
