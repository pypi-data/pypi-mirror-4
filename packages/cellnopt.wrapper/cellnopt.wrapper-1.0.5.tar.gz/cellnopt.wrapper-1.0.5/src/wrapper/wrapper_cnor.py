...
from rtools import RPackageManager, Rnames2attributes, buildDocString
from rtools import convertor
import os
# rpy2
...
try:
    pkg = "CellNOptR"
    pm = RPackageManager()
    if pkg not in pm.packages or pm.require(pkg, "1.4.0")==False:
        pm.biocLite(pkg)
    rpack_CNOR = pm[pkg].package
    version = pm[pkg].version

    #pm.biocLite(["Rgraphviz","RUnit", "BiocGenerics"])

    #rgbl = load_Rpackage("RBGL") # required by CellNOptR
    __all__  = ['readSIF', 'readMIDAS',  'makeCNOlist', 
...
@Rsetdoc
@Rnames2attributes # does not work for S4 class
#@S4Class
...
def writeMIDAS(cnolist, filename, timeIndices=None, overwrite=False):
    if version>="1.5.1":
        ...
        rpack_CNOR.writeMIDAS(cnolist, filename)
...
def cutAndPlot(cnolist, model, bStrings, show=True, filename=None):
    ...
    rp = Rplot(show=show, filename=filename)
    rp._start()
    rp.pythoncode("from cellnopt.wrapper import rpack_CNOR; rpack_CNOR.cutAndPlot(cnolist, model, bStrings)")
    rp._end()
