...
def _test_image4d_init(nslices):
    ...
    assert_array_equal(img4d.slice_order, list(range(nslices)))
    img4d = Image4d(data, aff, tr, slice_order='ascending')
    assert_array_equal(img4d.slice_order, list(range(nslices)))
    img4d = Image4d(data, aff, tr, slice_order='descending')
    assert_array_equal(img4d.slice_order, list(range(nslices))[::-1])
    # test interleaved slice order
    slice_order = list(range(nslices))[::2] + list(range(nslices))[1::2]
    ...
    assert_array_equal(img4d.slice_order, list(range(nslices)))
    # or list
    img4d = Image4d(data, aff, tr, slice_order=list(range(nslices)))
    assert_array_equal(img4d.slice_order, list(range(nslices)))
    ...
    for bad_slice_order in (
        ...
        list(range(nslices//2))*2,     # twice the same (would match in length for even nslices)
        ):
...
def test_realign4d():
    ...
    R2 = FmriRealign4d(runs, tr=2., slice_order=list(range(im.shape[slice_axis])))
