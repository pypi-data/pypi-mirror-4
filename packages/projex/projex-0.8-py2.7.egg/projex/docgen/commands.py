...
def generateUserDocs( path, target, page, breadcrumbs = None ):
    """
    ...
    base_url    = '.' + '/..' * len(breadcrumbs)    url_handler.setRootUrl(base_url)
    
    # setup the title
    main_title = text.pretty(os.path.normpath(path).split(os.path.sep)[-1])
    if ( not breadcrumbs ):
        main_title = 'Home'
    ...
    for entry in sorted(os.listdir(path)):
        ...
        # add wiki pages to the system
            ...
            for i, crumb in enumerate(breadcrumbs):
                crumbs.append(crumb % ('.' + '/..' * count))
            
            # add the base crumb
            crumb  = crumb_templ % {'url': './index.html', 'text': main_title}
            crumbs.append(crumb)
            ...
            entry_item = '<a href="./%s">%s</a>' % (url, title)
        ...
        # include sub-documents that contain index.wiki
            ...
            opts = (entry, entry_title)
            entry_item = '<a href="./%s/index.html">%s</a>' % opts
            entry_contents.append( entry_item )
            
            generateUserDocs(filepath, newtarget, page, newcrumbs)
