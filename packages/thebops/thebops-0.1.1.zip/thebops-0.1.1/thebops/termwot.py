...
complete waste of time (c); but it tries to be slightly more useful,
...
def make_snake(ch='o.', width=10, length=4):
    ...
    # liz.append('%*s%s' % (width-1, snake[:-2], head))
...
def generate_caterpillars(width=20, maxbuf=300, *generators):
    """
    ...
    if 0:
        ...
        pdb.set_trace()
    if not generators:
        generators = (caterpillar(copies=2, period=width),
                      caterpillar(copies=2, period=width, prolog=41))
    i = 0
    nobuf = 0
    localdebug = 0
    ...
    for s in caterpillar(period=period):
        ...
        if offset is not None:
            ...
            if s in buf:
                ...
                yield t
    # yield '*' * width
    # yield left('* im Buffer ', 20, '*')
    for p in range(pos):
        yield buf[pos-1][offset:]
        del buf[0]
    # yield left('* Buffer geleert ', 20, '*')
    for i in range(len(buf)):
        buf[i] = left(buf[i][offset:], width)
        # del buf[i][:offset]
    while 1:
...
try:
    if __name__ != '__main__':
        ...
        for i, s in zip(range(30), generate_caterpillar(20)):
            print '%2d: %s' % (i, s)
    elif 0:
        for i, s in zip(range(30), overlay(caterpillar(), caterpillar(23))):
            print '%2d: %s' % (i, s)
    elif 0:
        for s in generate_caterpillar(20):
            print s+'\r',
            _sleep(0.2)
    elif 0:
        for s in generate_caterpillars(maxbuf=200):
            print s
            _sleep(0.1)
except KeyboardInterrupt:
    raise SystemExit
