...
class MakoJavascriptLexer(DelegatingLexer):
    ...
    aliases = ['js+hmako', 'javascript+hmako']
...
def syntax_highlight(filename='', language=None):
    hmako_lexer = MakoLexer()
    if compat.py3k:
        python_lexer = Python3Lexer()
    else:
        python_lexer = PythonLexer()
    if filename.startswith('memory:') or language == 'hmako':
        return lambda string: highlight(string, hmako_lexer,
