...
from xadrpy.core.api.exceptions import IllegalWrapperOption, UnAuthorizedRequest, ServiceException
...
def authorize(func=None, **options):
    def inner(func):
        @wraps(func)
...
def service(func=None, out="result", success="success", error_status=200, **options):
    def inner(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            try:
                result = func(*args, **kwargs)
                if isinstance(out, basestring):#Key of result
                    response = { success: True, out: result, }
                elif isclass(out) and issubclass(out, dict):#Dict response
                    response = { success: True }
                    response.update(result)
                elif isinstance(out, dict):#Dict response with defaults
                    response = { success: True }
                    response.update(out)
                    response.update(result)
                elif isinstance(out, tuple):#List of response
                    response = { success: True }
                    response.update(dict(zip(out, result)))
                else:
                    raise IllegalWrapperOption("Invalid 'out' value: %s" % out)

                return response
            except Exception, e:
                response = _exception(e, success)
                raise ServiceException(unicode(e), e, response, response['error'].get("status", 200))
        return wrapper
    if func:
        return inner(func)
    return inner

def _exception(e, success):
    response = {
        success: False,
        "error": {
            "text": len(e.args) and unicode(e.args[0]) or "",
