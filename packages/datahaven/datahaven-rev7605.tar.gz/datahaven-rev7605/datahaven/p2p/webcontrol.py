...
class StoragePage(Page):
    ...
    def renderPage(self, request):
        ...
        StringDonated = diskspace.MakeStringFromBytes((totalCustomersMB+freeDonatedMB)*1024.0*1024.0)
...
class StorageDonatedImage(resource.Resource):
    ...
    def render_GET(self, request):
        ...
        try:
            if ( len(spaceDict) == 1 and spaceDict.has_key('free') ) or customers_count == 0:
                colorGiven = '#ffffff'
            ...
            if width >= 256 and height >= 256:
                if ( len(spaceDict) == 1 and spaceDict.has_key('free') ) or customers_count == 0:
                        givenBytes = int(float(spaceDict.get('free', 0))*1024*1024)
                    ...
                    for idurl in idurls:
                        ...
                        y1 = math.sin(a * math.pi / 180.0) * R * 0.7 + y0
                        if idurl == 'free':
                            s = diskspace.MakeStringFromBytes(givenBytes)
                            sw, sh = draw.textsize(s, font=font)
                            draw.text((self.toInt(x1-sw/2.0), self.toInt(y1-sh/2.0)), s, fill="#000000", font=font)
                        else:
