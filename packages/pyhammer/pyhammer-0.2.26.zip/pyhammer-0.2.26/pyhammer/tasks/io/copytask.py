...
class CopyTask(TaskBase):
    ...
    def do( self ):
        self.reporter.message( "src: '%s', dest: '%s'" % (self.srcDir, self.destDir) )
    ...
    def __copyFileList(self, fileFilter, srcDir, destDir, reporter):
        ...
        for fp in files:
            ...
            reporter.message("os.path.realpath( srcDir ).lower(): %s" % os.path.realpath( srcDir ).lower())
            reporter.message("fp: %s" % fp)
            reporter.message("os.path.realpath( srcDir ): %s" % os.path.realpath( srcDir ))
            reporter.message("relPath: %s" % relPath)

            destPath = os.path.join( os.path.realpath( destDir ), relPath )
            reporter.message("destPath: %s" % destPath)
            ...
            reporter.message("copying %s to %s" % (fp, destPath))
