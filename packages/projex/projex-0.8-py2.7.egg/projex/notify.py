...
NOTIFY_SERVER               = os.getenv('PROJEX_NOTIFY_SERVER','')
...
def sendEmail(  sender, 
    ...
    msg                 = py_email.MIMEMultipart( _subtype = 'related' )
    ...
    for filepath in filepaths:
        ...
        if ( os.path.exists( filename ) and not filename in attachments ):
            ...
            msgImage = py_email.MIMEImage(fp.read())
            fp.close()
    ...
    msgText                 = py_email.MIMEText(bodyhtml,'html')
    msgText['Content-type'] = contentType
    
    # include attachments
    for attach in attachments:
        fp = open( str(attach), 'rb' )
        txt = py_email.MIMEBase( 'application', 'octet-stream' )
        txt.set_payload( fp.read() )
        fp.close()
        
        py_email.Encoders.encode_base64(txt)
