...
class ParseContext(OutputStream, EventDispatcher):
    ...
    def __iter__(self):
        return self._scanner.__iter__()

    def assign(self, data):
        self._scanner.assign(data, self._termenc)
        if self._buffering:
...
# EventDispatcher
    def dispatch_esc(self, intermediate, final):
        if not self._handler.handle_esc(self, intermediate, final):
            self.put(0x1b) # ESC
    ...
    def dispatch_csi(self, parameter, intermediate, final):
        if not self._handler.handle_csi(self, parameter, intermediate, final):
            self.put(0x1b) # ESC
            self.put(0x5b) # [
            for c in parameter:
                self.put(c)
    ...
    def dispatch_ss2(self, final):
        if not self._handler.handle_ss2(self, final):
            self.put(0x1b) # ESC
            self.put(0x4e) # N
            self.put(final)

    def dispatch_ss3(self, final):
        if not self._handler.handle_ss3(self, final):
            self.put(0x1b) # ESC
            self.put(0x4f) # O
            self.put(final)

    def dispatch_control_string(self, prefix, value):
        if not self._handler.handle_control_string(self, prefix, value):
            self.put(0x1b) # ESC
            self.put(prefix)
            for c in value:
                self.put(c)
    ...
    def dispatch_char(self, c):
        if not self._handler.handle_char(self, c):
            self.put(c)

    def dispatch_invalid(self, seq):
        if not self._handler.handle_invalid(self, seq):
            for c in seq:
                self.put(c)
...
class Session:
    ...
    _alive = True
    _esc_timeout = _ESC_TIMEOUT_DEFAULT
    _esc_timer = None
    ...
    def process_input(self, data):
        ...
        if not self._inputparser.state_is_esc():
            ...
            def dispatch_esc():
                ...
                #self._inputcontext.flush()
                self._outputcontext.flush()
            self._esc_timer = threading.Timer(self._esc_timeout, dispatch_esc)
