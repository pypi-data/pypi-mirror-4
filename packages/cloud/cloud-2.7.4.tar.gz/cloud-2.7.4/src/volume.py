...
def sync(source, dest, delete=False):
    ...
    try:
        ...
        for attempt in xrange(retry_attempts):
            exit_code, stdout, stderr = _perform_sync(*sync_args, delete=delete)
            if not exit_code:
                break
            ...
            raise Exception('sync failed multiple attempts... Please contact PiCloud support')
    ...
    if success:
        ...
        raise cloud.CloudException('Volume sync failed with error code %s. See cloud.log' % exit_code)
...
def _perform_sync(src_arg, dest_arg, delete=False):
    ...
    if not os.path.exists(key_file):
        raise cloud.CloudException('%s not present. Something must have errored earlier. See cloud.log' % key_file)
    ...
    if delete:
        cmd = '%s -avz --delete -e "%s" %s %s' % (rsync_path, ssh_shell,
                                                  src_arg, dest_arg)
    else:
