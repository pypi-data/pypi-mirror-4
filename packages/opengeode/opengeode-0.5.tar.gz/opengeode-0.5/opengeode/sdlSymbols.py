...
class Decision(VerticalSymbol):
    ...
    def __init__(self, parent=None, ast=None):
        ...
        # Can be a terminal symbol if all branches are terminated!
        #self.terminalSymbol = False
    ...
    def terminalSymbol(self):
        ...
        for branch in self.branches():
            if not branch.lastBranchItem.terminalSymbol:
                return False
        else:        
            return True
...
#        pass
    ...
    def checkSyntax(self):
        ...
        try:
            self.scene().reportErrors(syntax_errors)
        except:
            print('[SYNTAX ERROR]', '\n'.join(syntax_errors))
    ...
    def updateConnectionPointPosition(self):
        ...
        if not self.scene():
            ...
            return
        new_y = 0
        new_x = self.boundingRect().width() / 2
        answers = False
        for branch in self.branches():
            answers = True
            last_cnx = None
            ...
            last_pos_y = self.mapFromScene(0, last.scenePos().y()).y()
            try:
            ...
            # Update MP 30/01/13: Don't use QGraphicsItemGroup,
            # it has bugs (with PySide 1.1.2)
            # It puts unwanted focus on editable text of other items
            # group = QGraphicsItemGroup(last.parentItem())
            # group.addToGroup(last)
            # groupHeight = group.boundingRect().height()
            # self.scene().destroyItemGroup(group)
            # branch_len = last_pos_y + groupHeight
            branch_len = branch.y() + (
                    branch.boundingRect() |
            ...
            if isinstance(last, Decision):
                branch_len = last_pos_y + last.connectionPoint.y()
