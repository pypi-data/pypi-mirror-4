...
SPECIAL_OPERATORS = {
    'length': {'type': {'Kind': 'IntegerType'},
        'paramsInOrdered': ['seqof_ref'], 'paramsOutOrdered': [],
        'in': {'seqof_ref': {'type': '_SEQOF'}}},
    'write': {'type': {'Kind': 'IntegerType'},
        'paramsInOrdered': None, 'paramsOutOrdered': None},
    'writeln': {'type': {'Kind': 'IntegerType'},
        'paramsInOrdered': None, 'paramsOutOrdered': None},
...
def check_and_fix_op_params(op_name, expr_list, process):
    '''
        Verify and/or set the type of a procedure/operator/output parameters
        Return false if there is a type mismatch
    '''
    # (1) Find the signature of the function
    for key, item in process.outputSignals.viewitems():
        ...
        for key, item in process.operators.viewitems():
            ...
            for key, item in process.procedures.viewitems():
                ...
                for key, item in SPECIAL_OPERATORS.viewitems():
                    ...
                    LOG.error('Operator/output/procedure not found: '
                            + op_name)
                    return False
    # (2) Build a list of all input and output params from the function spec
    in_params = item.get('paramsInOrdered')
    out_params = item.get('paramsOutOrdered')
    #all_params = in_params.copy().update(out_params)
    # (3) Check that the number of parameters matches
    if(in_params is not None and out_params is not None and
            len(expr_list) != len(in_params + out_params)):
        LOG.error('Wrong number of params in op call ' + op_name)
        return False
    ...
    for idx, p in enumerate(expr_list):
        ...
        if p.exprType.get('Kind') == 'UnknownType':
            if paramType is not None and key not in SPECIAL_OPERATORS.keys():
                ...
                LOG.error('Unable to determine param type: ' +
                        p.inputString + '(in operator ' + op_name + ')')
                return False
    return True
...
class sdlScc_AST:
        ...
        for child in root.getChildren():
            if child.type == ID:
                ...
                if(firstId.lower() in SPECIAL_OPERATORS or
                    ...
                    LOG.debug('[DEBUG] calling check_and_fix_op_params')
    ...
    def text_area(self, root, parent=None):
        ...
        for child in root.getChildren():
            if child.type == CIF:
                ...
                ta.pos_x, ta.pos_y, ta.width, ta.height = self.cif(child)
                ta.abs_x = ta.pos_x
                ta.abs_y = ta.pos_y
        ...
        err = [[e, [ta.abs_x, ta.abs_y]] for e in errors]
        warn = [[w, [ta.abs_x, ta.abs_y]] for w in warnings]
        return ta, err, warn
    ...
    def input_part(self, root, parent):
        ...
        for child in root.getChildren():
            if child.type == CIF:
                ...
                i.pos_x, i.pos_y, i.width, i.height = self.cif(child)
                i.abs_x = i.pos_x + parent.abs_x if parent else 0
                i.abs_y = i.pos_y + parent.abs_y if parent else 0
                ...
                # Report errors with symbol coordinates
                errors = [[e, [i.abs_x, i.abs_y]] for e in errors]
                warnings = [[w, [i.abs_x, i.abs_y]] for w in warnings]
    ...
    def state(self, root, parent):
        ...
        for child in root.getChildren():
            if child.type == CIF:
                # Get symbol coordinates
                s.pos_x, s.pos_y, s.width, s.height = self.cif(child)
                s.abs_x = s.pos_x + parent.abs_x if parent else 0
                s.abs_y = s.pos_y + parent.abs_y if parent else 0
    ...
    def end(self, root, parent=None):
        ...
        for child in root.getChildren():
            if child.type == CIF:
                # Get symbol coordinates
                c.pos_x, c.pos_y, c.width, c.height = self.cif(child)
                c.abs_x, c.abs_y = c.pos_x, c.pos_y
    ...
    def output(self, root, parent):
        ...
        for child in root.getChildren():
            if child.type == CIF:
                # Get symbol coordinates
                o.pos_x, o.pos_y, o.width, o.height = self.cif(child)
                o.abs_x = o.pos_x + parent.abs_x if parent else 0
                o.abs_y = o.pos_y + parent.abs_y if parent else 0
                ...
                warnings.append('Unsupported output child type: ' +
                        str(child.type))
        # Report errors with symbol coordinates
        err = [[e, [o.abs_x, o.abs_y]] for e in errors]
        warn = [[w, [o.abs_x, o.abs_y]] for w in warnings]
        return o, err, warn
    ...
    def alternative_part(self, root, parent):
        ...
        for child in root.getChildren():
            if child.type == CIF:
                # Get symbol coordinates
                ans.pos_x, ans.pos_y, ans.width, ans.height = self.cif(child)
                ans.abs_x = ans.pos_x + parent.abs_x if parent else 0
                ans.abs_y = ans.pos_y + parent.pos_y if parent else 0
            ...
            if child.type in (CLOSED_RANGE, CONSTANT,
                ...
                # Report errors with symbol coordinates
                errors = [[e, [ans.abs_x, ans.abs_y]] for e in errors]
                warnings = [[w, [ans.abs_x, ans.abs_y]] for w in warnings]
    ...
    def decision(self, root, parent):
        ...
        for child in root.getChildren():
            if child.type == CIF:
                # Get symbol coordinates
                dec.pos_x, dec.pos_y, dec.width, dec.height = self.cif(child)
                dec.abs_x = (dec.pos_x + parent.abs_x if parent else 0 +
                            dec.width / 2)
                dec.abs_y = (dec.pos_y + parent.abs_y if parent else 0 +
                            dec.height / 2)
            elif child.type == QUESTION:
                dec.kind = 'question'
                decisionExpr, err, warn = self.expression(child.getChild(0))
                for e in err:
                    errors.append([e, [dec.abs_x, dec.abs_y]])
                for w in warn:
                    warnings.append([w, [dec.abs_x, dec.abs_y]])
                ...
                for c in child.getChildren():
                    if c.type == CIF:
                        a.pos_x, a.pos_y, a.width, a.height = self.cif(c)
                        a.abs_x = a.pos_x + dec.abs_x
                        a.abs_y = a.pos_y + dec.abs_y
                    elif c.type == TRANSITION:
                        a.transition, err, warn = self.transition(c, parent=a)
                        for e in err:
                            errors.append([e, [a.abs_x, a.abs_y]])
                        for w in warn:
                            warnings.append([w, [a.abs_x, a.abs_y]])
                    elif child.type == HYPERLINK:
                ...
                warnings.append(['Unsupported DECISION child type: ' +
                    str(child.type), [dec.abs_x, dec.abs_y]])
    ...
    def terminator_statement(self, root, parent):
        ...
        for term in root.getChildren():
            if term.type == CIF:
                t.pos_x, t.pos_y, t.width, t.height = self.cif(term)
                t.abs_x = t.pos_x + parent.abs_x if parent else 0
                t.abs_y = t.pos_y + parent.abs_y if parent else 0
                ...
                warnings.append('Unsupported terminator type: ' +
                        str(term.type))
        # Report errors with symbol coordinates
        err = [[e, [t.abs_x, t.abs_y]] for e in errors]
        warn = [[w, [t.abs_x, t.abs_y]] for w in warnings]
        return t, err, warn
    ...
    def task(self, root, parent=None):
        ...
        for child in root.getChildren():
            if child.type == CIF:
                # Get symbol coordinates
                t.pos_x, t.pos_y, t.width, t.height = self.cif(child)
                t.abs_x = t.pos_x + parent.abs_x if parent else 0
                t.abs_y = t.pos_y + parent.abs_y if parent else 0
...
def parserInitialize(fileName=None, string=None):
    ...
    try:
        ...
        try:
            ...
            LOG.error('Impossible to initialize the parser')
            return
