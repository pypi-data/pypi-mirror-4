...
from fbf.utils.url import useragent, geturl2
...
from fbf.utils.url import posturl
from fbf.lib.errors import NoSuchBotType
from fbf.lib.threads import start_new_thread
from fbf.lib.datadir import getdatadir
from fbf.lib.fbfimport import import_byfile
from fbf.imports import getfeedparser
...
def subscribe(url):
    ...
    subscribe_args = {
        'hub.callback': urllib.parse.urljoin('https://fbfbot.appspot.com', '/hubbub'),
        'hub.mode': 'subscribe',
        'hub.topic': url,
        'hub.verify': 'async',
    ...
    except IOError: logging.error("missing %s" % getdatadir() + os.sep + "config" + os.sep + "credentials.py") ; return
...
try:
    from fbf.plugins.tinyurl import get_tinyurl
except ImportError:
    def get_tinyurl(url):
        return [url, ]
...
class HubbubItem(Persist):
    ...
    def __init__(self, name, url="", owner="", itemslist=['title', 'link'], watchchannels=[], running=1):
        filebase = getdatadir() + os.sep + 'plugs' + os.sep + 'fbf.plugs.wave.hubbub' + os.sep + name
...
examples.add('hb-subscribe', 'subscribe to a feed', 'hb-subscribe fbf-hg http://code.google.com/feeds/p/fbf/hgchanges/basi')
...
def handle_hubbubadd(bot, ievent):
    ...
    if not result: ievent.reply("can't subscribe to feed") ; return
...
examples.add('hb-add', 'hubbub-add <name> <url> to the watcher', 'hb-add fbf-hg http://code.google.com/feeds/p/fbf/hgchanges/basic')
...
def handle_hubbubstart(bot, ievent):
    ...
    if bot.type == "wave":
        ...
        if wave and wave.data:
            ...
            if wave.data.loud:
                try: ievent.set_title("FBFBOT - %s - #%s" % (' - '.join(wave.data.feeds), str(wave.data.nrcloned)))
                except Exception as ex: handle_exception()
...
def handle_hubbubregister(bot, ievent):
    ...
    result = subscribe(url)
    if not result: ievent.reply("can't subscribe") ; return
    if int(result.status) > 200 and int(result.status) < 300:
...
examples.add('hb-register', 'hb-register .. register url and start it in one pass', 'hb-register hgrepo http://code.google.com/feeds/p/fbf/hgchanges/basic')
