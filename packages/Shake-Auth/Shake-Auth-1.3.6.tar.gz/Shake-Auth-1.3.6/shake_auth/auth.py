...
class Auth(object):
    ...
    format (without the spaces):
        ...
        sha512 $ cost $ salt $ salted_and_hashed_password
        - or -
    ...
    def __init__(self, User, app=None, **settings):
        ...
        if not hash_alg:
            ...
            raise AssertionError("The `hash_alg` parameter must be either " \
                "'scrypt', 'bcrypt', 'dsha512' or 'dsha256'")
    ...
    def _loginpass_authentication(self, login, password, update):
        ...
        if update:
            ...
            if must_update_hash or must_update_cost:
                print '\n------------ Must update! -------------\n'
                print hash_alg, self.settings.hash_alg
                print hash_cost, self.settings.hash_cost
    ...
    def hash_password(self, password, salt=None):
        ...
        hashed = self.hash_func(password, salt=salt, cost=self.settings.hash_cost)
        return hashed
    ...
    def sign_in_view(self, request, **kwargs):
        """Signs a user in.
        """
        redirect_url = request.session.pop(REDIRECT_FIELD_NAME, None)
