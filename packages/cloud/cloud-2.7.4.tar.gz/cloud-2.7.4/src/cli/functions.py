...
def execute_map(command, args, mapargs, argfiles, duplicates,                
    ...
    if not mapargs and not argfiles:
        raise ValueError('At least one -n, or -a must be provided')
...
def ssh(jid, timeout=None):
    """SSH into a job"""
    from ..shortcuts.ssh import get_ssh_command
    ssh_cmd = get_ssh_command(jid, timeout)
    
    try:
        from .pseudo_terminal import Interceptor
        p = Interceptor()
    except ImportError:
        from ..shortcuts.ssh import get_ssh_info
        print >> sys.stderr, 'Cannot ssh into a job on your platform. To connect, manually ssh with the following parameters:'
        ssh_info = get_ssh_info(jid, timeout)
        for item in ssh_info.items():
            print >> sys.stderr, '%s: %s' % item
        return
    else:        
        cloudLog.debug('Invoking %s', ssh_cmd)
        p = Interceptor()    

    p.spawn(['/bin/sh', '-c', ssh_cmd])
