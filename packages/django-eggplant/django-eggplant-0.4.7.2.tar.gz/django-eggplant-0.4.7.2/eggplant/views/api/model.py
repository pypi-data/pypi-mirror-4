...
class ModelCollectionView(ModelViewBase):
    ...
    def get_collection(self, *args, **kwargs):
        ...
        if raw_order:
            ...
            if len(orders):
                query_set = self.order(query_set, orders, *args, **kwargs)

        # Filter
#        raw_filter = self.request.GET.get('filter', None)
#        if raw_filter:
#            try: filters = json.loads(raw_filter)
...
#                # TODO: do filter
#            except ValueError:
#                e = exceptions.BadRequestError('Invalid filter format')
#                e.user_info = {'raw_filter': filter}
#                raise e
    ...
    def order(self, query_set, orders, *args, **kwargs):
        result_order_list = []
        for direction, raw_field in orders:
            ...
            if isinstance(order, (list, tuple)):
                ...
                result_order_list += [order]
        return query_set.order_by(*result_order_list)
