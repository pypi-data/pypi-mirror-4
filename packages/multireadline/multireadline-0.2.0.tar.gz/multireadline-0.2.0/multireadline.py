...
class Multireadline(object):
    ...
    def get(self, name):
        # can't call get without a name 
        if not name in self.rls:
            raise IdMissError(name)
        self._switchrl(name, "NULL")
        # get line of input (and here do checks against 
        ok = False
        while not ok:
            try:
                line = self._getline(name)
            except BadInputError:
                continue
            ok = True
        self._switchrl("NULL", name)
        return line


    def _switchrl(self, incoming, outgoing):
        # 1. copy mrl history to outgoing history
        c = self.rls[outgoing]
        c['hist'] = [ readline.get_history_item(i)
                      for i in range(1, readline.get_current_history_length() + 1) ]
        # 2. copy mrl options to outgoing options
        if len(self.options) > 0:
            c['options'] = self.options
        # 3. copy incoming history to mrl history
        c = self.rls[incoming]
        readline.clear_history()
        for line in c['hist']:
            readline.add_history(line)
        # 4. assign incoming options to mrl options list
    ...
    def _input(self, name):
        return input(self.rls[name]['prompt'])
