...
def bench_lips3d():
    ...
    print("--------------------")
    print('Box1 %6.2f\n' % npt.measure('phi(c,box1)', repeat), end=' ')
    print('Box2 %6.2f\n' % npt.measure('phi(c, box2)', repeat), end=' ')
    print('Box1+2 %6.2f\n' % npt.measure('phi(c, box1 + box2)', repeat), end=' ')
    d = np.random.standard_normal((10,) + (bx_sz,) * 3)
    print('Box1+2 d %6.2f\n' % npt.measure('phi(d, box1 + box2)', repeat), end=' ')
    U = randorth(p=6)[0:3]
    e = np.dot(U.T, c.reshape((c.shape[0], -1)))
    e.shape = (e.shape[0],) + c.shape[1:]
    print('Box1+2 e %6.2f\n' % npt.measure('phi(e, box1 + box2)', repeat), end=' ')
    print('Box1+2 EC %6.2f\n' % npt.measure('EC3d(box1 + box2)', repeat), end=' ')
    sys.stdout.flush()
...
def bench_lips2d():
    ...
    e.shape = (e.shape[0],) + c.shape[1:]
    print('Box1+2 e %6.2f\n' % npt.measure('phi(e, box1 + box2)', repeat), end=' ')
    print('Box1+2 EC %6.2f\n' % npt.measure('EC2d(box1 + box2)', repeat), end=' ')
    sys.stdout.flush()
...
def bench_lips1d():
    ...
    e.shape = (e.shape[0],) + c.shape[1:]
    print('Box1+2 e %6.2f\n' % npt.measure('phi(e, box1 + box2)', repeat), end=' ')
    print('Box1+2 EC %6.2f\n' % npt.measure('EC1d(box1 + box2)', repeat), end=' ')
    sys.stdout.flush()
