...
class CRUDControllerMeta(type):
    def __init__(cls, name, bases, dct):
        if cls.__saclass__ is not None:
            ...
            cls.create = expose(cls.__wstype__, method='PUT')(cls.create)
            cls.create = validate(cls.__wstype__)(cls.create)

            cls.read = expose(cls.__wstype__, method='GET')(cls.read)
            cls.read = validate(cls.__wstype__)(cls.read)

            cls.update = expose(cls.__wstype__, method='POST')(cls.update)
            cls.update = validate(cls.__wstype__)(cls.update)

            cls.delete = expose(method='DELETE')(cls.delete)
