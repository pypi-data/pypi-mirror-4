    ...
    def template_list(cls, file_=None, source=None,          # |:clm:|
        ...
        if all_:
            ...
            result = list(sorted(
                ...
                key=lambda kt: '||'.join((
                    kt if
                    not (kt.startswith('-') or kt.startswith('!')) else
                    kt[1:]))))
        return result
            ...
            if section[0]:
                ...
                if mo:
                    ...
                    if mo:
                        ...
                        module = mo.group(2)
                if len(module) > 0:
                    module_flat = True if 'flat' in import_args.lower().split() else None
                    module_flat = False if 'full' in import_args.lower().split() else module_flat
                    module_forced = True if 'force' in import_args.lower().split() else forced
...
def main(argv):                                            # ||:fnc:||
    ...
    class AdHocAction(argparse.Action):
        def __call__(self, parser, namespace, values, option_string=None):
            map(lambda opt: setattr(namespace, opt, False),
                ('implode', 'explode', 'extract', 'template', 'compile', 'decompile'))
    ...
    if adhoc_op:
        ...
        AdHoc.template_process_hooks = {'doc/index.rst': tpl_hook_doc_index_rst}
