...
from fbf.utils.timeutils import strtotime, striptime, today 
...
from fbf.lib.datadir import datadir
    ...
    def delete(self, name, indexnr):
        ...
        try:
            ...
            if warnsec:
                alarmnr = 0 - warnsec
                if alarmnr > 0:
                    alarms = plugs.get("fbf.plugs.extra.alarm")
...
def handle_settodo(bot, ievent):
    ...
    if not ttime  == None:
        ievent.reply('time detected ' + time.ctime(ttime))
        what = striptime(what)
        karma = plugs.get("fbf.plugs.db.karma2")
