...
class Pack(object):
    ...
    def render(self, bit_diameter, bit_pad):
        ...
        for i in range(bol):
            ...
            if any( pos-margin <= item <= pos+bol_width for item in xmins ) or \
               any( pos-bol_width <= item <= pos+margin for item in xmins ):
                continue
...
def cut_and_pack(outer, bore, top_fractions, bottom_fractions, xsize, ysize, zsize, bit_diameter, save, extra_packables=[]):
    ...
    packs = pack(template, packables, float(ysize)/xsize)
    for item in packs:
        mill_squish(item)
