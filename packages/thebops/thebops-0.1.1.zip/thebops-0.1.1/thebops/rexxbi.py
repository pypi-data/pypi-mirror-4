...
def subword(s, first, count=None):
    ...
    if count is not None:
        ...
        eidx = None
    if 0:\
    print 'subword(%r, %d, %s)  --> (%d, %s)' \
            % (s, first, count, sidx, eidx)
    ii = 0
    indexes = _wordborders(s)
    import pdb
    # pdb.set_trace()
    for spos in indexes:
        # print 'indexes[%d] = %d' % (ii, spos)
        if ii == sidx:
            if count is None:
    ...
    if ii == 0:
        return ''
    # print 'Suche nach dem Ende ...'
    for epos in indexes:
        # print 'indexes[%d] = %d' % (ii, epos)
        if ii == eidx:
            return s[spos:epos]
        elawo = epos
        ii += 1
