...
from fbf.utils.generic import toenc, fromenc
from fbf.utils.generic import getrandomnick, strippedtxt
from fbf.utils.generic import fix_format, splittxt, uniqlist
from fbf.utils.locking import lockdec, lock_object, release_object
...
from fbf.lib.morphs import inputmorphs, outputmorphs
...
class Irc(BotBase):
    ...
    def _raw(self, txt):
        ...
        except Exception as ex: handle_exception() ; logging.error("%s - can't send: %s" % (self.cfg.name, str(ex)))
    ...
    def _readloop(self):
        ...
        while not self.stopped and not self.stopreadloop and self.sock and self.fsock:
            try:
                ...
                self.error = str(ex)
                if ex.errno == EAGAIN: time.sleep(0.5) ; continue
                elif ex.errno != EPIPE and ex.errno != EBADF: raise
    ...
    def logon(self):
        ...
        username = self.cfg.username or "fbfbot"
        realname = self.cfg.realname or "fbfbot"
        time.sleep(1)
    ...
    def quit(self, reason='http://github.com/feedbackflow/fbfbot & http:///docs/fbfbot'):
        """ send quit message. """
    ...
    def handle_ctcp(self, ievent):
        ...
        if ievent.txt.find('PING') != -1:
            try:
                ...
                pingtifbf = ievent.txt.split()[2]
                if pingtime:
                    self.ctcpreply(ievent.nick, 'PING ' + pingtime + ' ' + pingtifbf)
            except IndexError:
                pass
