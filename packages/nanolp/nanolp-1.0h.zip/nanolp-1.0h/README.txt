...
There is a special command line option `-r` to generate **references file**.
It has the same name as input file but with suffix `-refs` and extension `.html`.
There are all defined code chunks, cross-references, information about input
and other.

All commands there are links (cross-references). Commands pasted by glob-paths
are accessible via popup-box (move mouse over such glob-paths) (at the moment
it does not work in IE).

Also will be created CSS style file with name `nanolp.css`, if not exists
already in output directory (to avoid loosing of users CSS changes).
...
Run test.py::
    ...
    python test.py -f [-q] -- do only doctests [-quiet]
    python test.py -d -- do only file tests (see tests/ dir)

Extending
=========

* To add new format support, extend base class Parser (OOParser...)
* To add new command processor, extend Cmd (FileCmd...) - to obtain code from
  VCS or something else
...
Styles inheritence is supported.
...
Processes monospace fragments (`+some text+`) as inline chunks. Supported
block chunks looks::
...
Parser detects forms as \verb#some text# as inline chunk. Instead of verb any word is possible (set it in TEXPARSER_INLCMDS parameter in ConfigFile). There are, for example, verb and verb*. So, as inline are interpreters next forms::
...
Instead of verbatim any word of TEXPARSER_BLKCMDS from config file are possible 
