...
from mothership.database.model import *
import mothership.validate

import mothership.logging
...
class SSH(object):
    def unpack_ssh_public_key_file(cfg, keyfile):
        ...
        if os.path.isfile( keyfile ):
            ...
            raise IOError("public key file does not exist, aborting!")
    ...
    def write_key_to_file(cfg, pubkey_string, keyfile):
        ...
        f = open(keyfile, 'w')
        if not f:
            raise IOError("Unable to open keyfile")
