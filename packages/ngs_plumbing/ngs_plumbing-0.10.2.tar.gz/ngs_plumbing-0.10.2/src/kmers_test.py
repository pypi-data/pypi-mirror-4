...
class KmerTestCase(unittest.TestCase):
    ...
    def test_kmerbytearray_frombytes_iter(self):
        ...
        self.assertEquals(len(offsets), len(res))
        for i, o in enumerate(offsets):
            self.assertEquals(string[o:(o+step)], res[i])
    ...
    def test_kmerbit2_frombytes_iter(self):
        ...
        for i, o in enumerate(offsets):
            ...
            self.assertEquals(bitpacked, res[i])
    ...
    def test(self):
        ...
        for i, pos in enumerate(range(0, len(string), step)):
            kmer_pos, kmer = res[i]
            self.assertEquals(dna.PackedDNABytes(string[pos:(pos+K)]), kmer)
