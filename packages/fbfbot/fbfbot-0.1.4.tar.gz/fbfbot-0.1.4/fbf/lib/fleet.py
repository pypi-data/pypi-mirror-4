...
from fbf.utils.lazydict import LazyDict## simplejson imports
...
class Fleet(Persist):
    ...
    def loadall(self, names=[]):
        ...
        for name in target:
            ...
            except BotNotEnabled: logging.warn("%s is not enabled - use 'fbf fleet-enable %s' to enable" % (name, name)) ; continue
    ...
    def get(self, type, name, config={}):
        ...
        if not bot: logging.error("failed to get bot %s (%s) from %s" % (type, name, whichmodule(2)))
        return bot
    ...
    def addbot(self, bot):
        ...
        else: logging.error("%s bot is already in fleet" % bot.cfg.name) ; return False
