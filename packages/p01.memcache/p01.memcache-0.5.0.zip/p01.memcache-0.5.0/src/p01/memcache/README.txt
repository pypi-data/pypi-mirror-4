...
Anything else get pickled as key base:

  >>> import cPickle
  >>> client.namespace = None
  >>> p = cPickle.dumps(client, protocol=0)

  >>> client.buildKey(client) == md5(p).hexdigest()
  True

  >>> client.namespace = 'n'
  >>> p = cPickle.dumps(client, protocol=0)
  >>> client.buildKey(client) == md5('n%s' % p).hexdigest()
  True
...
As you can see we will get a different value for the different pickle protocol:
  ...
  >>> p1 = client.client.cache['acbd18db4cc2f85cedef654fccc4a4d8']
  >>> p1
  ('Vbar\np1\n.', datetime.datetime(...))
  ...
  >>> pc.set('foo', u'bar')
  'acbd18db4cc2f85cedef654fccc4a4d8'
  >>> p2 = client.client.cache['acbd18db4cc2f85cedef654fccc4a4d8']
  >>> p2
  ('X\x03\x00\x00\x00barq\x01.', datetime.datetime(...))
...
As you can see, our fake memcache stores a raw string as value:
  ...
  >>> client.client.cache['0cc175b9c0f1b6a831c399e269772661'][0]
  'value of a'
...
not implement this method yet:
  ...
  Traceback (most recent call last):
  ...
  AttributeError: 'FakeMemcached' object has no attribute 'get_stats'
