...
class Datatxt(object):
    ...
    def annotate(self, text, properties=True, wikilinks=False):
...
if __name__ == '__main__':
    d = Datatxt()
    res = d.annotate("Sulle colline basaltiche, vicino a Rovereto si produce il Marzemino")
    for ann in res['annotations']:
        print("You are talking about <%s> (@pos %d, %d), with confidence %f" %
                (ann['ref'], ann['start'], ann['end'], ann['rho']))
        for key, vlist in ann['properties'].items():
            if len(vlist) == 1:
                values = vlist[0]
            else:
                values =  ' | '.join(str(vlist)).decode('utf8')
