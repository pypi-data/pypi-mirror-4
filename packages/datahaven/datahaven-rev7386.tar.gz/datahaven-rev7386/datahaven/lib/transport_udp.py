...
class TransportUDP(automat.Automat):
    ...
    def doStartAllUDPSessions(self, arg):
        ...
        for idurl in all:
            ...
            dhnio.Dprint(6, 'transport_udp.doStartAllUDPSessions init %s with [%s]' % (sess.name, nameurl.GetName(idurl)))
        ...
        for idurl in arg[0]:
            ...
            if idurl not in arg[1]:
                ...
                if transport_udp_session.is_session_opened(address):
                    dhnio.Dprint(6, 'transport_udp.doRemoveOldUDPSession shutdown %s with [%s]' % (str(address), nameurl.GetName(idurl)))
                ...
                if address_local != address:
                    if transport_udp_session.is_session_opened(address_local):
                        dhnio.Dprint(6, 'transport_udp.doRemoveOldUDPSession shutdown %s with local peer [%s]' % (str(address_local), nameurl.GetName(idurl)))
    ...
    def doCreateNewUDPSession(self, arg):
        for idurl in arg[1]:
            ...
            if idurl not in arg[0]:
                ...
                address = self.remapAddress(address)
                sess = transport_udp_session.open_session(address)
                dhnio.Dprint(6, 'transport_udp.doCreateNewUDPSession %s with [%s]' % (sess.name, nameurl.GetName(idurl)))
                sess.automat('init', idurl)
    ...
    def doRestartUDPSession(self, arg):
        ...
        for sess in transport_udp_session.sessions():
            if  sess.remote_idurl is not None and \
                ...
                dhnio.Dprint(6, 'transport_udp.doRestartUDPSession shutdown %s with [%s]' % (sess.name, nameurl.GetName(idurl)))
                sess.automat('shutdown')
        sess = transport_udp_session.open_session(address)
        dhnio.Dprint(6, 'transport_udp.doRestartUDPSession init %s with [%s]' % (sess.name, nameurl.GetName(idurl)))
        sess.automat('init', idurl)
