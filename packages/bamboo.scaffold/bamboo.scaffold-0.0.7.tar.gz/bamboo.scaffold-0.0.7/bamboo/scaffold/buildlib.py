...
def myput(fromfile, appnamespace):
    put(fromfile, '/etc/init/%s.conf' % appnamespace,
           use_sudo=True, mode=0755)
...
def install_upstart(appnamespace, confd, port=None):
    """ An alias for :function:upstart() """
    return upstart(appnamespace, confd, port=None)

def upstart(appnamespace, confd, port=None):
    """ install onto ubunut a upstart tmpl 
    
    .. todo:: Rethink this -0 its a vital piece and is so fragile
    ...
    tmpl_repo = os.path.join(confdir, "upstart-%s.conf" % appnamespace)
    print "XXX upstart-%s.conf" % appnamespace
    tmpl = open(tmpl_repo).read()
    open("/tmp/foo.t", "w").write(tmpl % confd)
    
    execute(myput, "/tmp/foo.t", appnamespace)
