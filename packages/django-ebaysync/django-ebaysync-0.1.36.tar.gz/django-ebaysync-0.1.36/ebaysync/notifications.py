...
def ebay_timestamp_string(datetime_obj):
    ...
    # (truncate python's microseconds to eBay's milliseconds)
    # eBay timestamps are UTC... the 'Z' timezone
    return '%sZ' % datetime_obj.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3]
...
class NotificationHandler(object):
    ...
    def validate(self, message, signature):
        ...
        # make hash
        m = hashlib.md5()
        m.update(ebay_timestamp_string(message.Timestamp))
