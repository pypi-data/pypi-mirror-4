...
class Monitor(Actor):
    def pre_start(self):
        ...
        # self.msg_receive_started = time.time()
    ...
    def receive(self, msg):
        ...
        if ('connect', ANY) == msg:
            ...
            _, d, filter = msg
            d.set([(str(k.uri), v) for k, v in self.up.items() if k == filter])
        elif ('get-state', ANY, ANY) == msg:
            _, d, filter = msg
            d.set([(str(k.uri), v) for k, v in self.state.items() if k == filter])
        elif ('get-log', ANY, ANY) == msg:
            _, d, filter = msg
            d.set([x for x in self.log if x == filter])
            ...
            d.set([(uri, status, seen, state) for uri, (status, seen, state) in data.items()])
        else:
            raise Unhandled
...
class MonitorClient(Actor):
    ...
    def connect(self, delayed=False):
        if self.connecting is not None:
            self.connecting.kill()
        self.connecting = spawn_later(CLIENT_RECONNECT_INTERVAL, self.connect)

        def do_connect():
            self.watch(self.monitor)
            self.monitor << ('connect', self.ref)
        if delayed:
            spawn_later(1.0, do_connect)
        else:
            do_connect()
