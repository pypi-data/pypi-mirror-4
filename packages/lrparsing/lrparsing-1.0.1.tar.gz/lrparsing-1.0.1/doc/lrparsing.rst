...
(very roughly 25us per token from string to :term:`parse tree` on
a desktop CPU), pre-compilation of the grammar and error recovery
...
.. function:: compile_grammar(grammar)
    ...
    slow.  :func:`Repr_parse_table() <repr_parse_table>` displays
    a lot more information when the ``grammar`` is
    compiled by :func:`compile_grammar`.
