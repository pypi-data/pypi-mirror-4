    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if category.startswith('newcat-'):
                ...
                #category, created = Category.objects.get_or_create(user=request.user, name=u'ﺏﺩﻮﻧ ﺪﺴﺘﻫ ﺐﻧﺩی')
                category_id = None
            else:
                category = Category.objects.filter(user=request.user, id=category).all()[0]
                category_id = category.id
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if category.startswith('newcat-'):
                ...
                #category, created = Category.objects.get_or_create(user=request.user, name=u'بدون دسته بندی')
                category_id = None
            else:
