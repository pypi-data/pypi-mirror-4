...
#TODO: catch integer cast exception
            ...
            if max_level == 3:
                c3_pairs = [(s, t) for (s, t) in all_pairs if s.ibgp_l3_cluster == t.ibgp_l3_cluster] # l3 cluster pairs
                ...
                l2_l3_up_links = [ (s, t) for (s, t) in all_pairs if s.ibgp_level == 2 and t.ibgp_level == 3
                        if s.ibgp_l2_cluster == t.ibgp_l2_cluster
                ...
                for l3_cluster in l3_clusters:
                    ...
                    for l2_cluster in l2_clusters:
                        ...
                        if any(r.ibgp_level == 2 for r in l2_cluster_devices):
                            log.info("L2 cluster %s in L3 cluster %s in ASN %s has l2 devices, not adding extra links" % (l2_cluster, l3_cluster, asn))
                        else:
                            l1_routers = [r for r in l2_cluster_devices if r.ibgp_level == 1]
                            l3_routers = [r for r in l2_cluster_devices if r.ibgp_level == 3]
                            log.info("L2 cluster %s in L3 cluster %s in ASN %s has no level 2 iBGP routers."
                                    "Connecting level 1 routers (%s) to level 3 routers (%s)"
                                    % (l2_cluster, l3_cluster, asn, l1_routers, l3_routers))
