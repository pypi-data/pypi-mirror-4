...
_MenuItems = {
    ...
    '2|settings'            :('/'+_PAGE_CONFIG,             'icons/settings01.png'),
    '3|money'               :('/'+_PAGE_MONEY,              'icons/money01.png'),
...
#    global url_history
#    if len(url_history) == 0 or url_history[-1] != request.path:
#        url_history.append(request.path)
#    if len(url_history) > 1 and url_history[-1] == request.path:
#        del url_history[-1]
#    while len(url_history) > 10:
#        del url_history[0]
    ...
    if not d.has_key('back'):
        request_back = arg(request, 'back', None)
        if request_back is not None and request_back != request.path:
            d['back'] = '<a href="%s">[back]</a>' % request_back
        else:
            d['back'] = '&nbsp;'
    else:
        if d['back'] == '':
...
def GetSuppliersActiveCount():
    ...
    for i in range(0, settings.getCentralNumSuppliers()):
        if contact_status.isOnline(contacts.getSupplierID(i)):
            activeCount += 1
    return activeCount
...
class InstallPage(Page):
    ...
    def renderCentralSettingsPage(self, request):
        ...
        src += '</td>\n'
        src += '</tr>\n<tr>\n'
        src += '<td align=center valign=top nowrap><br><b>incoming bandwidth limit (KB./sec.)</b>\n'
        src += '<br><input type="text" name="bandin" size="10" value="%s" />\n' % self.bandin
        src += '</td>\n'
        src += '<td align=center valign=top nowrap><br><b>outgoing bandwidth limit (KB./sec.)</b>\n'
...
#                    blocks, percent = backups.GetBackupBlocksAndPercent(backupID)
    ...
    def _list(self, request):
        ...
        for backupDir, backupRuns, totalBackupsSizeForDir, is_running, recentBackupID in backup_db.GetBackupsByFolder():
            ...
            dhnio.Dprint(6, '    %s %s %s %s %s ' % (backupDir, 
                                                     str(backupRuns),
                                                     recentBackupID,
                                                     str(is_running),
                                                     str(is_run2),))
...
class BackupDiagramPage(Page):
    ...
    def renderPage(self, request):
        ...
        src += '<td bgcolor="#00f200">&nbsp;&nbsp;&nbsp;&nbsp;</td></tr></table></td>\n'
        src += '<td>local and remote copy is available</td></tr>\n'
        src += '<tr><td><table border=1 cellspacing=0 cellpadding=0><tr>'
        src += '<td bgcolor="#00d200">&nbsp;&nbsp;&nbsp;&nbsp;</td></tr></table></td>\n'
        src += '<td>no local data and remote copy is available</td></tr>\n'
        src += '<tr><td><table border=1 cellspacing=0 cellpadding=0><tr>'
...
class BackupDiagramImage(resource.Resource):
    ...
    colors = {'D': {'000': '#ffffff', # white | nor local, nor remote
                    ...
                    '001': '#ffffff', # whit  | nor local, nor remote, but supplier is here
                    '101': '#d2d2d2', # gray  | only local and user is here
                    '011': '#20d220', # green | only remote and user is here - this should be GREEN!
        ...
        if type == 'bar':
            ...
            if width > 256 and height > 256:
                for supplierNum in range(w):
                    ...
                    x1 = math.cos(a * math.pi / 180.0) * R / 2.0 + x0
                    y1 = math.sin(a * math.pi / 180.0) * R / 2.0 + y0
                    draw.text((x1-20, y1-5), '%s' % nameurl.GetName(contacts.getSupplierID(supplierNum)), fill="#000000")
        img.save(f, "PNG")
        f.seek(0)
        request.write(f.read())
        request.finish()
...
#        src += '<p>This will earn up to %s$ per day, depending on space used.</p>\n' % 'XX.XX'
        ...
        src += '<br><h3>block size: <a href="%s?back=%s">%s</a></h3>\n' % (
...
class SecurityPage(Page):
    ...
    def renderPage(self, request):
        ...
        if action == 'copy':
            ...
            message = '<font color="green">Your private key were copied to file %s</font>' % savefile
            comment = 'your private key were copied to file %s' % savefile
            del TextToSave
...
class MoneyPage(Page):
    ...
    def renderPage(self, request):
        ...
        src += 'Total balance: <b>%6.8f$</b> (<b>%d</b> days remaining)\n' % (bal + balnt, 0)
        src += '<br><br>Transferable balance: <b>%6.8f$</b>\n' % bal
        src += '<br><br>Not transferable balance: <b>%6.8f$</b>\n' % balnt
        src += '</td></tr></table>\n'
        src += '<br><br><br>\n'
...
class MoneyAddPage(Page):
    ...
    def renderPage(self, request):
        action = arg(request, 'action')
        # amount = arg(request, 'amount', '10.0')
...
#            return html(request, body=src, back=back, title='add money')
        ...
        src += '<input type="hidden" name="back" value="%s" />\n' % back
        #src += '<table cellspacing=2 cellpadding=0><tr><td align=right>\n'
        #src += '<input type="text" name="amount" value="%s" size=12 />\n' % amount
        #src += '</td><td align=left>$</td></tr>\n'
        #src += '<tr><td colspan=2>'
    ...
    def renderPage(self, request):
        ...
        if action == '':
            ...
            if res == 0:
                ...
                msg += 'Do you want to transfer <font color=blue><b>%6.8f$</b></font>' % float(amount)
                msg += ' of your total <font color=blue><b>%6.8f$</b></font> transferable funds ' % bal
                msg += ' to user <font color=blue><b>%s</b></font> ?<br>\n' % nameurl.GetName(recipient)
                msg += '<br>Your transferable balance will become <font color=blue><b>%6.8f$</b></font>.' % (float(bal) - float(amount))
                msg += '</td></tr></table>'
                typ = 'info'
            elif res == 1:
...
class ReceiptsPage(Page):
    ...
    def renderPage(self, request):
        ...
        src = '<h1>receipts</h1>\n'
        self.showdaily = arg(request, 'showdaily', 'false')
...
class MonitorTransportsPage(Page):
    ...
    def renderPage(self, request):
        ...
        for info in transfers:
            if not index.has_key(info.remote_idurl):
                index[info.remote_idurl] = {'send': [], 'receive':[]}
            index[info.remote_idurl][info.send_or_receive].append((info.transfer_id, str(info.size)))
        src = ''
        ...
        for idurl in sorted(all):
            ...
            if len(index.get(idurl, {'receive': []})['receive']) > 0:
                src += '<table border=0 cellspacing=0 cellpadding=0><tr>\n'
                for tranfer_id, proto in index[idurl]['receive']:
                    b = bytes_stats[tranfer_id]
                    progress = diskspace.MakeStringFromBytes(b) if b else ''
                    src += '<td>&nbsp;</td>\n'
                    src += '<td bgcolor="#a0a0f0"><font size=-2>&nbsp;%s %s&nbsp;</font></td>\n' % (proto, progress)
                src += '</tr></table>\n'
            else:
                src += '&nbsp;'
            src += '</td>\n'
            src += '<td align=center nowrap><b>%s</b></td>\n' % nameurl.GetName(idurl)
            src += '<td align=left>'
            if len(index.get(idurl, {'send': []})['send']) > 0:
                src += '<table border=0 cellspacing=0 cellpadding=0><tr>\n'
                for tranfer_id, proto in index[idurl]['send']:
                    b = bytes_stats[tranfer_id]
                    progress = diskspace.MakeStringFromBytes(b) if b else ''
                    src += '<td bgcolor="#a0f0a0"><font size=-2>&nbsp;%s %s&nbsp;</font></td>\n' % (proto, progress)
                    src += '<td>&nbsp;</td>\n'
                src += '</tr></table>\n'
            else:
                src += '&nbsp;'
...
#                DHNViewSendCommand('queue both %s %d %d none none' % (
#                    nameurl.GetName(supid), contacts.numberForSupplier(supid), contacts.numSuppliers()))
#            DHNViewSendCommand('queue send %s %d %d F20120205022801PM init' % (
#                nameurl.GetName(contacts.getSupplierID(1)), 1, contacts.numSuppliers()))
#            DHNViewSendCommand('queue send %s %d %d F20120205022802PM init' % (
#                nameurl.GetName(contacts.getSupplierID(2)), 2, contacts.numSuppliers()))
#            DHNViewSendCommand('queue request %s %d %d F20120205022803PM init' % (
#                nameurl.GetName(contacts.getSupplierID(2)), 2, contacts.numSuppliers()))
#            DHNViewSendCommand('queue request %s %d %d F20120205022804PM init' % (
#                nameurl.GetName(contacts.getSupplierID(2)), 2, contacts.numSuppliers()))
        else:
