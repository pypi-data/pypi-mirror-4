...
import binio.types_builder as biniotypes
...
def _parse_dsl_definition( objectDef ):
    triplets = biniotypes._parse_tuples( objectDef )
    items = []
    for row in triplets:
        if len(row) != 3:
            raise ParsingError( 'too many tokens in definition "%s"' % str(row) )
            
        typeObj = biniotypes.__dict__.get( 't_'+row[1].replace('@','_'), None )
        if not typeObj:
...
def _validate_identifier( ident ):
...
def _validate_definition( itemList ):
    ...
    for item in itemList:
        ...
        if type(typeObj) != biniotypes.Type:
