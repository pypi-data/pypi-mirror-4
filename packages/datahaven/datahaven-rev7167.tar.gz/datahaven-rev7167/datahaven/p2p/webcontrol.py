...
class MonitorTransportsPage(Page):
    ...
    def renderPage(self, request):
        ...
        for info in transfers:
            ...
            index[info.remote_idurl][info.send_or_receive].append(info.transfer_id)
        src = ''
        ...
        for idurl in sorted(all):
            ...
            if len(index.get(idurl, {'receive': []})['receive']) > 0:
                ...
                for tranfer_id in index[idurl]['receive']:
                    progress = diskspace.MakeStringFromBytes(bytes_stats[tranfer_id]) 
                    src += '<td>&nbsp;</td>\n'
                    src += '<td bgcolor="#a0a0f0"><font size=-2>&nbsp;%s&nbsp;</font></td>\n' % (progress)
                src += '</tr></table>\n'
            else:
                src += '&nbsp;'
            src += '</td>\n'
            ...
            if len(index.get(idurl, {'send': []})['send']) > 0:
                src += '<table border=0 cellspacing=0 cellpadding=0><tr>\n'
                for tranfer_id in index[idurl]['send']:
                    progress = diskspace.MakeStringFromBytes(bytes_stats[tranfer_id]) 
                    src += '<td bgcolor="#a0f0a0"><font size=-2>&nbsp;%s&nbsp;</font></td>\n' % (progress)
                    src += '<td>&nbsp;</td>\n'
                src += '</tr></table>\n'
            else:
                src += '&nbsp;'
