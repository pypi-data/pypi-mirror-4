    ...
    def getConnection(self, url):
        ...
        try:
            if parts.scheme == "https":
                connection = httplib.HTTPSConnection(parts.netloc, key_file=certfile, cert_file=certfile, timeout=90)
            else:
                connection = httplib.HTTPConnection(parts.netloc, timeout=90)
...
class Client:
	...
	import pprint
        logging.debug(pprint.pformat(parts.path))
    ...
    def getTransferError(self, url, uri):
        ...
        while status in ['PENDING', 'QUEUED', 'EXECUTING', 'UNKNOWN' ]:
            logging.info("status of  %s: %s" %(phaseURL, status))
            time.sleep(1.5)
            status = VOFile(phaseURL, self.conn, method="GET", followRedirect=False).read() 
        #status = VOFile(phaseURL, self.conn, method="GET", followRedirect=False).read()
