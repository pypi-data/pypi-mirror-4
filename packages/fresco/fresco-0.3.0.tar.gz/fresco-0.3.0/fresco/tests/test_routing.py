...
class TestRouteKwargs(object):
    ...
    def test_requestarg_raises_exception(self):
        ...
        app.route('/', GET, view, x=FormArg())
        with app.requestcontext('/'):
            assert_raises(BadRequest, app.view)
...
class TestDelegatedRoutes(object):
    ...
    def test_urlfor_with_dynamic_delegated_route_and_view_self(self):
        ...
        # XXX: maintain context.routecollections[] listing nested
        # RouteCollections associated with the request
        #
        # Each entry should contain (routecollection, urlbase) so we can
        # quickly work out URLs at each level up the tree
        result = []
