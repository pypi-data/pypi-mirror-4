(function($) {
	...
	$.ZTFY.form = {
		...
		initHints: function(form_id) {
			var $form = $('FORM[id='+form_id+']');
			$('INPUT, SELECT, TEXTAREA', $form).each(function(index, item) {
				$(item).tipsy({
					fallback: $('A.hint', $(item).parents('DIV.row')).attr('title'),
					gravity: 'nw'
				});
				$('A.hint', $(item).parents('DIV.row')).hide();
			});
			$('A.hint', $form).tipsy({
				gravity: 'e'
			});
