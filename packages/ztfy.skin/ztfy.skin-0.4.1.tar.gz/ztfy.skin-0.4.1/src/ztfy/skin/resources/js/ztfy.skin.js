		...
		_addCallback: function(result, status) {
			if (status == 'success') {
				...
				if (output == 'ERRORS') {
					...
					$.ZTFY.dialog.close();
					$('DIV.status').remove();
					$('DIV.required-info').after('<div class="status success"><div class="summary">' + $.ZTFY.I18n.DATA_UPDATED + '</div></div>');
				} else if (output == 'NONE') {
					$.ZTFY.dialog.close();
					$('DIV.status').remove();
					$('DIV.required-info').after('<div class="status warning"><div class="summary">' + $.ZTFY.I18n.NO_UPDATE + '</div></div>');
				} else if (output == 'PASS') {
					$.ZTFY.dialog.close();
					$('DIV.status').remove();
