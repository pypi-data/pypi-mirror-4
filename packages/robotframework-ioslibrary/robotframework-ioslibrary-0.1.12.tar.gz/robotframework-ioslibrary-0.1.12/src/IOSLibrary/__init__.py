...
class IOSLibrary(object):
    ...
    def _post(self, endp, request):
        ...
        logging.error("Request to device %s: %s", url, request)

        res = requests.post(url, data=request, headers={
          'Content-Type': 'application/json;charset=utf-8'
        })

        logging.error("Response from device %s: %s", url, res.text)
        return res

    def _get(self, endp):
        logging.error("Request to device %s", urljoin(self._url, endp))
        res = requests.get(urljoin(self._url, endp))
        logging.error("result %s", res.status_code)
    ...
    def _playback(self, recording, options=None):
        ...
        logging.error("text %s",res.text)
        logging.error("status_code %s",res.status_code)
