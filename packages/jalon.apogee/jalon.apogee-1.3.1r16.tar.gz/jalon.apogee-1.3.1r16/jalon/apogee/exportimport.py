...
class ApogeeSettingsXMLAdapter(XMLAdapterBase):

    name = "apogee"

    _LOGGER_ID = "portal_apogee"

    attributes = {
        "layout": {
            "url_connexion" : {'type' : 'Text', 'default' : "dbschema://username:password@hostname/databasename"},
            "cod_anu"       : {'type' : 'Text', 'default' : "1900"},
            "uel"           : {'type' : 'Text', 'default' : "WUEL0"},
            "type_base"     : {'type' : 'Text', 'default' : "apogee"},
        }
    }
    ...
    def _importNode(self, node):
        ...
        self._logger.info('Apogee Settings imported.')
...
def importApogeeSettings(context):
    """Import Apogee Settings"""
    site = context.getSite()
    tool = getToolByName(site, 'portal_apogee', None)
    if tool is None:
        return
...
def exportApogeeSettings(context):
    """Export Apogee Settings"""
    site = context.getSite()
    tool = getToolByName(site, 'portal_apogee', None)
    if tool is None:
        return
