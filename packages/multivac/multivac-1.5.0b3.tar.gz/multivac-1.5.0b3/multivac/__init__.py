...
def setup_mappers(**kwargs):
    ...
    properties['element'] = {
        ...
        '_ancestors'    : relationship(mapping['element'],                                 collection_class=list,
                                       primaryjoin = tables['element'].c.parent_id == tables['element'].c.id,
                                       remote_side = [ tables['element'].c.id ], lazy='dynamic', viewonly=True, uselist=True),
...
class Element(ORM_Base):
    ...
    def filter_tags(tags={}):
        """
        Transform a given query to apply tag-based custom filtering, on top of
        common equality filter.
        
        The filter is specified using a mapping. The key is the name of the tag,
        the value can be as it follows:
         - True  : That tag has to be present
         - None  : That tag has to be None
         - <str> : That tag has to be <str>
        
        @param tags: Tag names and values to apply as a filter
        @type tags: dict
        @return: A transformed query filtering the selected L{Element} instances
                 by their tags
        @rtype: Query
        """
...
class Project(ORM_Base):
    ...
    def filter_tags(tags={}):
        ...
        @param tags: Tag names and values to apply as a filter
        @type tags: dict
        @return: A transformed query filtering the selected L{Project} instances
                 by their tags
        @rtype: Query
        """
