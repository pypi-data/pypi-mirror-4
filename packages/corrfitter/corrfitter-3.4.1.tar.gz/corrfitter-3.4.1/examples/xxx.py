...
def main():
    ...
    for nterm in ntermlist:
    	...
    	for i in range(len(models)):
        	fitter = CorrFitter(models=models[i:i+1], svdcut=svdcut, svdnum=svdnum)
	        fit = fitter.lsqfit(prior=prior, data=data, aux_param=aux_param, p0=p0[i])
	        aux_param.update(fit.p)
	        p0[i] = fit.pmean
        # print fit
        print_results(fitter, prior, data)
        print('\nVERSION 2\n--------')
        fitter = CorrFitter(models=models, svdcut=svdcut, svdnum=svdnum, maxit=2000)
        fit = fitter.lsqfit(prior=prior, data=data, p0=fit.pmean , print_fit=False)
        print_results(fitter, prior, data)
