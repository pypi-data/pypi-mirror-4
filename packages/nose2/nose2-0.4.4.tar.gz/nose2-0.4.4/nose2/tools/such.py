...
class Scenario(object):
    ...
    def _makeGroupTest(self, mod, group, parent_layer=None, position=0):
        ...
        log.debug("Made test case %s with layer %s from %s position %s",
                  case, layer, group, position)
    ...
    def _makeLayer(self, group, parent_layer=None, position=0):
        ...
        log.debug("made layer %s with bases %s and mixins %s at %s",
                  layer, layer.__bases__, layer.mixins, position)
        return layer
