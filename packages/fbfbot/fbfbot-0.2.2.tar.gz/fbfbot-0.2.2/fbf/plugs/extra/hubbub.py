...
from fbf.lib.errors import NoSuchBotType, RequireError
...
from fbf.api.hooks import get_hooks, post_hooks
from fbf.utils.creds import getcredsmod
...
try:
    from fbf.utils.tinyurl import get_tinyurl
except ImportError:
    def get_tinyurl(url):
        return [url, ]
...
class HubbubItem(Persist):
    ...
    def __init__(self, name, url="", owner="", itemslist=['title', 'link'], watchchannels=[], running=1):
        filebase = getdatadir() + os.sep + 'plugs' + os.sep + 'fbf.plugs.extra.hubbub' + os.sep + name
...
examples.add('hb-subscribe', 'subscribe to a feed', 'hb-subscribe fbf https://github.com/feedbackflow/fbfbot/commits/master.atom')
...
examples.add('hb-add', 'hubbub-add <name> <url> to the watcher', 'hb-add fbf https://github.com/feedbackflow/fbfbot/commits/master.atom')
...
examples.add('hb-register', 'hb-register .. register url and start it in one pass', 'hb-register fbfrepo https://github.com/feedbackflow/fbfbot/commits/master.atom')
