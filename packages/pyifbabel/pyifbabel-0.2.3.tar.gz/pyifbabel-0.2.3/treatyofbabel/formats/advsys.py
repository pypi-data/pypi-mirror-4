...
def claim_story_file(file_buffer):
    if len(file_buffer) > 8:
        # bytes 2-8 contain the text "ADVSYS", unobfuscated by adding 30 to
        # each byte and then performing a bitwise NOT on it
        # since the Python NOT (~) operator works only on signed (long) int 
        # values, producing negative values in this case, we instead perform an
        # XOR with 0xff.
