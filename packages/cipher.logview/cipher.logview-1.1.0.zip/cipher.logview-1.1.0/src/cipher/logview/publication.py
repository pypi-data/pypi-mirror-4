...
def getClassOf(view):
    """Return the class of a view."""
    if inspect.ismethod(view):
        cls = view.__self__.__class__
    else:
        cls = view.__class__
...
def getNameOf(view):
    """Return the full name of the view class/method."""
    view = stripAllProxies(view)
    cls = getClassOf(view)
    if inspect.ismethod(view):
        return cls.__module__ + '.' + cls.__name__ + '.' + view.__name__
    else:
...
def getSourceOf(view):
    ...
    view = stripAllProxies(view)
    if not inspect.ismethod(view):
        view = getClassOf(view)
    try:
    ...
    try:
        lines, firstlineno = inspect.getsourcelines(view)
...
def getContext(view, default=UNKNOWN):
    ...
    view = stripAllProxies(view)
    if inspect.ismethod(view):
        context = getattr(view.__self__, 'context', UNKNOWN)
    else:
        context = getattr(view, 'context', UNKNOWN)
