...
2. Call cloud.setkey( api_key=your_api_key, 
   api_secretkey=your_api_secretkey)
3. Set use_simulator to true in cloudconf.py or run cloud.start_simulator() 
   to use the cloud simulator, where cloud is run locally
"""
"""
...
def start_simulator(force_restart = False):
    ...
    Additional logging information by default will be enabled.
    For more information, see 
    `PiCloud documention <http://docs.picloud.com/client_adv.html#simulator>`_
    """
...
def setkey(api_key, api_secretkey=None, server_url=None, restart=False, immutable=False):
    ...
    The *api_secretkey* is stored on your machine.  However, if you have not previously used 
