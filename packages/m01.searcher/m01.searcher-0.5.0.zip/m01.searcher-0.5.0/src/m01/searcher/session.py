...
###############################################################################
"""
$Id: session.py 3072 2012-09-06 03:43:55Z roger.ineichen $
"""
...
class SearchSession(m01.session.session.Session):
    ...
    # no converters required for filters because we store directly in dict
    # This means a filter is not an object if they don't get explicit loaded
    # with self.load
    filters = MongoFieldProperty(interfaces.ISearchSession['filters'])
    ...
    def addFilter(self, filterName, searchFilter):
        ...
        SearchFilter. See load method for more info. 
