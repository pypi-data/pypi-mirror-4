...
class VFrozenDict(VERBase, VEntity, VLockable):
    ...
    def values(self):
        ...
        return tuple(iter(list(self.__values.values())))

    def items(self):
        """Similar to :meth:`dict.items`\ ."""
        return tuple((key, val) for key, val in list(self.__value.items()))

    def _v_as_tagged(self, context):
        tags = VSECodes.DICTIONARY.tags(context)
        data = []
        with self:
            for key, value in list(self.__value.items()):
                data.append(key)
                data.append(value)
            data = tuple(data)
