...
class DualPHModel(object):
    ...
    def _compute_convergence(self):
        model = self._model
        #print(model.dual.getValue(model.beta_sum_one))
        #print(self._compute_xbar_wbar_inner_product())
        #print(self._alphas[self._iter])
        dual_convergence = model.dual.getValue(model.beta_sum_one)+self._compute_xbar_wbar_inner_product()-self._alphas[self._iter]
        ...
        for stage in self._ph._scenario_tree._stages[:-1]: # all blended stages
            for tree_node in stage._tree_nodes:
                ...
                for idx, con_data in iteritems(block.con):
                    ...
                    tree_node._xbars[var_name][var_index] = model.dual.getValue(con_data)
