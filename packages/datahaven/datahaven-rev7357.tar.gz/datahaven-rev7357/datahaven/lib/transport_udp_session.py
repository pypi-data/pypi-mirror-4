...
#              'timer-01sec':    (0.02,   ['SENDING',])}
    ...
    def A(self, event, arg):
        ...
        #---AT_STARTUP---
            ...
            if event == 'init' :
                ...
                self.doDestroyMe(arg)
        #---REMOTE_ID---
        elif self.state is 'REMOTE_ID':
            if event == 'remote-id-received' and not self.isIPPortChanged(arg) :
                self.state = 'PING'
                self.doPing(arg)
            elif event == 'remote-id-received' and self.isIPPortChanged(arg) :
                self.state = 'CLOSED'
                self.doTransportUDPNotify(arg)
                self.doDestroyMe(arg)
    ...
    def isIPPortChanged(self, arg):
        ...
        try:
            ...
            return True
        new_address = identitycache.RemapContactAddress(new_address)
    ...
    def doRequestRemoteID(self, arg):
        if self.remote_idurl is None:
            ...
            # we do not need to request remote ID too often
            if time.time() - self.remote_id_request_last_time < 60*5:
                self.automat('remote-id-received')
            else:
                self.remote_id_request_last_time = time.time()
        ...
        #--- GREETING
            ...
            if greeting_idurl:
                ...
                if self.remote_idurl is None:
                    ...
                    dhnio.Dprint(6, 'transport_udp_session.doReceiveData recognized remote idurl from GREETING packet: %s' % greeting_idurl)
                else:
                    if self.remote_idurl != greeting_idurl:
                        dhnio.Dprint(2, 'transport_udp_session.doReceiveData WARNING incorrect remote idurl in GREETING packet: %s' % greeting_idurl)
            self.sendDatagram(COMMANDS['ALIVE'])
        #--- PING
        elif cmd == 'PING':
            self.sendDatagram(COMMANDS['GREETING']+misc.getLocalID())
    ...
    def doTransportUDPNotify(self, arg):
        reactor.callLater(0, transport_udp.A, 'ip-port-changed', self.remote_idurl)
...
def data_received(datagram, address):
    ...
    if not is_session_opened(address):
        s = open_session(address)
        s.event('init', None)
        dhnio.Dprint(4, 'transport_udp_session.data_received WARNING from unknown address %s, made a new session' % str(address))
