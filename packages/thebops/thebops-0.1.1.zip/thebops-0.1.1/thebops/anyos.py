...
VERSION = (0,
           4, # Bugfix für Linux (keine PATHEXT-Variable)
              # (scanpath-Baustelle noch geöffnet?)
...
def quoted_arg(s):
    u"""
    nimm einen String entgegen und versieh ihn mit Anführungszeichen
    """
...
def find_progs(progname,
               ...
               inroots=None):
    ur"""
    Finde ein geeignetes Programm, das möglicherweise im PATH steht -
    ...
    Alle weiteren Argumente bitte *immer* benannt übergeben, da sich die
    Reihenfolge ändern könnte:

    parentsof -- ein Verzeichnis, dessen übergeordnete Verzeichnisse ebenfalls
                 durchsucht werden.
                 Für lokale Konfigurationsdateien, die Defaults in
                 allgemeineren Verzeichnissen übersteuern; daher als erstes
                 ausgewertet

    indirs -- Sequenz von zu durchsuchenden Verzeichnissen; Umgebungsvariablen
              dürfen in Python-Syntax angegeben werden ('%(name)s')
    ...
    vfunc -- Funktion, die aus den vdirs-Verzeichnisnamen die jeweilige
             Version extrahiert und als Tupel zurückgibt; Verzeichnisse, für
             die diese Funktion None zurückgibt, werden ignoriert
    ...
    xroots -- Auszuschließende Wurzelverzeichnisse, z.B. r'c:\Windows'
              (weil sich darunter Programme finden, die nicht passen,
              weil z.B. das 'find.exe' von Windows etwas völlig anderes
              tut)

    pathvar -- üblicherweise PATH (default)
    ...
    die Namen von Umgebungsvariablen enthalten dürfen, z.B. '%(windir)s'
    ...
    # parents of (für lokale Konfigurationsdateien):
    if parentsof:
    ...
    if vdirs:
        ...
        if found:
            found.sort()
            found.reverse() # höhere Versionen zuerst
            for tup in found:
                yield tup[1]
...
def vsplit_digits(s):
    ...
    enthält (z.B. C:\Program Files\Vim\vim73), und gib diese als Tupel zurück.
    Wandle jede Dezimalziffer für sich in eine Zahl, und brich beim ersten
...
def vdir_digits(s):
    ur"""
    Wende vsplit_digits an auf den Verzeichnisnamen der übergebenen Datei
...
def vsplit_delim(s):
    ...
    Ziffern folgt; '73' ergibt also eine höhere Version als '7.3'
...
def vdir_delim(s):
    ur"""
    Wende vsplit_delim an auf den Verzeichnisnamen der übergebenen Datei
...
def np(s):
    u"""
    gib eine normierte Fassung des übergebenen Pfadstrings zurück
    """
