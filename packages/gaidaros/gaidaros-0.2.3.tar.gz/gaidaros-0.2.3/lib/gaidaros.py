...
class Gaidaros(object):
    ...
    def __init__( self, verbose = None, conf = None, host = None, port = None, ip_version = None, \
        ...
        if conf:
            ...
            cnf_fp.close()
            if verbose == None: verbose = getbool('global', 'verbose')
            if host == None: host = get('global', 'host')
            if port == None: port = getint('global', 'port')
            if ip_version == None: ip_version = getint('global', 'ip_version')
            if backlog == None: backlog = getint('global', 'backlog')
            if poll_timeout == None: poll_timeout = getint('global', 'poll_timeout')
            if recv_size == None: recv_size = getint('global', 'recv_size')
            if handler_class == None: handler_class = get('handler', 'class')
            if handler_class_args == None: handler_class_args = map(lambda x: x.strip(), get('handler', 'class_args').split(','))
            if handler_module == None: handler_module = get('handler', 'module')
            if handle_request == None: handle_request = get('handler', 'handle_request')
            if end_request == None: end_request = get('handler', 'end_request')
            if split_request == None: end_request = get('handler', 'end_request')
