...
class StoragePage(Page):
    ...
    def renderPage(self, request):
        ...
        freeDonatedMB = 0.0
        try:
            for idurl, MB in spaceDict.items():
                if idurl.strip() == 'free':
                    freeDonatedMB = float(MB)
                else:
                    totalCustomersMB += float(MB)
        except:
            dhnio.DprintException()
        currentlyUsedDonatedSpace = dhnio.getDirectorySize(dataDir)
        src = ''
        src += '<h1>storage</h1>\n'
        src += '<table><tr>\n'
        ...
        src += '</table>\n'
        src += '<a href="%s">Go to Suppliers</a>\n' % ('/'+_PAGE_SUPPLIERS)
        src += '</td>\n'
        src += '<td valign=top align=center>\n'
        ...
        src += '<td nowrap>used space</td></tr>\n'
        src += '<tr><td colspan=2 align=center><table border=1 cellspacing=0 cellpadding=0><tr>\n'
        src += '<td bgcolor="#ffffff">&nbsp;&nbsp;&nbsp;&nbsp;</td></tr></table></td>\n'
        src += '<td nowrap>free space</td></tr>\n'
        src += '</table>\n'
        src += '<table>\n'
        src += '<tr><td nowrap>your donated space:</td><td nowrap><b>%s</b></td></tr>\n' % diskspace.MakeStringFromString(settings.getCentralMegabytesDonated())
        src += '<tr><td nowrap>free space on the disk:</td><td nowrap><b>%s</b></td></tr>\n' % diskspace.MakeStringFromBytes(dataDriveFreeSpace)
        src += '<tr><td nowrap>number of customers:</td><td nowrap><b>%d</b></td></tr>\n' % customers_count
        src += '<tr><td nowrap>space taken by customers:</td><td nowrap><b>%s</b></td></tr>\n' % diskspace.MakeStringFromBytes(totalCustomersMB*1024.0*1024.0)
        src += '<tr><td nowrap>free donated space:</td><td nowrap><b>%s</b></td></tr>\n' % diskspace.MakeStringFromBytes(freeDonatedMB*1024.0*1024.0)
        src += '<tr><td nowrap>percentage allocated:</td><td nowrap><b>%3.2f %%</b></td></tr>\n' % (100.0*totalCustomersMB/(totalCustomersMB+freeDonatedMB))
        src += '<tr><td nowrap>space used by customers:</td><td nowrap><b>%s</b></td></tr>\n' % diskspace.MakeStringFromBytes(currentlyUsedDonatedSpace)  
        src += '<tr><td nowrap>percentage used:</td><td nowrap><b>%3.2f %%</b></td></tr>\n' % (100.0*currentlyUsedDonatedSpace/((totalCustomersMB+freeDonatedMB)*1024*1024))
        src += '</table>\n'
        src += '<a href="%s">Go to Customers</a>\n' % ('/'+_PAGE_CUSTOMERS)
        src += '</td>\n'
        src += '</tr></table>\n'
...
class StorageDonatedImage(resource.Resource):
    ...
    def render_GET(self, request):
        ...
        try:
            for idurl, MB in spaceDict.items():
                if idurl.strip() == 'free':
                    freeDonatedBytes = int(float(MB)*1024.0*1024.0)
                else:
                    totalCustomersBytes += int(float(MB)*1024.0*1024.0)
        ...
        try:
            ...
            if width >= 256 and height >= 256:
                for idurl in idurls:
                    ...
                    x1 = math.cos(a * math.pi / 180.0) * R * 0.7 + x0
                    y1 = math.sin(a * math.pi / 180.0) * R * 0.7 + y0
                    s1 = nameurl.GetName(idurl) if idurl != 'free' else idurl 
...
class MoneyPage(Page):
    ...
    def renderPage(self, request):
        ...
        src += 'Total balance: <b>%6.8f$</b>\n' % (bal + balnt) # (<b>%d</b> days remaining)
        src += html_comment('total balance: %6.8f$' % (bal + balnt))
        src += '<br><br>Transferable balance: <b>%6.8f$</b>\n' % bal
        src += html_comment('transferable balance: %6.8f$' % bal)
        src += '<br><br>Not transferable balance: <b>%6.8f$</b>\n' % balnt
        src += html_comment('not transferable balance: %6.8f$' % balnt)
        src += '</td></tr></table>\n'
        src += '<br><br><br>\n'
