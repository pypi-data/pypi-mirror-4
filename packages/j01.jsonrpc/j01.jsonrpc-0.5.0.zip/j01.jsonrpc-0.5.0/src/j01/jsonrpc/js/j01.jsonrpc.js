...
// XXX: implement nextState as a JSONRPC version of nextURL and prevent redirect
...
function j01RenderContentSuccess(response, contentTargetExpression) {
    if (response.nextURL) {
        ...
        if (response.nextHash) {
            // trigger hash change and notify statechange without invoke
            // a jsonrpc call
            alert("nextHash is not implemented yet");
        }
    }
}
...
var J01ContentLoader = function (settings) {
    ...
    if (typeof this.settings.callbackName !== "string") {
        ...
        alert("j01LoadContent: callbackName must be a string not " + t);
    }
