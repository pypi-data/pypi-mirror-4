...
def _PyScript(env, target, source, command = "python $SOURCES", capture=None,
        execute=None, tempdir=None, exit=False, lightweight=True, *args, **kwargs): 
    """
    ...
    basename = None
    if (execute or exit) and tempdir == None:
        m = hashlib.sha1()
        m.update("\n".join(command_sig))
...
PyScript(targets, sources, CommandGeneratorAction(gen,{}))
""" % (repr(command_sig), repr(command))
        file.close()
    ...
    if exit and "exit" in ARGUMENTS: command = "exit %s # %s" % (ARGUMENTS["exit"], basename)
...
def PySconsSetup(env=None, args=ARGUMENTS, tools=['default', PYTOOL()]):
    ...
    if "print" not in args and os.path.exists(force):
        env.Execute("tar xjf " + force)
        env.Execute("mv %s %s" % (force, out))
        env.Exit(0)
