...
class ScalarData(object):
    ...
    def pprint(self, ostream, option, prefix="", repn=None):
        ...
        # the following is specialized logic for handling variable and constraint maps - which
        # are dictionaries of dictionaries, with at a minimum an "id" element per sub-directionary.
        # TODO: This logic should be moved into a separate class for these maps...
        if isinstance(value, dict) and (len(value) > 0) and isinstance(advance_iterator(itervalues(value)), dict) and ("Id" in advance_iterator(itervalues(value))):
            id_dict_map = {}
            ...
            if entries_to_print is True:
                ostream.write("\n")
                for entry_id in sorted(iterkeys(id_dict_map)):
                    if id_nonzeros_map[entry_id] is True:
...
class MapContainer(dict):
    ...
    # XXX begin debugging
    def __del__(self):
        #print("Called __del__ on MapContainer")
        self._order = []
        self.__dict__ = {}
    # XXX end debugging
