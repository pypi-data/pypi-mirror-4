...
def run(args=None):
    ...
    # seed the generator is a user-supplied seed is provided. otherwise,
    ...
    reference_model, reference_instance, full_scenario_tree, scenario_tree_instance = load_models(options)
    if (reference_model is None) or (reference_instance is None) or (full_scenario_tree is None) or (scenario_tree_instance is None):
        raise RuntimeError("***ERROR: Failed to initialize reference model/instance pair and/or the scenario tree.")

    if find_active_objective(reference_model,safety_checks=True).is_minimizing():
    ...
    if AllInOne is True:
        ...
        n_g = options.n_g
        num_scenarios_per_sample = scenario_count
        wasted_scenarios = scenario_count - num_scenarios_for_solution - n_g * num_scenarios_per_sample
    ...
    xhat_ph = ph_for_bundle(0, num_scenarios_for_solution, reference_model, full_scenario_tree, reference_instance, scenario_tree_instance, index_list, options)
    xhat_obj = None
    ...
    if options.solve_xhat_with_ph is False:
        ...
        ef_results = write_and_solve_ef(hatex_ef, xhat_ph._instances, options)
        ...
        if options.verbose is True:
            ...
            print("Time="+time.asctime())
        xhat_ph._scenario_tree.snapshotSolutionFromInstances(xhat_ph._instances)
    ...
    for k in range(1, n_g+1):
        if AllInOne is True:
            ...
            stop_index = start_index + num_scenarios_per_sample - 1
            print("")
            ...
            gk_ph = ph_for_bundle(start_index, stop_index, reference_model, full_scenario_tree, reference_instance, scenario_tree_instance, index_list, options)
        else:
            ...
            reference_model, reference_instance, scenario_tree_for_soln, scenario_tree_instance = load_models(options)
            if (reference_model is None) or (reference_instance is None) or (scenario_tree_for_soln is None) or (scenario_tree_instance is None):
                raise RuntimeError("***ERROR: Failed to initialize reference model/instance pair and/or the scenario tree.")

            gk_ph = create_ph_from_scratch(options, reference_model, reference_instance, scenario_tree_for_soln)
        ...
        ef_results = write_and_solve_ef(gk_ef, gk_ph._instances, options)
        ...
        gk_ph._scenario_tree.snapshotSolutionFromInstances(gk_ph._instances)
        ...
        objective_name, objective = advance_iterator(iteritems(ef_results.solution(0).objective)) # take the first one - we don't know how to deal with multiple objectives.
        xstar_obj_given_xhat = float(objective.value)
        """if sense == 'min':
    ...
    for k in range(0, n_g):
        print("g_supk_of_xhat[k]= "+str(g_supk_of_xhat[k]))
    ...
    if options.write_xhat_solution is True:
        ...
        for key, value in iteritems(root_node._solutions):
            for idx in value:
                if value[idx] != 0.0:
                    print("%s %s %s" % (str(key), str(idx), str(value[idx]())))
    ...
    if options.append_file is not None:
        ...
        if options.write_xhat_solution is True:
            ...
            for key, value in iteritems(root_node._solutions):
                for idx in value:
                    if value[idx] != 0.0:
                        output_file.write("%s %s %s" % (str(key), str(idx), str(value[idx]())))
        output_file.close()
        print("")
...
#
def ph_for_bundle(bundle_start, bundle_stop, reference_model, full_scenario_tree, reference_instance, scenario_tree_instance, index_list, options):

    scenarios_to_bundle = []
    for i in range(bundle_start, bundle_stop+1):
...
def fix_first_stage_vars(scenario_tree, instance):
    ...
    for variable_name, (variable, index_template) in iteritems(stage._variables):
        ...
        scenario_tree_var = root_node._solutions[variable_name]
        instance_var = instance.find_component(variable_name)

        for index in variable_indices:
            if getattr(instance, variable_name)[index].stale is False:
                fix_value = scenario_tree_var[index]
                if isinstance(variable_type, IntegerSet) or isinstance(variable_type, BooleanSet):
                    fix_value = int(round(fix_value))
                instance_var[index].fixed = True
                instance_var[index].value = fix_value
...
def write_and_solve_ef(master_instance, scenario_instances, options):
    ...
    ef_file_name = os.path.expanduser("dlwef.lp")
    if options.verbose:
        print("Writing extensive form to LP file.")
    symbol_map = write_ef(master_instance, scenario_instances, ef_file_name)
    ...
    if ef_solver.warm_start_capable():
        ef_action_handle = ef_solver_manager.queue(ef_file_name, opt=ef_solver, warmstart=False, tee=options.output_ef_solver_log)
    else:
        ef_action_handle = ef_solver_manager.queue(ef_file_name, opt=ef_solver, tee=options.output_ef_solver_log)
    ef_results = ef_solver_manager.wait_for(ef_action_handle)
    ef_results._symbol_map = symbol_map
    os.remove(ef_file_name)
    print("write_and_solve_ef() terminated.")
    return ef_results
