...
class QamasuTestCase(unittest.TestCase):
  ...
  def testRegisterFunc(self):
    self.assertEqual(Func.objects.all().count(), 0)
    self.assertTrue(self.qamasu.register_func('test_worker.for_test_one'))
    self.assertEqual(Func.objects.all().count(), 1)
    f = Func.objects.all()[0]
    self.assertEqual(f.name, 'test_worker.for_test_one')
  ...
  def testFindJob(self):
    ...
    self.assertEqual(job.funcname, 'test_worker.for_test_one')
    job.complete()
    job = self.qamasu.manager.find_job()
  ...
  def testFindJobRespectToPriority(self):
    self.qamasu.register_func('test_worker.for_test_one')
    self.assertEqual(len(self.qamasu.job_list()), 0)
    ...
    self.assertEqual(len(self.qamasu.job_list()), 3)
    
    job = self.qamasu.manager.find_job()
    self.assertEqual(job.arg['number'], 1)
    self.assertEqual(job.org_job.priority, 5)
    job.complete()
    self.assertEqual(len(self.qamasu.job_list()), 2)
    job = self.qamasu.manager.find_job()
    self.assertEqual(job.arg['number'], 2)
    self.assertEqual(job.org_job.priority, 1)
    job.complete()
    self.assertEqual(len(self.qamasu.job_list()), 1)
    job = self.qamasu.manager.find_job()
    self.assertEqual(job.arg['number'], 3)
    self.assertEqual(job.org_job.priority, 3)
    job.complete()
    self.assertEqual(len(self.qamasu.job_list()), 0)
  ...
  def testWorkerForAllFunc(self):
    ...
    job.complete()
    job = qamasu_w_both.manager.find_job()
    self.assertEqual(job.funcname, 'test_worker.for_test_two')
    self.assertEqual(job.arg['number'], 3)
    job.complete()
    job = qamasu_w_both.manager.find_job()
    self.assertEqual(job.funcname, 'test_worker.for_test_one')
    self.assertEqual(job.arg['number'], 4)
    job.complete()
    job = qamasu_w_both.manager.find_job()
    self.assertEqual(job.funcname, 'test_worker.for_test_two')
    self.assertEqual(job.arg['number'], 5)
    job.complete()
  ...
  def testWorkerForTestOne(self):
    ...
    self.assertEqual(len(qamasu_w_one.job_list()), 2)
  ...
  def testWorkerForTestOne(self):
    ...
    self.assertEqual(len(qamasu_w_two.job_list()), 3)
