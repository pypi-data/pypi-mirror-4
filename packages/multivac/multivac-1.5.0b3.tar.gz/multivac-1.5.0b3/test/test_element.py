...
class TestCreate(TestElementBase):
    ...
    def test_id(self, **kwargs):
        ...
        def check_id(field):
            p = Element(**dict({field: self.randint()}))
...
class TestQuery(TestElement):
    ...
    def test_tag_true(self):
        r = self.session.query(Element).with_transformation(Element.filter_tags(tags={'t5': True})).all()
        assert len(r) == 13
        assert self.element['e1533'] in r
    ...
    def test_tag_none(self):
        r = self.session.query(Element).with_transformation(Element.filter_tags(tags={'t5': None})).all()
        assert len(r) == 3
        assert self.element['e1745'] in r
        assert self.element['e2745'] in r
        assert self.element['e3745'] in r
    ...
    def test_tag_value(self):
        r = self.session.query(Element).with_transformation(Element.filter_tags(tags={'t5': 'e3533t5'})).all()
        assert len(r) == 1
        assert self.element['e3533'] in r
    
    def test_tag_filter(self):
        r = self.session.query(Element).filter_by(project=self.project['p2']).with_transformation(Element.filter_tags(tags={'t5': True})).all()
        assert len(r) == 2
        assert self.element['e2533'] in r
        assert self.element['e2745'] in r
