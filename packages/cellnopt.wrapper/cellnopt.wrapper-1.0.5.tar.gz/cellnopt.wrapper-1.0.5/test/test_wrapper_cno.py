...
midasfile = cnodata('MD-ToyMKM.csv')
...
def test_writeMIDAS():
    ...
    cnolist = cnor.CNOlist(midasfile)
    ...
    try:
        ...
        pass
    cnor.writeMIDAS(cnolist, "test.csv", overwrite=True)
    cnolist2 = cnor.CNOlist("test.csv")
    os.remove("test.csv")
...
def test_computeScoreTN():
    """These numbers are correct from version 1.3.19 of CellNOptR before using computeScoreTN"""
    # here we test computeScoreT2 so we need another data set that comprises
    # 2 time steps.
    data = cnor.readMIDAS(midasfile2)
    model = cnor.readSIF(siffile)
    cnolist = cnor.CNOlist(cnor.makeCNOlist(data))
    newmodel = cnor.preprocessing(cnolist, model)
