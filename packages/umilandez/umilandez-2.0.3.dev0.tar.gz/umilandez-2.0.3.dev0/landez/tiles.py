...
class TilesManager(object):
	...
	wal_journal -- sets journal mode for sqlite (default is DELETE mode)
        """
 	...
 	if kwargs.get('wal_journal',True):
	    self.wal_journal = True
	else:
	    self.wal_journal = False
...
class MBTilesBuilder(TilesManager):
    ...
    def run(self, force=False):
        ...
        logger.info(_("%s tiles to be packaged.") % self.nbtiles)
        ...
        # Package it!
	logger.info(_("Metadata information"))
	logger.info(_("Minzoom : '%s' ") % metadata['minzoom'])
	logger.info(_("Maxzoom : '%s' ") % metadata['maxzoom'])
	logger.info(_("Bounds  : '%s' ") % metadata['bounds']) 
        logger.info(_("Build MBTiles file '%s'.") % self.filepath)
        disk_to_mbtiles(self.tmp_dir, self.filepath, wal_journal=self.wal_journal)
    ...
    def _gather(self, (z, x, y)):
        ...
        tmp_dir = os.path.join(self.tmp_dir ,"tiles")
	tmp_dir = os.path.join(tmp_dir,tile_dir)
    ...
    def _clean_gather(self):
        logger.debug(_("Clean-up %s") % os.path.split(self.tmp_dir)[0])
        try:
            shutil.rmtree(os.path.split(self.tmp_dir)[0])
