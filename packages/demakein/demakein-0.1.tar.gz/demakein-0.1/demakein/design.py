...
def sqrt_scaler(value):
    ...
    def func(self):
        scale = self.scale
        return [ item*math.sqrt(scale) for item in value ]
    return func
...
class Instrument_designer(config.Action_with_output_dir):
    ...
    def score(self, inst):
        ...
        for note, fingers in self.fingerings:
            ...
            #score += (abs(w1-w2)/w1) ** 2
            #scale += 1 #w1 ** 2
            #score += ( math.log(w1)-math.log(w2) )**2
            diff = abs(math.log(w1)-math.log(w2))*s
            score += diff*diff*diff / ((100.0 + diff)**2)
            
        return (score/len(self.fingerings))
    ...
    def _save(self, state_vec):
        ...
        for i in range(self.n_holes):
            ...
            if i < self.n_holes-1:
                ...
                text_y = diagram.text(text_x + 45.0, this_y,
        ...
        for note, fingers in self.fingerings:
            w1 = wavelength(note, self.transpose)
            w2, grad = self.instrument.true_wavelength_near(w1, fingers, self.max_grad)
            ...
            probes = [ low * pow(step,i) for i in range(n_probes) ]
            scores = [ self.instrument.resonance_score(probe, fingers) for probe in probes ]
        ...
        tick_x = diagram.min_x * 1.25
        for pos in self.instrument.inner_kinks:
            diagram.line([(tick_x,-pos),(tick_x-5,-pos)])
        for pos in self.instrument.inner.pos[1:-1]:
            diagram.line([(tick_x-2,-pos),(tick_x-3,-pos)])
        for pos in self.instrument.outer_kinks:
            diagram.line([(tick_x-10,-pos),(tick_x-15,-pos)])
        for pos in self.instrument.outer.pos[1:-1]:
            diagram.line([(tick_x-12,-pos),(tick_x-13,-pos)])
    ...
    def run(self):
        ...
        state_vec = optimize.improve('Optimize', self._opt_score, state_vec, monitor=self._save)
