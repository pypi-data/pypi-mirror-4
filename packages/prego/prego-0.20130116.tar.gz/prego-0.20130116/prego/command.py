...
class Command(Assertion):
    def __init__(self, task, cmdline,
                 ...
                 expected=0, timeout=5,
                 signal = signal.SIGTERM,
                 cwd=None):

        self._returncode = None
        interpolator = Interpolator()
        self.cmdline = interpolator.apply(checked_type(str, cmdline))
        self.expected = checked_type((int, NoneType),  expected)
        super(Command, self).__init__(task)

        self._stdout = interpolator.apply(stdout)
        self._stderr = interpolator.apply(stderr)

        cwd = cwd or gvars.context.get('cwd', None) or '.'
        self.cwd     = interpolator.apply(checked_type(str, cwd))

        self.tinit   = time.time()
        self.tend    = None
        self.timeout = checked_type((int, NoneType), timeout)
        self.signal  = checked_type(int, signal)
    ...
    def on_time(self):
        return self.current_elapsed() < self.timeout
    ...
    def wait(self):
        if self.timeout is not None:
            ...
            if not self.terminated:
                self.log.debug('{0}$name timeout exceeded: {1:1.2f}s >= {2:1.2f}s, terminating...'.format(
                        INDENTST, self.current_elapsed(), self.timeout))
