...
class DocumentMeta(type):
    ...
    def __new__(meta, name, bases, attrs):
        klass = super(DocumentMeta, meta).__new__(meta, name, bases, attrs)
        ...
        # Add the backend that this document maps to
        if isinstance(klass._meta.backend_type, str):
            klass._backend_manager = BackendManager(
                klass._meta.backend_type)
            # FIXME: This logic *must* be moved into the backend itself.
            ...
            klass._backend_manager = klass._meta.backend_type()
...
# This construct is valid for python2 and python3. In python2 you would declare
# a metaclass like that:
#     class A(object):
#         __metaclass__ = AMeta
#
# in python3 this looks like:
#     class A(object, metaclass=AMeta): pass
# The approach below works for both versions.
DocumentBase = DocumentMeta('DocumentBase', (object, ), {})
