    ...
    def hack_find(self, name, parent=None):
        ...
        if parent == None:
            ...
            if mod:
                ...
                if filename and '__init__' in filename and os.path.exists(filename):
                    try:
                        #return open(filename, 'U'), filename, (os.path.splitext(filename)[1]
                        cloudLog.debug('django hack resolving %s' % filename)
                        return (None, os.path.split(filename)[0], ('', '', imp.PKG_DIRECTORY))
                    except IOError, i:
                        cloudLog.debug('django hack failed to resolve %s due to exception %s', filename, i)
                        pass
