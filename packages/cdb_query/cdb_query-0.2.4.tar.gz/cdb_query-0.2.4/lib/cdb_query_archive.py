...
def simulations(simulations_desc):
    ...
    for center in paths_dict['data_pointers'].keys():
        if center[0]!='_':
            for model in paths_dict['data_pointers'][center].keys():
                if model[0]!='_':
                    for rip in paths_dict['data_pointers'][center][model].keys():
                        if rip[0]!='_':
                            print center+','+model+','+rip
    return
