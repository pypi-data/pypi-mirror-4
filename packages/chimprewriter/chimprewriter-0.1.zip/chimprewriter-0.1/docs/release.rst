...
Releasing a new version of `chimprewriter` involves the following steps:
...
In the following example we are releasing version 0.1 of `chimprewriter`. The
...
.. sourcecode:: bash
  ...
  $ bin/mkrelease -d pypi -pq ./
  Releasing chimprewriter 0.1
  Tagging chimprewriter 0.1
  To git@github.com:niteoweb/chimprewriter.git
  * [new tag]         0.1 -> 0.1
  running egg_info
  running sdist
