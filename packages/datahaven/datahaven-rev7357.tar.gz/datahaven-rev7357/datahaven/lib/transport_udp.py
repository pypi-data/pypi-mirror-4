            ...
            if idurl not in arg[1]:
                ...
                if address_local != address:
                    if transport_udp_session.is_session_opened(address_local):
                        dhnio.Dprint(6, 'transport_udp.doRemoveOldUDPSession with local peer %s [%s]' % (nameurl.GetName(idurl), str(address_local)))
...
#            idurl = idurls[0] 
#            address = self.remapAddress(address)
#            transport_udp_session.outbox_file(address, idurl, filename, fast, description)
#        else:
