...
def Fail(request):
    dhnio.Dprint(6, "p2p_service.Fail from [%s]: %s" % (newpacket.CreatorID, newpacket.Payload))
...
def Retrieve(request):
    ...
    if not data:
        dhnio.Dprint(1, "p2p_service.Retrieve ERROR empty data on disk " + filename)
    ...
    del data 
    if packet is None:
        dhnio.Dprint(1, "p2p_service.Retrieve ERROR Unserialize fails, not Valid packet " + filename)
