...
class TransportUDPServer():
    ...
    def process_sending(self):
        ...
        for addr in self.peers.keys():
            ...
            for file_id in failed_ids:
                file_sent(addr, self.peers[addr].outgoingFiles[file_id].filename, 'failed', 'udp', None, 'has failed blocks')
    ...
    def process_close_sessions(self):
        ...
        for addr in dead_peers:
            ...
            del self.peers[addr]
        del dead_peers
        dhnio.Dprint(4, 'transport_udp_server.process_close_sessions peers=%d, outbox_files=%d, inbox_files=%d' % (peers_count, outbox_files_count, inbox_files_count))
