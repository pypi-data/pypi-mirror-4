...
def apache_conf_path():
    return '/etc/httpd/conf.d/beanstalk.conf'
...
def apps_pool_path():
    ...
    beanstalk_stack_base = beanstalk_settings['BEANSTALK_STACK_BASE']
    return os.path.join(beanstalk_stack_base, 'apps.json')
...
def setup(settings_path=None, **settings_patches):
    ...
    # Apache conf
    local('touch %s' % apache_conf_path())
...
def create_app(project_name, settings_path=None, **settings_patches):
    ...
    print section_title('Add apache to apache httpd')
    ...
    if not '# %s' % project_name in apache_conf:
        ...
        for command in commands:
            sed_commands.append('$a\\\n%s' % command)
        sed_command = ';'.join(sed_commands)
        local('sed -i.bak -e \'%s\' %s && rm -rf %s.bak' % (sed_command, apache_conf_path(), apache_conf_path()))
...
def commit_app(project_name, processes=2, threads=20, settings_path=None, **settings_patches):
    ...
    project_source = project_source_path(project_name)
    with settings(lcd(project_source), hide('warnings'), warning=True):
