...
class LinearCanonicalRepn(object):
    ...
    def __str__(self):
        return "Constant %s  Linear %s" % (str(self.constant), str(self.linear))
...
def _collect_identity(exp, idMap, multiplier, coef, varmap):
    if len(exp._args) == 1:
        _linear_collectors[exp._args[0].__class__](exp._args[0], idMap, multiplier, coef, varmap)
    elif len(exp._args) == 0:
        pass
    else:
        # GAH: Only because I don't know what to do here.
        raise TypeError( "Unsupported identity expression with multiple terms: %s: %s" % (exp, str(exp._args)) )
...
_linear_collectors = {
    ...
    expr._IdentityExpression          : _collect_identity,
