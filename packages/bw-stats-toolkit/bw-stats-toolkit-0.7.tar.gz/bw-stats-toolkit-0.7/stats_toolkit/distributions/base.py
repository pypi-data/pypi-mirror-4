...
from stats_toolkit.utils import one_row_params_array, construct_params_array
...
* params : A :ref:`params-array`."""
        ...
        if (params['minimum'] > params['amount']).sum() or (
                params['maximum'] < params['amount']).sum():
            raise ImproperBoundsError
...
* threshold : A percentage between 0 and 1. The minimum amount of the distribution covered by the bounds before an error is raised."""
...
{'mean': mean value, 'mode': mode value, 'median': median value, 'upper': upper limit value, 'lower': lower limit value}. All values should be floats (not single-element arrays). Parameters that are not defined should be returned `None`, not omitted.
        """
        return {'mean': params['amount'], 'mode': None, 'median': None,
...
class BoundedUncertaintyBase(UncertaintyBase):
    ...
    def rescale(cls, params):
        ...
        adjusted_means = (params['amount'] - params['minimum']) / scale
