...
_SettingsItems = {
    ...
    '3|emergency'           :('/'+_PAGE_EMERGENCY,          'emergency01.png'),
...
class SupplierRemoteFilesPage(Page):
    ...
    def renderPage(self, request):
        ...
        src += '<p><a href="%s?action=files&back=back">Request a list of My Files from %s</a></p>\n' % (request.path, self.name)
...
class DevelopmentPage(Page):
    ...
    def renderPage(self, request):
        ...
        src += '<br><br><p>You can watch current memory usage on the <a href="%s">Memory page</a>.</p>\n' % ('/'+_PAGE_MEMORY)
...
class EmergencyPage(Page):
    ...
    def renderPage(self, request):
        ...
        src += 'if your backups are not working, or if your machine appears to not be working.\n'
        src += 'Leave the input box blank if you do not wish to use this method.</p>\n'
        src += '<br><br><b>%s</b>\n' % settings.uconfig().get('emergency.emergency-email', 'info')
        src += '<br><br><input type="text" name="email" size="25" value="%s" />\n' % arg(request, 'email')
        src += '<br><br><b>%s</b>\n' % settings.uconfig().get('emergency.emergency-phone', 'info')
        src += '<br><br><input type="text" name="phone" size="25" value="%s" />\n' % arg(request, 'phone')
