...
def has_key(url):
    global _IdentityCache
    return _IdentityCache.has_key(url)
...
def idset(url, id_obj):
    ...
    global _IDURL2Contacts
    if not has_key(url):
        dhnio.Dprint(6, 'identitydb.idset new identity: ' + url)
    _IdentityCache[url] = id_obj
    for contact in id_obj.getContacts():
        if not _Contact2IDURL.has_key(contact):
            _Contact2IDURL[contact] = set()
        else:
            if len(_Contact2IDURL[contact]) >= 1 and url not in _Contact2IDURL[contact]:
                dhnio.Dprint(6, 'identitydb.idset WARNING another user have same contact: ' + str(list(_Contact2IDURL[contact])))
        _Contact2IDURL[contact].add(url)
        if not _IDURL2Contacts.has_key(url):
            _IDURL2Contacts[url] = set()
        _IDURL2Contacts[url].add(contact) 
