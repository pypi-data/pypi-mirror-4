...
class UnknownDependency(PkgmeError):
    ...
    def __init__(self, libraries):
        super(UnknownDependency, self).__init__(
            "Can't find dependency for %s." %
            ', '.join('"%s"' % lib for lib in libraries))
...
def get_packages_for_libraries(library_names, arch):
    """Get packages that provide libraries.

    :param library_names: A set of library names.
    :param arch: A single architecture to get packages for.
    :return: A set of package names that are dependencies that provide
        the libraries.
    """
    conf = load_configuration()
    lib_overrides = conf.libraries_overrides
    needs_overrides = set(lib_overrides.keys()) & library_names
    library_names -= needs_overrides
    # ensure that overrides always "trump" the existing set if they
    # are found
    # XXX: we don't consider arch in overrides, do we need to?
    # -- james_w
    deps = set(lib_overrides[lib] for lib in needs_overrides)
    db = get_dependency_database()
    found = db.get_binaries_for_libraries(library_names, [arch])[arch]
    missing = library_names - set(found.keys())
    if missing:
        raise UnknownDependency(missing)
    for lib_deps in found.values():
        deps |= set(lib_deps)
    return deps
...
def guess_embedded_libs_search_paths(path):
...
def guess_dependencies(path):
    ...
    # go over the rest
    deps = get_packages_for_libraries(libraries, arch)
    return deps, arch
